...
class plgSystemHikashopuser extends hikashopJoomlaPlugin {
	...
	public function __construct(&$subject, $config) {
		...
		if(!empty($this->currency) && version_compare(JVERSION, '4.0', '>=')) {
			$currency = (int)$this->currency;
			$app->getDispatcher()->addListener('onPageCacheGetKey', function($event) use ($currency) {
				$event->addResult('hk_currency_' . $currency);
			});
		}
	...
	public function onUserAuthenticate($credentials, $options, &$response)
	{
		...
		if($this->isWordpressHash($password)) {
			if($this->verifyWordpressPassword($credentials['password'], $password)) {
				$userClass = JUser::getInstance($user->id);

				$userClass->password = $credentials['password']; // This will be hashed by Joomla on save
				$userClass->save(true); // Save user to update password

				$response->status = JAuthentication::STATUS_SUCCESS;
				$response->username = $userClass->username;
				$response->fullname = $userClass->name;
				$response->email = $userClass->email;
				$response->type = 'Wordpress';
	...
	private function isWordpressHash($hash)
	{
		return (
			strpos($hash, '$wp$2y$') === 0 ||
			strpos($hash, '$2y$') === 0 ||
	...
	{
		if(strpos($hash, '$wp$') === 0) {
			$hash = '$' . substr($hash, 4); // Remove 'wp$' prefix
		}
	...
	private function checkPhpassPassword($password, $hash)
	{
		...
		$computed_hash = md5($salt . $password, true);
		do {
			$computed_hash = md5($computed_hash . $password, true);
		} while(--$count);

		$output = substr($hash, 0, 12);
		$output .= $this->encode64($computed_hash, 16, $itoa64);
	...
	public function onAfterProductCreate(&$product) {
		if(defined('HIKASHOP_WORDPRESS')) return;
		$app = JFactory::getApplication();
	...
	public function onAfterRender() {
		...
		$new_body = $this->_injectHtmlAfterFormElement($body, $data);
		if($new_body === false)
			return;

		if($alternate_body)
			$app->setBody($new_body);
		else
			JResponse::setBody($new_body);
	}

	private function _injectHtmlAfterFormElement($body, $data) {
		if(!preg_match('#<form[^>]*class="[^"]*form-validate[^"]*"[^>]*>#is', $body, $match, PREG_OFFSET_CAPTURE))
			return false;

		$form_pos = $match[0][1];
		$rest_of_body = substr($body, $form_pos);

		$fieldset_pos = stripos($rest_of_body, '</fieldset>');
		$table_pos = stripos($rest_of_body, '</table>');

		if($fieldset_pos === false && $table_pos === false)
			return false;

		if($fieldset_pos === false) {
			$insert_offset = $table_pos + strlen('</table>');
		} elseif($table_pos === false) {
			$insert_offset = $fieldset_pos + strlen('</fieldset>');
		} else {
			if($fieldset_pos < $table_pos)
				$insert_offset = $fieldset_pos + strlen('</fieldset>');
			else
				$insert_offset = $table_pos + strlen('</table>');
		}

		$actual_insert_pos = $form_pos + $insert_offset;
		return substr($body, 0, $actual_insert_pos) . $data . substr($body, $actual_insert_pos);
	}
