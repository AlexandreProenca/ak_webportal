...
(function(d,s,id) {
	...
	function _addWhatsappButton(&$plugin, $params) {
		if(!empty($plugin->params['legacy']) || empty($plugin->params['display_whatsapp']))
			return '';
	...
	function _getImageURL(&$element, &$plugin) {
		...
		if(!empty($file_path)) {
			...
			$width = !empty($plugin->params['thumbnail_x']) ? (int)$plugin->params['thumbnail_x'] : 100;
			$height = !empty($plugin->params['thumbnail_y']) ? (int)$plugin->params['thumbnail_y'] : 100;

			$img = $imageHelper->getThumbnail(
				$file_path,
				array('width' => $width, 'height' => $height),
				array('default' => true,'forcesize'=>$config->get('image_force_size',true),'scale'=>$config->get('image_scale_mode','inside'))
			);
			if($img->success) {
				$url = $img->url;
