...
class plgHikashopMassaction_order extends JPlugin
{
	...
	function onMassactionTableTriggersLoad(&$table, &$triggers, &$triggers_html, &$loadedData) {
		...
		$triggers['onAfterOrderConfirmed']=JText::_('AFTER_AN_ORDER_IS_CONFIRMED');
	}
	...
	function onProcessOrderMassFilteraccessLevel(&$elements,&$query,$filter,$num){
		...
		if(count($elements)){
			...
			if(count($user_ids)) {
				...
				if(count($cms_ids)) {
					if(defined('HIKASHOP_WORDPRESS')) {
						$role_slug = hikashop_wp_role_from_rid($filter['group']);
						if($role_slug) {
							foreach($users as $u) {
								$wp_user = get_userdata((int)$u->user_cms_id);
								if($wp_user && in_array($role_slug, $wp_user->roles))
									$groups[$u->user_cms_id] = $u;
							}
						}
					} else {
			...
			if(defined('HIKASHOP_WORDPRESS')) {
				$role_slug = hikashop_wp_role_from_rid($filter['group']);
				if($role_slug) {
					$wp_users = get_users(array('role' => $role_slug, 'fields' => 'ID'));
					if(!empty($wp_users)) {
						$query->leftjoin['user'] = hikashop_table('user').' as hk_user ON hk_order.order_user_id = hk_user.user_id';
						$query->where[] = 'hk_user.user_cms_id '.$filter['type'].' ('.implode(',', array_map('intval', $wp_users)).')';
					}
				}
			} else {
	...
	function onProcessOrderMassActionchangeGroup(&$elements,&$action,$k){
		...
		if(defined('HIKASHOP_WORDPRESS')) {
			$role_slug = hikashop_wp_role_from_rid($action['value']);
			if($role_slug) {
				foreach($user_ids as $cms_id) {
					$wp_user = new WP_User((int)$cms_id);
					if(!$wp_user->exists()) continue;
					if($action['type'] == 'replace')
						$wp_user->set_role($role_slug);
					elseif($action['type'] == 'add')
						$wp_user->add_role($role_slug);
					elseif($action['type'] == 'remove')
	...
	function onBeforeOrderCreate(&$order,&$do){
		if(!$do) return;
		$orders = array($order);
		$result = $this->massaction->trigger('onBeforeOrderCreate',$orders);
	...
	function onBeforeOrderUpdate(&$order,&$do){
		...
		$orders = array($o);
		$result = $this->massaction->trigger('onBeforeOrderUpdate',$orders);
		if(is_array($result) && !empty($result['cancelled'])) {
			$do = false;
			if(!empty($result['message'])) {
				$app = JFactory::getApplication();
	...
	function onBeforeOrderDelete(&$elements, &$do){
		...
		$result = $this->massaction->trigger('onBeforeOrderDelete',$toDelete);
		if(is_array($result) && !empty($result['cancelled'])) {
			$do = false;
			if(!empty($result['message'])) {
				$app = JFactory::getApplication();
	...
	function onAfterOrderCreate(&$order,&$send_email){
		...
		$this->_triggerConfirmed($getOrder, true);
	}
	...
	function onAfterOrderUpdate(&$order,&$send_email){
		...
		$this->_triggerConfirmed($order);
	}

	function _triggerConfirmed(&$order, $new = false){
		...
		$isConfirmed = in_array($order->order_status, $invoice_statuses);
		$wasNotConfirmed = $new || !in_array($order->old->order_status, $invoice_statuses);
		$needToTrigger = $isConfirmed && $wasNotConfirmed;
		if(!$needToTrigger)
			return true;
		$o = clone $order;
		if(!empty($order->old)) {
		...
		$orders = array($o);
		$this->massaction->trigger('onAfterOrderConfirmed',$orders);
	}
