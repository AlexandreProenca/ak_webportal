...
class plgHikashopMassaction_product extends JPlugin
{
	...
	function onProcessProductMassFilterpriceColumn(&$elements,&$query,$filter,$num){
		...
		if(count($elements)){
			...
			$query->group = 'hk_product.product_id';
	...
	function onProcessProductMassFiltercharacteristicColumn(&$elements,&$query,$filter,$num){
		...
		if(count($elements)){
			$db = JFactory::getDBO();
			$db->setQuery('SELECT characteristic_id, characteristic_value, characteristic_parent_id FROM '.hikashop_table('characteristic'). ' ORDER BY characteristic_parent_id ASC, characteristic_ordering ASC');
			$characteristics = $db->loadObjectList('characteristic_id');

			foreach($elements as $k => $element){
				$in = false;
				if(!empty($element->characteristics)){
					foreach($element->characteristics as $charId => $charObj){
						if(!isset($characteristics[$charId]))
							continue;
						if(!isset($characteristics[$characteristics[$charId]->characteristic_parent_id]))
							continue;

						if($characteristics[$characteristics[$charId]->characteristic_parent_id]->characteristic_value == $filter['type'] && $characteristics[$charId]->characteristic_value == $filter['value'])
							$in = true;
					}
				}
				if(!$in) unset($elements[$k]);
			...
			foreach($data->ids as $i => $id){
				...
				if(!empty($data->elements[$id]->price_value_with_tax)){
					...
					if(!empty($data->elements[$id]->product_tax_id)){
						$tax_id_for_conversion = $data->elements[$id]->product_tax_id;
					}elseif(!empty($oldElement->product_tax_id)){
						$tax_id_for_conversion = $oldElement->product_tax_id;
					}else{
						$parent_id = 0;
						if(!empty($data->elements[$id]->product_parent_id))
							$parent_id = (int)$data->elements[$id]->product_parent_id;
						elseif(!empty($oldElement->product_parent_id))
							$parent_id = (int)$oldElement->product_parent_id;

						if(!empty($parent_id)) {
							if(isset($parentTaxCache[$parent_id])) {
								$tax_id_for_conversion = $parentTaxCache[$parent_id];
							} elseif(isset($productClass->all_products[$parent_id]) && !empty($productClass->all_products[$parent_id]->product_tax_id)) {
								$tax_id_for_conversion = $productClass->all_products[$parent_id]->product_tax_id;
								$parentTaxCache[$parent_id] = $tax_id_for_conversion;
							} else {
								$db->setQuery('SELECT product_tax_id FROM '.hikashop_table('product').' WHERE product_id = '.$parent_id);
								$parent_tax = $db->loadResult();
								if(!empty($parent_tax))
					...
					if($tax_id_for_conversion){
						if(strpos($data->elements[$id]->price_value_with_tax,'|')===false){
							...
							$data->elements[$id]->price_value = $currencyHelper->getUntaxedPrice($price,hikashop_getZone(),$tax_id_for_conversion);
						}else{
	...
	function onProcessProductMassActiondisplayResults(&$elements,&$action,$k){
		...
		} elseif($this->massaction->api()){
			echo hikashop_getLayout('massaction','json',$params,$js, true);
		}
	}
	...
	function onBeforeProductCreate(&$element,&$do){
		...
		$result = $this->massaction->trigger('onBeforeProductCreate',$elements);
	...
	function onBeforeProductUpdate(&$element,&$do){
		...
		$result = $this->massaction->trigger('onBeforeProductUpdate',$products);
		if(is_array($result) && !empty($result['cancelled'])) {
			$do = false;
			if(!empty($result['message'])) {
				$app = JFactory::getApplication();
	...
	function onBeforeProductDelete(&$ids,&$do){
		...
		$this->deletedProd =& $products;
		$result = $this->massaction->trigger('onBeforeProductDelete',$products);
		if(is_array($result) && !empty($result['cancelled'])) {
			$do = false;
			if(!empty($result['message'])) {
				$app = JFactory::getApplication();
	...
	function onBeforeProductCopy(&$template, &$product, &$do){
		...
		$result = $this->massaction->trigger('onBeforeProductCopy',$elements);
		if(is_array($result) && !empty($result['cancelled'])) {
			$do = false;
			if(!empty($result['message'])) {
				$app = JFactory::getApplication();
