...
class plgHikashopMassaction_user extends JPlugin
{
	...
	function onBeforeUserCreate(&$element,&$do){
		...
		$result = $this->massaction->trigger('onBeforeUserCreate',$elements);
	...
	function onBeforeUserUpdate(&$element,&$do){
		...
		$users = array($getUser);
		$result = $this->massaction->trigger('onBeforeUserUpdate',$users);
		if(is_array($result) && !empty($result['cancelled'])) {
			$do = false;
			if(!empty($result['message'])) {
				$app = JFactory::getApplication();
	...
	function onBeforeUserDelete(&$element,&$do){
		...
		$result = $this->massaction->trigger('onBeforeUserDelete',$users);
		if(is_array($result) && !empty($result['cancelled'])) {
			$do = false;
			if(!empty($result['message'])) {
				$app = JFactory::getApplication();
