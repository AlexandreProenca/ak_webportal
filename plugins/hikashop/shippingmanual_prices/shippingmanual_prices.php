	...
	function _getShippings($product, $vendor = null, $edit = true) {
		...
		foreach($shippings as $key => $shipping) {
			...
			if(!empty($temp_shipping->shipping_warehouse_filter)) {
				$filter_ids = array_map('intval', explode(',', (string)$temp_shipping->shipping_warehouse_filter));
				$exclude = !empty($temp_shipping->shipping_warehouse_filter_type);
				$pw = !empty($product->product_warehouse_id) ? (int)$product->product_warehouse_id : 0;

				if($pw != 0) {
					$match = in_array($pw, $filter_ids) || in_array(0, $filter_ids);
					if($exclude) $match = !in_array($pw, $filter_ids);
					if(!$match) unset($shippings[$key]);
				} else {
					if(!in_array(0, $filter_ids) && !$exclude)
						unset($shippings[$key]);
				}
			}
