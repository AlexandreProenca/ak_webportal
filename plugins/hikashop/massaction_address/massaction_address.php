...
class plgHikashopMassaction_address extends JPlugin
{
	...
	function onBeforeAddressCreate(&$element,&$do){
		...
		$this->init();
		$result = $this->massaction->trigger('onBeforeAddressCreate',$elements);
	...
	function onBeforeAddressUpdate(&$element,&$do){
		...
		$result = $this->massaction->trigger('onBeforeAddressUpdate',$addresses);
		if(is_array($result) && !empty($result['cancelled'])) {
			$do = false;
			if(!empty($result['message'])) {
				$app = JFactory::getApplication();
	...
	function onBeforeAddressDelete($elements,&$do){
		...
		$result = $this->massaction->trigger('onBeforeAddressDelete',$addresses);
		if(is_array($result) && !empty($result['cancelled'])) {
			$do = false;
			if(!empty($result['message'])) {
				$app = JFactory::getApplication();
