...
class plgHikashopMassaction_category extends JPlugin
{
	...
	function onBeforeCategoryCreate(&$element,&$do){
		...
		$result = $this->massaction->trigger('onBeforeCategoryCreate',$elements);
	...
	function onBeforeCategoryUpdate(&$element,&$do){
		...
		$result = $this->massaction->trigger('onBeforeCategoryUpdate',$categories);
		if(is_array($result) && !empty($result['cancelled'])) {
			$do = false;
			if(!empty($result['message'])) {
				$app = JFactory::getApplication();
	...
	function onBeforeCategoryDelete(&$ids,&$do){
		...
		$result = $this->massaction->trigger('onBeforeCategoryDelete',$toDelete);
		if(is_array($result) && !empty($result['cancelled'])) {
			$do = false;
			if(!empty($result['message'])) {
				$app = JFactory::getApplication();
