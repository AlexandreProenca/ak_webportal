...
if(version_compare($jversion,'5.4.0','>=')) {
	include_once(__DIR__.'/hikashopproduct_j6.php');
} else {
	include_once(__DIR__.'/hikashopproduct_j5.php');
}

class plgButtonHikashopproduct extends plgButtonHikashopproductBridge
{
	...
	function onDisplay($name, $asset='', $author='') {
		...
		if (	$user->authorise('core.edit', $asset)
			...
			$plugin = $pluginsClass->getByName('editors-xtd','hikashopproduct', false);
			...
			if(version_compare($jversion,'5.4.0','>=')) {
				$button = new Joomla\CMS\Editor\Button\Button(
					...
					[
						'action'  => 'modal',
						'link'    => $link,
						'text'    => JText::_('PRODUCT'),
						'icon'    => 'cube',
						'iconSVG' => $iconSVG,
						'name' => 'hikashopproduct',
					]
				);
			} else {
	...
	<script language="JavaScript" type="text/javascript">
		...
		function checkSelect(){
			...
			if( nbboxProd > 0 )
			{
				...
				if(document.getElementById('pricedisc').value==4 && document.getElementById('price').checked==true){
					document.getElementById('product_insert').value =document.getElementById('product_insert').value +  '|pricedis4';
				}
	...
	<form action="<?php echo hikashop_currentURL();?>" method="POST" name="adminForm" id="adminForm">
		...
		<fieldset class="hika_field adminform">
			...
			<div id="productInsertOptions">
				...
				<br/>
				Menu ID : <input type="text" name="menuid" id="menuid"  <?php if(!empty($_REQUEST['menuid'])) echo 'value="'.htmlspecialchars($_REQUEST['menuid'], ENT_QUOTES, 'UTF-8').'"';?> />
