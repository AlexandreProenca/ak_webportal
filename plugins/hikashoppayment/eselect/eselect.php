...
class plgHikashoppaymentEselect extends hikashopPaymentPlugin {
	...
	function onBeforeOrderCreate(&$order, &$do) {
		...
		$db = JFactory::getDBO();
		$db->setQuery('SELECT MAX(order_id)+1 FROM '.hikashop_table('order'));
		$next_order_id = (int)$db->loadResult();
		if(empty($next_order_id))
			$next_order_id = 1;
		$moneris_order_id = hikashop_encode($next_order_id);
		...
		if($ret !== null && $ret != 'null') {
			...
			if( $ret < 50 && $mpgResponse->getComplete() == 'true') {
				...
				$history->notified = 1;
				$history->history_data = 'Moneris TransID: ' . $mpgResponse->getTxnNumber() . "\r\n" . 'ReferenceNum: ' . $mpgResponse->getReferenceNum();
