...
class plgHikashoppaymentPaypalExpress extends hikashopPaymentPlugin
{
	...
	public function onPaymentNotification(&$statuses) {
		...
		if (isset($vars['setExpressCheckout'] ) ) {
			...
			if($amountTheorical <= 0) {
				...
				$existingOrder = $this->_getExistingOrderForCart($cart->cart_id);
				if($existingOrder) {
					$this->app->setUserState('com_hikashop.order_id', $existingOrder->order_id);
					$this->app->setUserState('com_hikashop.order_token', @$existingOrder->order_token);
					hikaInput::get()->set('order_token', $existingOrder->order_token);
				...
				$this->_storeOrderForCart($cart->cart_id, $order->order_id);
	...
	protected function _getExistingOrderForCart($cart_id) {
		$lastOrderId = @$_SESSION['hikashop_paypalexpress_order_id'];
		$lastCartId = @$_SESSION['hikashop_paypalexpress_cart_id'];

		if(empty($lastOrderId) || (int)$lastCartId !== (int)$cart_id)
			return null;
		...
		$query = 'SELECT order_id, order_token FROM ' . hikashop_table('order') .
			' WHERE order_id = ' . (int)$lastOrderId .
			' AND order_status = ' . $db->Quote($created_status) .
			' AND order_type = ' . $db->Quote('sale');
		$db->setQuery($query);
		return $db->loadObject();
	}

	protected function _storeOrderForCart($cart_id, $order_id) {
		$_SESSION['hikashop_paypalexpress_order_id'] = (int)$order_id;
		$_SESSION['hikashop_paypalexpress_cart_id'] = (int)$cart_id;
	}
}
