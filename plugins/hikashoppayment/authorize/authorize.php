...
class plgHikashoppaymentAuthorize extends hikashopPaymentPlugin
{
	...
	function _loadStandardVars(&$order){
		...
		if(!empty($order->cart->billing_address)){
			$vars["x_first_name"]=preg_replace('/[\p{Cc}\p{Cf}]/u', '', mb_substr((string)@$order->cart->billing_address->address_firstname,0,50));
			$vars["x_last_name"]=preg_replace('/[\p{Cc}\p{Cf}]/u', '', mb_substr((string)@$order->cart->billing_address->address_lastname,0,50));
			$vars["x_address"]=preg_replace('/[\p{Cc}\p{Cf}]/u', '', mb_substr((string)@$order->cart->billing_address->address_street,0,60));
			$vars["x_company"]=preg_replace('/[\p{Cc}\p{Cf}]/u', '', mb_substr((string)@$order->cart->billing_address->address_company,0,50));
			...
			$vars["x_city"]=preg_replace('/[\p{Cc}\p{Cf}]/u', '', mb_substr((string)@$order->cart->billing_address->address_city,0,40));
			$vars["x_state"]=mb_substr((string)@$order->cart->billing_address->address_state->zone_name_english,0,40);
			$vars["x_phone"]=preg_replace('/[^\d\s\+\-\.\(\)]+/', '', mb_substr((string)@$order->cart->billing_address->address_telephone,0,25));
		}
		if(!empty($order->cart->shipping_address)){
			$vars["x_ship_to_first_name"]=preg_replace('/[\p{Cc}\p{Cf}]/u', '', mb_substr((string)@$order->cart->shipping_address->address_firstname,0,50));
			$vars["x_ship_to_last_name"]=preg_replace('/[\p{Cc}\p{Cf}]/u', '', mb_substr((string)@$order->cart->shipping_address->address_lastname,0,50));
			$vars["x_ship_to_address"]=preg_replace('/[\p{Cc}\p{Cf}]/u', '', mb_substr((string)@$order->cart->shipping_address->address_street,0,60));
			$vars["x_ship_to_company"]=preg_replace('/[\p{Cc}\p{Cf}]/u', '', mb_substr((string)@$order->cart->shipping_address->address_company,0,50));
			...
			$vars["x_ship_to_city"]=preg_replace('/[\p{Cc}\p{Cf}]/u', '', mb_substr((string)@$order->cart->shipping_address->address_city,0,40));
