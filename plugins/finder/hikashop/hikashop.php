...
class plgFinderHikashop extends plgFinderHikashopBridge
{
	...
	protected function handleOtherLanguages(&$item) {
		...
		if($translationHelper->isMulti()){
			...
			foreach($languages as $language) {
				if(method_exists($item, '__serialize')) {
					$serialize = $item->__serialize();
					$class = $this->resultClass;
					$copy = new $class();
					$serialize[15] = array(); // reset taxonomy
					@$copy->__unserialize($serialize);
				} else {
				...
				$extra = $this->_getElementMenuItem($copy, $language->code);
	...
	protected function _getElementMenuItem(&$item, $code = null) {
		...
		if($config->get('auto_search_menu_item_based_on_main_category', 1)) {
			...
			if(!isset($menuItems[$code]) || is_null($menuItems[$code])) {
				$filters = array(
					'a.type=\'component\'',
					'a.published=1',
					'b.title IS NOT NULL',
				...
				$filters[] = "a.language IN ('*', '', ".$database->Quote($code).")";
				...
				$menuItems[$code] = $database->loadObjectList();
			}

			if (!empty($menuItems[$code])) {
				...
				if(!empty($category)) {
					foreach($menuItems[$code] as $menuItem) {
	...
	protected function getListQuery($query = null)
	{
		...
		if($category) {
			$query->select('c.category_name AS category, c.category_alias as categoryalias, c.category_published AS cat_state');
			$query->select('CASE WHEN c.category_access = \'all\' THEN 1 ELSE 2 END AS cat_access');
		}
