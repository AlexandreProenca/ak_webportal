...
class plgHikashopshippingFedEx2 extends hikashopShippingPlugin
{
	var $pluginConfig = array(
		...
		'group_package' => array('GROUP_PACKAGE', 'boolean', 0),
	...
	var $fedex_methods = array(
		...
		array('key'=>7,'code' => 'SMART_POST', 'name' => 'FedEx Ground Economy / Smart Post', 'countries' => 'USA, PUERTO RICO', 'zones' => array('country_United_States_of_America_223','country_Puerto_Rico_172'), 'destinations' => array('country_United_States_of_America_223','country_Puerto_Rico_172')),
		...
		array('key'=>22,'code' => 'FEDEX_ECONOMY', 'name' => 'FedEx Economy'),
		array('key'=>23,'code' => 'INTERNATIONAL_ECONOMY_FREIGHT', 'name' => 'FedEx International Economy Freight'),
		array('key'=>24,'code' => 'INTERNATIONAL_PRIORITY_FREIGHT', 'name' => 'FedEx International Priority Freight'),
		array('key'=>25,'code' => 'FEDEX_INTERNATIONAL_DEFERRED_FREIGHT', 'name' => 'FedEx International Deferred Freight'),
		array('key'=>26,'code' => 'FEDEX_PRIORITY_FREIGHT', 'name' => 'FedEx Priority Freight'),
		array('key'=>27,'code' => 'FEDEX_ECONOMY_FREIGHT', 'name' => 'FedEx Economy Freight'),
		array('key'=>28,'code' => 'INTERNATIONAL_DISTRIBUTION_FREIGHT', 'name' => 'International Distribution Freight'),
		array('key'=>29,'code' => 'INTERNATIONAL_PRIORITY_DISTRIBUTION', 'name' => 'FedEx International Priority DirectDistribution'),
		array('key'=>30,'code' => 'INTERNATIONAL_ECONOMY_DISTRIBUTION', 'name' => 'FedEx International Economy DirectDistribution'),
		array('key'=>31,'code' => 'FEDEX_REGIONAL_ECONOMY_FREIGHT', 'name' => 'FedEx Regional Economy Freight'),
		array('key'=>32,'code' => 'FEDEX_PRIORITY_EXPRESS_FREIGHT', 'name' => 'FedEx Priority Express Freight'),
		array('key'=>33,'code' => 'FEDEX_ECONOMY_SELECT', 'name' => 'FedEx Economy'),
		array('key'=>34,'code' => 'SAME_DAY_CITY', 'name' => 'FedEx SameDay City'),
		array('key'=>35,'code' => 'SAME_DAY', 'name' => 'FedEx SameDay'),
		array('key'=>36,'code' => 'FEDEX_1_DAY_FREIGHT', 'name' => 'FedEx 1Day Freight'),
		array('key'=>37,'code' => 'FEDEX_2_DAY_FREIGHT', 'name' => 'FedEx 2Day Freight'),
		array('key'=>38,'code' => 'FEDEX_3_DAY_FREIGHT', 'name' => 'FedEx 3Day Freight'),
		array('key'=>39,'code' => 'INTL_GROUND_DISTRIBUTION', 'name' => 'International Ground Distribution (IGD)'),
		array('key'=>40,'code' => 'FEDEX_2_DAY_AM', 'name' => 'FedEx 2Day AM'),
	);
	...
	protected function _fedex_request_rates ($method_params, $address, $order) {
		...
		$addr_state_code = isset($address->shipping_address->address_state->zone_code_3) ? $address->shipping_address->address_state->zone_code_3 : '';
		...
		try {
			...
			if (isset($auth_result->access_token)) {
				...
				$pkg_options = array(
					'weight_unit' => $new_data->fedex_pack_weight_unit,
					'volume_unit' => $new_data->fedex_pack_dim_unit,
				);
				if(empty($method_params->shipping_params->group_package))
					$pkg_options['limit'] = array('unit' => 1);
				$packages = $this->getOrderPackage($new_data, $pkg_options);
				if($packages === false)
					$packages = array();
				elseif(isset($packages['w']))
				...
				foreach($packages as $package) {
					if(empty($package['w']))
						continue;
					$item = (new Item)
						->setWeight((new Weight)->setValue((float)$package['w'])->setUnit($new_data->fedex_weight_unit))
						->setDimensions((new Dimensions)->setLength((float)$package['y'])->setWidth((float)$package['x'])->setHeight((float)$package['z'])->setUnits($new_data->fedex_dim_unit));
					if ($method_params->shipping_params->add_insurance)
						$item->setDeclaredValue((new Value)->setAmount((float)$package['price'])->setCurrency($this->shipping_currency_code));
					$items[] = $item;
				}
	...
	protected function DimWeightProcess ($order, $shipping_params) {
		...
		$new_data->fedex_pack_weight_unit = $weight_unit;
		$new_data->fedex_pack_dim_unit = $Unit_links[$weight_unit];
		return $new_data;
	}
