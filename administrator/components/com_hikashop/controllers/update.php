...
class updateController extends HikashopBridgeController {
	...
	function install(){
		...
		$fromversion = hikaInput::get()->getCmd('fromversion');
		if(!empty($fromversion) && version_compare($fromversion, '6.4.0', '<')) {
			$this->_checkProductViewOverrides();
		}

		$this->_iframe(HIKASHOP_UPDATEURL.'install&fromversion='.$fromversion);
	}

	function _checkProductViewOverrides() {
		$criticalFiles = array(
			'show.php',
			'show_block_characteristics.php',
			'show_quantity.php',
			'show_default.php',
			'option.php'
		);
		...
		$templates = JFolder::folders($templatePath);
		if(empty($templates))
			return;

		foreach($templates as $template) {
			$overridePath = $templatePath . DS . $template . DS . 'html' . DS . 'com_hikashop' . DS . 'product';
			if(!is_dir($overridePath))
				continue;

			foreach($criticalFiles as $file) {
				if(file_exists($overridePath . DS . $file)) {
					$found[] = $template . '/html/com_hikashop/product/' . $file;
				}
			}
		}
		...
		echo '<div style="background:#fff3cd; color:#856404; border:1px solid #ffc107; border-radius:6px; padding:15px 20px; margin:10px 0 15px 0; font-size:14px; line-height:1.6;">'
			. '<div style="font-weight:bold; font-size:16px; margin-bottom:8px;">&#9888; ' . JText::_('HIKASHOP_VIEW_OVERRIDE_WARNING_TITLE') . '</div>'
			. '<p style="margin:0 0 10px 0;">' . JText::_('HIKASHOP_VIEW_OVERRIDE_WARNING_DESC') . '</p>'
			. '<ul style="margin:0 0 10px 15px; padding:0;">';
		foreach($found as $f) {
			echo '<li><code>' . htmlspecialchars($f) . '</code></li>';
		}
		echo '</ul>'
			. '<p style="margin:0;">' . JText::_('HIKASHOP_VIEW_OVERRIDE_WARNING_ACTION') . '</p>'
			. '</div>';
	}
