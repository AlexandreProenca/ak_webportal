...
class OrderController extends hikashopController {
		...
		if($set_address) {
			...
			if(isset($formData['order']['order_user_id'])) {
				...
				if(hikaInput::get()->getString('cart_id','0') != '0'){
					...
					$query = 'UPDATE '.hikashop_table('cart').' SET user_id = '.(int)$user_id.' WHERE cart_id = '.(int)hikaInput::get()->getInt('cart_id',0);
					$db->setQuery($query);
					$db->execute();
	...
	function _cleanOrder(){
		...
		foreach($formData['order'] as $column => $value){
			...
			if($column == 'product') {
				...
				if(is_object($element->product) && !empty($formData['order']['product']['tax_namekeys']) && is_array($formData['order']['product']['tax_namekeys'])) {
					$cleanKeys = array();
					foreach($formData['order']['product']['tax_namekeys'] as $nk)
						$cleanKeys[] = strip_tags($nk);
					$element->product->tax_namekeys = $cleanKeys;
				}
	...
	function step2import() {
		...
		if(empty($import_content)) {
			$app = JFactory::getApplication();
			$app->enqueueMessage(JText::_('ORDER_IMPORT_NO_DATA'), 'error');
			return $this->import();
		}
		...
		if(empty($parsed['header'])) {
			$app = JFactory::getApplication();
			$app->enqueueMessage(JText::_('ORDER_IMPORT_INVALID_FORMAT'), 'error');
			return $this->import();
		}

		$import_mode = $importHelper->detectImportMode($parsed['data'], $parsed['header']);
		...
		$view->assignRef('import_content', $import_content);
		$view->assignRef('import_mode', $import_mode);
	...
	function importprocess() {
		...
		$import_mode = hikaInput::get()->getCmd('import_mode', 'one_order_per_row');
		...
		$fieldClass = hikashop_get('class.field');
		$newFieldTypes = array(
			'__new_order_field__' => 'order',
			'__new_item_field__' => 'item',
			'__new_user_field__' => 'user',
			'__new_address_field__' => 'address',
		);
		foreach($mapping as $csvCol => $hkCol) {
			if(isset($newFieldTypes[$hkCol])) {
				$fieldTable = $newFieldTypes[$hkCol];
				$fieldNamekey = strtolower(preg_replace('/[^a-zA-Z0-9_]/', '_', $csvCol));
				$fieldNamekey = preg_replace('/_+/', '_', trim($fieldNamekey, '_'));

				$existingField = $fieldClass->getField($fieldNamekey, $fieldTable);
				if(empty($existingField)) {
					$field = new stdClass();
					$field->field_table = $fieldTable;
					$field->field_namekey = $fieldNamekey;
					$field->field_realname = $csvCol;
		...
		$imported = $importHelper->processImport($parsed['data'], $mapping, $import_mode);

		$msg = JText::sprintf('X_ORDERS_IMPORTED', $imported);
		if(!empty($importHelper->errors)) {
			$msg .= ' ' . JText::sprintf('ORDER_IMPORT_ERRORS', count($importHelper->errors));
		}
		...
		echo 'setTimeout(function(){ window.parent.location.href="' . hikashop_completeLink('order', false, true) . '"; window.parent.hikashop.closeBox(); }, 2000);' . "\n";
		echo '</script>' . "\n";
		$app = JFactory::getApplication();
		$app->close();
	}
	...
	public function edit() {
		...
		if(!in_array($task , $this->popupSubtasks)) {
			...
			if($tmpl == 'component') {
				$html = ob_get_clean();
				if(strpos($html, 'id="hikashop_messages_') !== false) {
					echo $html;
				}
				$app = JFactory::getApplication();
