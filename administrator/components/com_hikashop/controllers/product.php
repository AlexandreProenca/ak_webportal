...
class ProductController extends hikashopController {
	...
	public function __construct($config = array()) {
		...
		$this->display = array(
			...
			'getprice','formatprice','addimage','selectimage','addfile','selectfile','file_entry',
	...
	public function getSizeFile($url) {
		...
		$headers = !empty($meta['wrapper_data']) ? $meta['wrapper_data'] : null;
		if(!empty($headers) && preg_match($regex, implode("\n", $headers), $matches))
	...
	protected function _saveFile() {
		...
		switch($filemode) {
			...
			case 'video':
				if(isset($formData['videopath'])) {
					$file->file_path = trim($formData['videopath']);
				}
				break;
	...
	public function getprice() {
		...
		$rate_namekeys = hikaInput::get()->get('rate_namekeys', array(), 'array');
		...
		if(!empty($rate_namekeys) && is_array($rate_namekeys)) {
			...
			if($conversion) {
				$totalRate = 0;
				$taxObjects = array();
				foreach($rate_namekeys as $namekey) {
				...
				$basePrice = ($totalRate > 0) ? round($price / (1 + $totalRate), 5) : $price;
			}
			...
			foreach($rate_namekeys as $namekey) {
				$tax = $taxClass->get($namekey);
				if($tax) {
					$taxAmount = round($basePrice * (float)$tax->tax_rate, 5);
					$totalTax += $taxAmount;
					$taxes[] = array(
						'tax_namekey' => $namekey,
						'tax_amount' => $taxAmount,
						'tax_rate' => (float)$tax->tax_rate,
						'tax_amount_formatted' => $currency ? $currencyClass->format($taxAmount, $currency) : $taxAmount,
					);
				}
			}
			...
			$result = array(
				'price' => $basePrice,
				'price_with_tax' => round($basePrice + $totalTax, 5),
				'total_tax' => round($totalTax, 5),
				'taxes' => $taxes,
			);
	...
	public function formatprice() {
		...
		hikashop_nocache();
		hikashop_cleanBuffers();
		echo $currencyClass->format($price, $currency);
		exit;
	}
	...
	public function manageUpload($upload_key, &$ret, $uploadConfig, $caller = '') {
		...
		if($caller == 'galleryselect') {
			$file = new stdClass();
			$type = 'product';
			if(!empty($uploadConfig['extra']['file_type']) && $uploadConfig['extra']['file_type'] == 'file') {
				$type = $uploadConfig['extra']['file_type'];
				$file->file_name = substr($ret->name, 0, strrpos($ret->name, '.'));
			}
			$file->file_type = $type;
			$file->file_ref_id = $product_id;
			$file->file_path = $sub_folder.$ret->name;
