...
if(!function_exists('com_install') && (!defined('JVERSION') || version_compare(JVERSION, '4.0', '<'))) {
...
class hikashopInstall {
	...
	public function displayInfo() {
		...
		$isCli = (PHP_SAPI === 'cli') || ($app && method_exists($app, 'isClient') && $app->isClient('cli'));
		if($isCli) {
			try {
				$updateHelper = hikashop_get('helper.update');
				$updateHelper->runPostInstall();
				echo "HikaShop: plugins and modules installed (headless).\n";
			} catch(\Throwable $e) {
				echo "HikaShop: headless extension install failed: ".$e->getMessage()."\n";
			}
			return;
		}
	...
	public function updatePref() {
		...
		if(version_compare($this->fromVersion,'1.5.6','<')){
			...
			$this->db->setQuery("INSERT IGNORE #__hikashop_config (`config_value`,`config_default`,`config_namekey`) VALUES ('0','1','detailed_tax_display'),('2','2','simplified_breadcrumbs'),(".(int)$config->get('thumbnail_x',100).",'100','product_image_x'),(".(int)$config->get('thumbnail_y',100).",'100','product_image_y');");
			$this->db->execute();
		}
...
CREATE TABLE IF NOT EXISTS `#__hikashop_plugin` (
	...
	PRIMARY KEY (`plugin_id`)
		...
		if(version_compare($this->fromVersion, '6.1.2', '<')) {
			$this->databaseHelper->addColumns("vote", array(
				"`vote_answer` text NOT NULL",
			));
		}


		if(version_compare($this->fromVersion, '6.2.0', '<')) {
			$this->databaseHelper->addColumns("order", array(
				"`order_shipped` INT UNSIGNED DEFAULT 0",
				"`order_delivered` INT UNSIGNED DEFAULT 0"
			));

			$this->db->setQuery("CREATE TABLE IF NOT EXISTS `#__hikashop_withdrawal` (
				`withdrawal_id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
				`withdrawal_order_id` INT(10) UNSIGNED NOT NULL DEFAULT '0',
				`withdrawal_status` VARCHAR(255) NOT NULL DEFAULT '',
				`withdrawal_created` INT(10) UNSIGNED NOT NULL DEFAULT '0',
				`withdrawal_modified` INT(10) UNSIGNED NOT NULL DEFAULT '0',
				`withdrawal_products` TEXT NULL,
				`withdrawal_reason` TEXT NULL,
				`withdrawal_user_check` TINYINT(4) NOT NULL DEFAULT '0',
				PRIMARY KEY (`withdrawal_id`),
				KEY `withdrawal_order_id` (`withdrawal_order_id`)
			) ENGINE=MyISAM DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci");
			try { $this->db->execute(); } catch(Exception $e) {}

			$this->db->setQuery("SELECT config_value FROM #__hikashop_config WHERE config_namekey='withdrawal_order_statuses'");
			if(!$this->db->loadResult()){
				$this->db->setQuery("INSERT INTO #__hikashop_config (`config_namekey`,`config_value`) VALUES ('withdrawal_order_statuses','delivered')");
				try { $this->db->execute(); } catch(Exception $e) {}
				$this->db->setQuery("INSERT INTO #__hikashop_config (`config_namekey`,`config_value`) VALUES ('withdrawal_period','14')");
				try { $this->db->execute(); } catch(Exception $e) {}
			}

			$this->db->setQuery("SELECT orderstatus_id FROM #__hikashop_orderstatus WHERE orderstatus_namekey='delivered'");
			if(!$this->db->loadResult()){
				$this->db->setQuery("INSERT INTO #__hikashop_orderstatus (`orderstatus_name`, `orderstatus_description`, `orderstatus_published`, `orderstatus_ordering`, `orderstatus_namekey`) VALUES ('delivered', 'When an order has been delivered', 1, 8, 'delivered')");
				try { $this->db->execute(); } catch(Exception $e) {}
			}

			$this->databaseHelper->addColumns("product", "KEY `product_alias` (`product_alias`)");
			$this->databaseHelper->addColumns("category", "KEY `category_alias` (`category_alias`)");
		}

		if(version_compare($this->fromVersion, '6.4.1', '<')) {
			$this->databaseHelper->addColumns("zone", "`zone_ordering` INT(10) UNSIGNED NOT NULL DEFAULT 0");
		}

		if(version_compare($this->fromVersion, '6.5.0', '<')) {
			$this->databaseHelper->addColumns("user", "`user_type` enum('registered','guest') NOT NULL DEFAULT 'registered' AFTER `user_cms_id`");

			$this->db->setQuery("UPDATE `#__hikashop_user` SET `user_type` = 'guest' WHERE `user_cms_id` = 0");
			try { $this->db->execute(); } catch(Exception $e) {}

			$this->db->setQuery("ALTER TABLE `#__hikashop_user` DROP INDEX `user_email`");
			try { $this->db->execute(); } catch(Exception $e) {}
			$this->databaseHelper->addColumns("user", "UNIQUE KEY `user_email_type` (`user_email`, `user_type`)");
		}
	}
	...
	public function addPref() {
		...
		$allPref = array(
			...
			'withdrawal_admin_notification.html' => 1,
			'withdrawal_admin_notification.subject' => 'HIKA_WITHDRAWAL_REQUEST_ADMIN_INTRO',
			'withdrawal_admin_notification.published' => 1,
			'withdrawal_admin_notification.template' => 'default',
			'withdrawal_user_notification.html' => 1,
			'withdrawal_user_notification.subject' => 'HIKA_WITHDRAWAL_REQUEST_USER_NOTIFICATION',
			'withdrawal_user_notification.published' => 1,
			'withdrawal_user_notification.template' => 'default',
