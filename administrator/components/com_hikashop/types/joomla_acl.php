	...
	protected function load() {
		...
		if(defined('HIKASHOP_WORDPRESS')) {
			...
			foreach($wp_roles->role_names as $slug => $name) {
				...
				$group->id = $id;
				$group->value = $id;
				$group->text = translate_user_role($name);
...
'.$id.'.callbackCheck = function(treeObj, id, value) {
	...
	if(node.state == 5 || (node.value === "" && node.name)) {
		if(value === true) {
			treeObj.chks("*",false);
			e.value = "all";
		} else if(value === false) {
...
window.aclMgr.updateJoomlaAcl = function(el,id,tree_id) {
	...
	private function getData($values, $allBtn = false, $min = false) {
		...
		if(defined('HIKASHOP_WORDPRESS') && $allBtn) {
			$ret .= '{"status":2,"name":"'.JText::_('HIKA_ALL').'","icon":"folder","value":""';
			if($values == 'all')
				$ret .= ',"checked":true';
			$ret .= ',"data":[';
			$sep = '';
			foreach($this->groups as $k => $group) {
				$ret .= $sep.'{"status":0,"name":"'.str_replace('"','&quot;',$group->text).'"';
				$ret .= ',"value":'.$group->id;
