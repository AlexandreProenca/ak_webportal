...
class hikashopNameboxType extends hikashopType {
	...
	protected $types = array(
		...
		'withdrawal' => array(
			...
			'name' => 'withdrawal_id',
			'mode' => 'list',
			'displayFormat' => '{withdrawal_id} - {order_number}',
			'url' => 'withdrawal&task=findList'
		),
		'rawlist' => array(
	...
	<div class="nametext">
		...
		if(isset($options['checkbox'])) {
			$namebox_options['checkbox'] = $options['checkbox'];
		}
		if(isset($options['tricheckbox'])) {
			$namebox_options['tricheckbox'] = $options['tricheckbox'];
		}
		if(isset($options['checkbox_leaves'])) {
			$namebox_options['checkbox_leaves'] = $options['checkbox_leaves'];
		}
...
<div class="namebox-popup">
	<div style="display:none;" data-oresize="'.$id.'" class="namebox-popup-resize namebox-popup-container">
		<div id="'.$id.'_otree" class="oTree namebox-popup-content"></div>';
			if(!empty($namebox_options['checkbox'])) {
				$ret .= '<div style="position:absolute;bottom:0;right:20px;text-align:center;padding:5px;background-color:transparent;z-index:10;"><button class="btn btn-success" onclick="window.oNameboxes[\''.$id.'\'].validateCheckboxes();return false;">'.JText::_('ADD').'</button></div>';
			}
			$ret .= '
	</div>
</div>';
