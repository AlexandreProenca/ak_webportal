	...
	<div class="'.$options['classes']['mainDiv'].'">
		...
		if(!empty($options['uploadUrls'])) {
			...
			$ret .= '<span id="'.$id.'-btn" class="opload-btn">'.
				$this->popup->display(
					...
					$id.'_uploadpopup',
					(int)$config->get('upload_popup_width', 750), (int)$config->get('upload_popup_height', 460), 'class="hikabtn hikabtn-primary" onclick="return window.hkUploaderList[\''.$id.'\'].uploadFile(this);"'.$opt, '', 'link'
				).
			...
			var hkUploader_'.$id.' = new hkUploaderMgr("'.$id.'", 
				{
					...
					options:{
						maxPostSize:'.(int)$options['maxPostSize'].',
						maxFileSize:'.(int)$options['maxFileSize'].'
					}
				}
			);';
		}
		...
		if(!empty($options['browseUrl'])) {
			...
			$ret .= $this->popup->display(
				...
				$options['browseUrl'],
				$id.'_addpopup',
				(int)$config->get('gallery_popup_width', 1080), (int)$config->get('gallery_popup_height', 650), 'class="hikabtn hikabtn-primary" onclick="return window.hkUploaderList[\''.$id.'\'].browseImage(this);"'.$opt, '', 'link'
			);
		}
