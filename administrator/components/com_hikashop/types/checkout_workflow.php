...
<div id="'.$id.'_container" class="checkout_edition" data-checkout="container" data-checkout-id="'.$id.'">';
		...
		foreach($workflow['steps'] as $step_id => $step_content) {
			list($begin, $end) = $this->getStepTemplate((int)$step_id, isset($step_content['name']) ? $step_content['name'] : '', ($step_id < count($workflow['steps'])-1));
			$html .= $begin;
			unset($begin);
			...
			foreach($step_content['content'] as $content) {
				...
				$html .= str_replace(
					...
					array(
						...
						$this->processParams( $checkoutlist[$content['task']]['params'], isset($content['params']) ? $content['params'] : array() )
					),
...
<dl class="checkout_content_block_column">';
		...
		switch($s['type']) {
			...
			case 'list':
				...
				if(!HIKASHOP_J40)
					...
					$ret .= JHTML::_('select.genericlist',  $s['values'], $name, 'class="custom-select no-chzn" data-checkout-param="'.$k.'" onchange="window.checkoutWorkflowEditor.onChange(this);"', 'value', 'text', $v);
				break;
			case 'groupedlist':
				if(!isset($s['values']))
					break;
			...
			case 'html':
				...
				break;
			case 'textarea':
				$ret .= '<textarea data-checkout-param="'.htmlspecialchars($k, ENT_QUOTES, 'UTF-8').'" '.@$s['attributes'].' onchange="window.checkoutWorkflowEditor.onChange(this);" >'.htmlspecialchars($v, ENT_QUOTES, 'UTF-8').'</textarea>';
				break;
			case 'text':
			default:
				$ret .= '<input type="text" data-checkout-param="'.htmlspecialchars($k, ENT_QUOTES, 'UTF-8').'" value="'.htmlspecialchars($v, ENT_QUOTES, 'UTF-8').'" '.@$s['attributes'].' onchange="window.checkoutWorkflowEditor.onChange(this);" />';
				break;
		}
		return $ret;
	}
	...
	protected function convertLegacyData($checkout_config) {
		$legacy_steps = explode(',', (string)$checkout_config);
