...
';
	...
	public function displayFE(&$element,$params){
		...
		$chromePath = JPATH_THEMES . DS . $app->getTemplate() . DS . $chromeSubDir . DS . 'hikashop_characteristics.php';
		if(!file_exists($chromePath)) {
			$parentTemplate = hikashop_getParentTemplate();
			if(!empty($parentTemplate)) {
				$chromePath = JPATH_THEMES . DS . $parentTemplate . DS . $chromeSubDir . DS . 'hikashop_characteristics.php';
			}
		}
		if(file_exists($chromePath)) {
	...
	public function display($map, $value, $values, $characteristic_display = 'dropdown') {
		...
		foreach($values as $key => $val){
			...
			if(true) {
				...
				$optionValue = ($characteristic_display != 'radio' && !empty($clean) ? $clean : $val);
			}
