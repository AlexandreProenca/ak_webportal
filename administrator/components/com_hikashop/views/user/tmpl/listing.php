...
<div class="hk-row-fluid">
	<div class="hkc-md-10 <?php echo $class_search; ?>"><?php
		echo $this->loadHkLayout('search', array());
	?></div>
	<div class="hkc-md-2 hikashop_listing_filters <?php echo $class_filters; ?>"><?php
		...
		$userTypeOptions = array(
			JHTML::_('select.option', '', JText::_('HIKA_ALL')),
			JHTML::_('select.option', 'registered', JText::_('HIKA_REGISTERED')),
			JHTML::_('select.option', 'guest', JText::_('GUEST')),
		);
		echo JHTML::_('select.genericlist', $userTypeOptions, 'filter_user_type',
			'class="custom-select" onchange="this.form.limitstart.value=0;this.form.submit();"',
			'value', 'text', @$this->pageInfo->filter->filter_user_type);
		if ((!empty($this->extrafilters)) && (count($this->extrafilters))) {
...
<table id="<?php echo $hikashop_id; ?>" class="adminlist table table-striped table-hover" cellpadding="1">
	<thead>
		<tr>
			...
			?></th>
			<th class="title"><?php
				echo JHTML::_('grid.sort', JText::_('HIKA_TYPE'), 'huser.user_type', $this->pageInfo->filter->order->dir, $this->pageInfo->filter->order->value);
			?></th>
<?php
	$count++;
...
<?php } ?>
			...
			<td><?php
				echo $row->user_email;
			?></td>
			<td class="hika_user_type_<?php echo htmlspecialchars(@$row->user_type, ENT_QUOTES, 'UTF-8'); ?>"><?php
				echo JText::_((@$row->user_type === 'guest') ? 'GUEST' : 'HIKA_REGISTERED');
			?></td>
<?php
		if(!empty($this->fields)){
			foreach($this->fields as $field){
