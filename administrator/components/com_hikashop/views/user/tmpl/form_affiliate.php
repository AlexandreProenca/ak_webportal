...
?><div class="hkc-lg-6 hikashop_tile_block hikashop_user_addresses_general"><div>
	...
	<dl class="hika_options large">
		...
		?></dd>
		<dt><label><?php
			echo JText::_('OVERRIDE_CATEGORY_RESTRICTION');
		?></label></dt>
		<dd><?php
			$categories = null;
			$dbCategories = @$this->user->user_params->affiliation_category_restriction;
			if(!empty($dbCategories))
				$categories = explode(',', $dbCategories);
			echo $this->nameboxType->display(
				'data[user][user_params][affiliation_category_restriction]',
				$categories,
