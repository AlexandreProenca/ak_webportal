...
class UserViewUser extends hikashopView {
	...
	public function listing() {
		...
		$pageInfo->filter->filter_user_type = $app->getUserStateFromRequest($this->paramBase.'.filter_user_type', 'filter_user_type', '', 'string');

		$filters = array();
		$order = '';

		if(in_array($pageInfo->filter->filter_user_type, array('registered', 'guest'), true)) {
			$filters[] = 'huser.user_type = ' . $db->Quote($pageInfo->filter->filter_user_type);
		}
	...
	public function form() {
		...
		if(defined('HIKASHOP_WORDPRESS'))
			$url_link = admin_url('user-edit.php?user_id=' . (int)$user->user_cms_id);
		else
