...
class ViewViewView extends hikashopView{
			...
			if(!empty($clientTemplates)){
			  ...
			  if(defined('HIKASHOP_WORDPRESS')) {
				...
				foreach($allThemes as $themeSlug => $themeObj) {
					$wpThemes[] = (object)array(
						'directory' => $themeSlug,
						'path' => $themeObj->get_stylesheet_directory(),
						'published' => true,
						'assigned' => ($themeSlug === $activeTheme),
					);
				}

				for($i = 0; $i < count($wpThemes); $i++) {
					$templateValues[$wpThemes[$i]->directory] = $wpThemes[$i]->directory;

					if(!empty($pageInfo->filter->template) && $wpThemes[$i]->directory != $pageInfo->filter->template) {
						continue;
					}

					$templateFolder = $wpThemes[$i]->path . DS;
					$wpAdminSubdir = ($client_id == 1) ? 'administrator' . DS : '';
					foreach($clientTemplates as $template) {
						$templatePerJoomlaTemplate = clone($template);
						$templatePerJoomlaTemplate->template = $wpThemes[$i]->directory;
						$templatePerJoomlaTemplate->component = $component_name;
						$templatePerJoomlaTemplate->override = $templateFolder . 'hikashop' . DS . $wpAdminSubdir;
					...
					$overrideHtmlDir = $templateFolder . 'hikashop' . DS . $wpAdminSubdir;
					if(is_dir($overrideHtmlDir)) {
						$folders = JFolder::folders($overrideHtmlDir);
						if(!empty($folders)) {
							foreach($folders as $folder) {
								$files = JFolder::files($overrideHtmlDir . $folder);
								if(empty($files))
									continue;
								foreach($files as $file) {
									if(substr($file, -4) != '.php')
									...
									$filename = $overrideHtmlDir . $folder . DS . $file;
									$found = false;
									...
									if(!$found) {
										...
										$obj->client_id = $client_id;
										$obj->view = $folder;
										$obj->template = $wpThemes[$i]->directory;
										$obj->type = 'component';
										$obj->type_name = $component;
										$obj->file = substr($file, 0, strlen($file) - 4);
	...
	public function form() {
		...
		if(!empty($obj->content) && !defined('HIKASHOP_WORDPRESS'))
