...
<form action="<?php echo hikashop_completeLink('order',true); ?>" method="post" name="adminForm">
	<table width="100%" class="admintable table">
		...
		<tr>
			...
			<td>
				<input type="hidden" name="data[order][product][order_product_tax]" value="<?php echo @$this->element->order_product_tax; ?>" />
<?php
				$taxRows = array();
				if(!empty($this->element->order_product_tax_info)) {
					$taxInfo = $this->element->order_product_tax_info;
				...
				if(empty($taxRows)) {
					...
					$emptyTax->tax_namekey = @$this->element->tax_namekey;
					$taxRows[] = $emptyTax;
				}
				foreach($taxRows as $i => $taxInfo) {
					echo $this->ratesType->display('data[order][product][tax_namekeys]['.$i.']', @$taxInfo->tax_namekey, true, '', true);
				}
?>
			</td>
		</tr>
