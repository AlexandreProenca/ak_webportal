...
$isLocked = in_array($this->order->order_status, $locked_statuses);
if($isLocked) {
		...
		window.addEventListener('load', function() {
			...
			for(var i=0; i<selectors.length; i++) {
				...
				for(var j=options.length-1; j>=0; j--) {
					if(lockedStatuses.indexOf(options[j].value) === -1) {
						selectors[i].removeChild(options[j]);
					}
				}
				if(typeof jQuery != 'undefined') {
					jQuery(selectors[i]).trigger('liszt:updated');
					jQuery(selectors[i]).trigger('chosen:updated');
				}
			}
		});
	");
