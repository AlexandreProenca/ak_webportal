...
<div class="hika_edit"><?php
	if(!$this->isLocked) {
		echo $this->popup->display(
			...
			860, 550, 'onclick="return window.orderMgr.addProduct(this);" class="btn btn-primary"', '', 'link'
		);
		echo ' ';
		echo $this->popup->display(
			...
			860, 550, 'onclick="return window.orderMgr.selectProduct(this);" class="btn btn-primary"', '', 'link'
		);
	}
?></div>
...
<!-- PRODUCTS TOP EXTRA DATA -->
<?php if(!empty($this->order->extraData->productsTop)) { echo implode("\r\n", $this->order->extraData->productsTop); } ?>
<!-- EO PRODUCTS TOP EXTRA DATA -->
...
?>
					<td class="hikashop_order_product_<?php echo $namekey; ?>_value"><?php
						if(hikashop_level(2)){
			...
			<td class="hikashop_order_item_price_value"><?php
				...
				if(bccomp(sprintf('%F',$product->order_product_tax),0,5)) {
					...
					if(!empty($product->order_product_tax_info)) {
						$taxInfo = $product->order_product_tax_info;
						if(is_string($taxInfo))
							$taxInfo = hikashop_unserialize($taxInfo);
						if(is_array($taxInfo) && count($taxInfo) > 1) {
							foreach($taxInfo as $t) {
								if(!empty($t->tax_namekey) && $t->tax_namekey != '-1') {
									$taxDetails[] = hikashop_translate($t->tax_namekey) . ': ' . $this->currencyHelper->format(@$t->tax_amount, $this->order->order_currency_id);
								}
							}
						}
					}
					if(!empty($taxDetails)) {
						echo '<br/>' . implode('<br/>', $taxDetails);
					} else {
...
<!-- PRODUCTS BOTTOM EXTRA DATA -->
<?php if(!empty($this->order->extraData->productsBottom)) { echo implode("\r\n", $this->order->extraData->productsBottom); } ?>
<!-- EO PRODUCTS BOTTOM EXTRA DATA -->
<?php
echo $this->popup->display(
	'',
	...
	860, 550, 'style="display:none;"', '', 'link'
);
?>
<script type="text/javascript">
<!--
...
<script type="text/javascript">
<!--
window.orderMgr.updateProducts = function() {
	window.hikashop.xRequest('<?php echo hikashop_completeLink('order&task=show&subtask=products&cid='.$this->order->order_id, true, false, true); ?>', {update: 'hikashop_order_products'});
}
