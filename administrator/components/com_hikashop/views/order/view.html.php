...
class OrderViewOrder extends hikashopView{
	...
	public function listing() {
		...
		$filter_payment = hikaInput::get()->getVar('filter_payment');
		if($filter_payment !== null && $filter_payment != $app->getUserState($this->paramBase.".filter_payment")) {
			$app->setUserState($this->paramBase.'.limitstart', 0);
			$pageInfo->limit->start = 0;
		}
		...
		$searchMap = array('c.id','c.username','c.name','a.user_email','b.order_user_id','b.order_number','b.order_id','b.order_invoice_number','b.order_invoice_id','b.order_full_price','d.address_firstname','d.address_lastname', 'b.order_discount_code', 'b.order_weight', 'b.order_volume');
		...
		if($manage) {
			...
			$importUrl = hikashop_completeLink('order&task=import&tmpl=component');
			$hiddenImportLink = $popupHelper->display('&nbsp;', JText::_('IMPORT'), $importUrl, 'hidden-import-link', 800, 550, 'style="display:none"');
			$doc = JFactory::getDocument();
			$doc->addCustomTag($hiddenImportLink);
		}

		$this->toolbar = array(
			array('name' => 'popup', 'icon' => 'cogs', 'title' => JText::_('HIKASHOP_ACTIONS'), 'alt' => JText::_('HIKASHOP_ACTIONS'), 'url' => hikashop_completeLink('order&task=batch&tmpl=component'), 'width' => $config->get('actions_popup_width','1024'), 'height' => $config->get('actions_popup_height','520'), 'check' => true, 'display' => $manage && JFactory::getUser()->authorise('core.admin', 'com_hikashop')),
			array('name' => 'export'),
			array('name' => 'link', 'icon' => 'import', 'url' => 'javascript:window.hikashop.openBox(document.getElementById(\'hidden-import-link\'));', 'alt' => JText::_('IMPORT'), 'display' => $manage),
	...
	public function show($tpl = null, $toolbar = true) {
		...
		if(empty($this->order) || empty($this->order->order_id)) {
			$this->displayCompleted = true;
			return;
		}
		...
		$this->isLocked = in_array($this->order->order_status, explode(',', $this->config->get('order_locked_statuses','')));
	}
	...
	public function edit_additional($tpl = null) {
		...
		$this->order->shipping_tax_info = array();
		$this->order->discount_tax_info = array();
		$this->order->payment_tax_info = array();
		if(!empty($this->order->order_tax_info)){
			foreach($this->order->order_tax_info as $tax){
				if(isset($tax->tax_amount_for_shipping)){
					...
					$t = new stdClass();
					$t->tax_namekey = $tax->tax_namekey;
					$t->tax_amount = $tax->tax_amount_for_shipping;
					$this->order->shipping_tax_info[] = $t;
				}
				if(isset($tax->tax_amount_for_coupon)){
					$this->order->order_discount_tax_namekey = $tax->tax_namekey;
					$t = new stdClass();
					$t->tax_namekey = $tax->tax_namekey;
					$t->tax_amount = $tax->tax_amount_for_coupon;
					$this->order->discount_tax_info[] = $t;
				}
				if(isset($tax->tax_amount_for_payment)){
					$this->order->order_payment_tax_namekey = $tax->tax_namekey;
					$t = new stdClass();
					$t->tax_namekey = $tax->tax_namekey;
					$t->tax_amount = $tax->tax_amount_for_payment;
					$this->order->payment_tax_info[] = $t;
				}
			}
		}
	}
		...
		if(!empty($users)) {
			...
			if(!empty($rows)) {
				...
				foreach($rows as $v) {
					...
					$db = JFactory::getDBO();
					$db->setQuery('SELECT a.* FROM '.hikashop_table('address').' AS a WHERE a.address_user_id = '.(int)$v->user_id.' AND a.address_published = 0 ORDER BY a.address_id DESC');
					$unpublishedAddresses = $db->loadObjectList('address_id');
					if(!empty($unpublishedAddresses) && !empty($this->addresses)) {
						foreach($this->addresses as $aid => $addr) {
							unset($unpublishedAddresses[$aid]);
						}
					}
					$this->unpublishedAddresses = !empty($unpublishedAddresses) ? $unpublishedAddresses : array();
					$this->isGuestUser = (@$v->user_type === 'guest' || empty($v->user_cms_id) || (int)$v->user_cms_id == 0);
