...
$options = array(
	...
	'toolbar' => array(
		$this->popup->display(
			...
			JText::_('SELECT_A_FILE'),
			hikashop_completeLink('upload&task=galleryimage&uploader=product&field=product_file&product_id='.(int)@$this->product->product_id.'&product_type='.$product_type.'&file_type=product',true),
			'hikashop_file_select',
			750, 460, 'class="hikabtn hikabtn-primary" onclick="return window.productMgr.addFile(this,'.(int)@$this->product->product_id.',\''.$product_type.'\');"'.' data-toggle="hk-tooltip" data-title="'.JText::_('SELECT_A_FILE').'"', '', 'link'
		),
		$this->popup->display(
...
window.productMgr.addSelectedFile = function(files, type) {
	var t = window.hikashop;
	if(type === undefined || type == '') type = 'product';
	if(type == 'variant') type = 'product_variant';
	var d = document, c = d.getElementById('hikashop_'+type+'_file_content');
	var myData = document.createElement('div');
	hkjQuery(myData).html(files.files);
	c.appendChild(myData);
	hkjQuery('#hikashop_'+type+'_file_empty').hide();
	t.closeBox();
	return false;
