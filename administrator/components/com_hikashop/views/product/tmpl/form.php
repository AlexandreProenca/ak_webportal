...
<?php
	...
	$blockWidths = array();
	if(isset($_COOKIE['hikashop_blockWidths'])) {
		$decoded = json_decode($_COOKIE['hikashop_blockWidths'], true);
		if(is_array($decoded)) {
			$validWidths = array('block_width_small', 'block_width_mid', 'block_width_max');
			foreach($decoded as $id => $w) {
				if(in_array($w, $validWidths))
					$blockWidths[preg_replace('/[^a-z0-9_]/', '', $id)] = $w;
			}
		}
	}
	if(empty($blockWidths['description']) && isset($_COOKIE['hikashop_descWidth_cookie'])) {
		$map = array('desc_width_small' => 'block_width_small', 'desc_width_mid' => 'block_width_mid', 'desc_width_max' => 'block_width_max');
		if(isset($map[$_COOKIE['hikashop_descWidth_cookie']]))
			$blockWidths['description'] = $map[$_COOKIE['hikashop_descWidth_cookie']];
	}

	$blockCols = array();
	if(isset($_COOKIE['hikashop_blockCols'])) {
		$decoded = json_decode($_COOKIE['hikashop_blockCols'], true);
		if(is_array($decoded)) {
			$validCols = array('block_cols_1', 'block_cols_2', 'block_cols_3');
			foreach($decoded as $id => $c) {
				if(in_array($c, $validCols))
					$blockCols[preg_replace('/[^a-z0-9_]/', '', $id)] = $c;
			}
		}
	}
	$config = hikashop_config();
	$delay = (int)$config->get('switcher_cookie_retaining_period', 31557600);
	$blockBtnOptions = array('delay' => $delay, 'hideBtn' => $this->customize && hikashop_level(2));
?>
	</div>
<div id="hikashop_product_backend_page_edition" class="<?php echo $customize_css; ?>">
	...
	<div class="hkc-xl-4 hkc-lg-6 hikashop_product_block hikashop_product_edit_general<?php
		if(!empty($blockWidths['general'])) echo ' '.$blockWidths['general'];
		if(!empty($blockCols['general'])) echo ' '.$blockCols['general'];
	?>" data-id="general"><div>
		<div class="hikashop_product_part_title hikashop_product_edit_general_title"><?php
			echo JText::_('MAIN_OPTIONS');
			echo $this->blockButtons('general', $blockBtnOptions);
		?></div>
...
?>
	<div class="hkc-xl-4 hkc-lg-6 hikashop_product_block hikashop_product_edit_images<?php
		if(!empty($blockWidths['images'])) echo ' '.$blockWidths['images'];
	?>" data-id="images"><div>
		<div class="hikashop_product_part_title hikashop_product_upload_title"><?php
			echo JText::_('IMAGES_AND_FILES');
			echo $this->blockButtons('images', array_merge($blockBtnOptions, array('widthOnly' => true)));
		?></div>
<?php
...
?>
	...
	<div class="hkc-xl-4 hkc-lg-6 hikashop_product_block hikashop_product_edit_price<?php
		if(!empty($blockWidths['price'])) echo ' '.$blockWidths['price'];
		if(!empty($blockCols['price'])) echo ' '.$blockCols['price'];
	?>" data-id="prices"><div>
		<div class="hikashop_product_part_title hikashop_product_edit_price_title">
			<?php echo JText::_('PRICES_AND_TAXES');
			echo $this->blockButtons('price', $blockBtnOptions);
			?>
		</div>
<?php
...
<?php
	...
	<div class="hkc-xl-4 hkc-lg-6 hikashop_product_block hikashop_product_edit_description<?php
		if(!empty($blockWidths['description'])) echo ' '.$blockWidths['description'];
	?>" data-id="description"><div>
		<div class="hikashop_product_part_title hikashop_product_edit_description_title"><?php
			echo JText::_('HIKA_DESCRIPTION');
			echo $this->blockButtons('description', array_merge($blockBtnOptions, array('widthOnly' => true)));
		?></div>
		<?php echo $this->editor->display();
		if($this->aiHelper->isAvailable()) {
			$this->aiHelper->renderAssist($this->editor->id, 'description', array(
...
function blockWidth(blockId, delay) {
	var block = document.querySelector('.hikashop_product_edit_' + blockId);
	if(!block) return;
	var states = ['block_width_small', 'block_width_mid', 'block_width_max'];
	var current = -1;
	for(var i = 0; i < states.length; i++) {
		if(block.classList.contains(states[i])) { current = i; break; }
	}
	var next = (current < 0) ? 1 : ((current + 1) % states.length);
	if(current >= 0) block.classList.remove(states[current]);
	block.classList.add(states[next]);

	var cookie = {};
	try { var m = document.cookie.match('(?:^|;)\\s*hikashop_blockWidths=([^;]*)'); if(m) cookie = JSON.parse(decodeURIComponent(m[1])); } catch(e) {}
	cookie[blockId] = states[next];
	window.hikashop.setCookie('hikashop_blockWidths', encodeURIComponent(JSON.stringify(cookie)), delay);
}
function blockCols(blockId, delay) {
	var block = document.querySelector('.hikashop_product_edit_' + blockId);
	if(!block) return;
	var states = ['block_cols_1', 'block_cols_2', 'block_cols_3'];
	var current = -1;
	for(var i = 0; i < states.length; i++) {
		if(block.classList.contains(states[i])) { current = i; break; }
	}
	...
	var cookie = {};
	try { var m = document.cookie.match('(?:^|;)\\s*hikashop_blockCols=([^;]*)'); if(m) cookie = JSON.parse(decodeURIComponent(m[1])); } catch(e) {}
	cookie[blockId] = states[next];
	window.hikashop.setCookie('hikashop_blockCols', encodeURIComponent(JSON.stringify(cookie)), delay);
}
<?php 
...
?>
	<div class="hkc-xl-4 hkc-lg-6 hikashop_product_block hikashop_product_edit_meta<?php
		if(!empty($blockWidths['meta'])) echo ' '.$blockWidths['meta'];
		if(!empty($blockCols['meta'])) echo ' '.$blockCols['meta'];
	?>" data-id="meta"><div>
		<div class="hikashop_product_part_title hikashop_product_edit_meta_title"><?php
			echo JText::_('SEO');
			echo $this->blockButtons('meta', $blockBtnOptions);
		?></div>
		<dl class="hika_options">
<?php
...
?>
	...
	<div class="hkc-xl-4 hkc-lg-6 hikashop_product_block hikashop_product_edit_restrictions<?php
		if(!empty($blockWidths['restrictions'])) echo ' '.$blockWidths['restrictions'];
		if(!empty($blockCols['restrictions'])) echo ' '.$blockCols['restrictions'];
	?>" data-id="restrictions"><div>
		<div class="hikashop_product_part_title hikashop_product_edit_restrictions_title"><?php
			...
			echo $this->blockButtons('restrictions', $blockBtnOptions);
		?></div>
		<dl class="hika_options">
<?php
...
?>
	...
	<div class="hkc-xl-4 hkc-lg-6 hikashop_product_block hikashop_product_edit_specifications<?php
		if(!empty($blockWidths['specifications'])) echo ' '.$blockWidths['specifications'];
		if(!empty($blockCols['specifications'])) echo ' '.$blockCols['specifications'];
	?>" data-id="specifications"><div>
		<div class="hikashop_product_part_title hikashop_product_edit_specifications_title"><?php
			echo JText::_('SPECIFICATIONS');
			echo $this->blockButtons('specifications', $blockBtnOptions);
		?></div>
		<dl class="hika_options">
<?php
...
?>
	...
	<div class="hkc-xl-4 hkc-lg-6 hikashop_product_block hikashop_product_edit_display<?php
		if(!empty($blockWidths['display'])) echo ' '.$blockWidths['display'];
		if(!empty($blockCols['display'])) echo ' '.$blockCols['display'];
	?>" data-id="display"><div>
		<div class="hikashop_product_part_title hikashop_product_edit_display_title"><?php
			echo JText::_('DISPLAY');
			echo $this->blockButtons('display', $blockBtnOptions);
		?></div>
		<dl class="hika_options">
<?php
...
?>
	<div class="hkc-xl-4 hkc-lg-6 hikashop_product_block hikashop_product_edit_fields<?php
		if(!empty($blockWidths['fields'])) echo ' '.$blockWidths['fields'];
		if(!empty($blockCols['fields'])) echo ' '.$blockCols['fields'];
	?>" data-id="fields"><div>
		<div class="hikashop_product_part_title hikashop_product_edit_fields_title"><?php
			echo JText::_('FIELDS');
			echo $this->blockButtons('fields', $blockBtnOptions);
		?></div>
<?php
...
window.hikashop.ready( function() {
	window.productDragOptionsKey = window.formCustom.initDragAndDrop({
		...
		widthTip: <?php echo json_encode(JText::_('HIKA_BLOCK_WIDTH')); ?>,
		colsTip: <?php echo json_encode(JText::_('HIKA_BLOCK_COLS')); ?>,
		toggleTip: <?php echo json_encode(JText::_('HIKA_TOGGLE_BLOCK')); ?>,
		moveUpTip: <?php echo json_encode(JText::_('HIKA_MOVE_UP')); ?>,
		moveDownTip: <?php echo json_encode(JText::_('HIKA_MOVE_DOWN')); ?>,
	});
});
</script>
<?php
