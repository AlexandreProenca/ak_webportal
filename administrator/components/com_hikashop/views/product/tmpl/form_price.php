...
<?php // Let plugins add inputs to a saved price row (e.g. its price category).
if(!empty($this->extraData->priceRowFields[$i])) { echo implode("\r\n", $this->extraData->priceRowFields[$i]); } ?>
			</td>
...
<?php // Same slot for the JS row template used when adding a new price row.
if(!empty($this->extraData->priceRowTemplateFields)) { echo implode("\r\n", $this->extraData->priceRowTemplateFields); } ?>
			</td>
			<td style="text-align:center;">
				{EDIT_BUTTON}
				<a href="#delete" onclick="window.hikashop.deleteRow(this); return false;" title="<?php echo JText::_('HIKA_DELETE'); ?>"><i class="fas fa-trash"></i></a>
...
window.productMgr.addPrice = function(formkey) {
	...
	if(typeof w.productMgr.afterAddPriceRow === 'function') w.productMgr.afterAddPriceRow(formkey, newRow, max_id, id);
