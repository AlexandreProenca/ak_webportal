...
class ProductViewProduct extends hikashopView
{
	...
	public function listing() {
		...
		$this->toolbar = array(
			array('name' => 'export'),
			array('name' => 'popup', 'icon' => 'cogs', 'title' => JText::_('HIKASHOP_ACTIONS'), 'alt' => JText::_('HIKASHOP_ACTIONS'), 'url' => hikashop_completeLink('product&task=batch&tmpl=component'), $config->get('actions_popup_width','1024'), 'height' => $config->get('actions_popup_height','520'), 'check' => true, 'display' => $manage && JFactory::getUser()->authorise('core.admin', 'com_hikashop')),
	...
	public function form_legacy() {
		...
		if(!empty($product_id)){
			...
			if(empty($element)){
				...
				$this->reloadFailedProductImages($element, $database);
			}
			$task='add';
		}
		if(!empty($element->related)){
	...
	protected function reloadFailedProductImages(&$element, $db) {
		if(empty($element->images))
			return;
		hikashop_toInteger($element->images);
		$image_ids = $element->images;
		$db->setQuery('SELECT * FROM '.hikashop_table('file').' WHERE file_id IN ('.implode(',', $image_ids).')');
		$loaded_images = $db->loadObjectList('file_id');
		$element->images = array();
		foreach($image_ids as $image_id) {
			if(isset($loaded_images[$image_id]))
				$element->images[] = $loaded_images[$image_id];
		}
	}
	...
	public function galleryimage() {
		...
		$pageInfo = new stdClass();
		$pageInfo->limit = new stdClass();
		$pageInfo->limit->value = $app->getUserStateFromRequest( $this->paramBase.'.gallery.list_limit', 'limit', 50, 'int' );
	...
	public function form() {
		...
		if(!empty($product_id)) {
			...
			$this->reloadFailedProductImages($product, $db);
		} else {
			$product = new stdClass();
		...
		$this->assignRef('aiHelper', $aiHelper);
	...
	public function blockButtons($blockId, $options = array()) {
		$delay = isset($options['delay']) ? $options['delay'] : 0;
		$widthOnly = !empty($options['widthOnly']);
		$hideBtn = !empty($options['hideBtn']);

		$html = '<button type="button" onclick="blockWidth(\''.$blockId.'\', \''.$delay.'\'); return false;" class="hikashop_block_width hikabtn hikabtn-primary" style="display:inline-block;" data-toggle="hk-tooltip" title="'.JText::_('HIKA_BLOCK_WIDTH').'"><i class="fas fa-chevron-left" aria-hidden="true"></i><i class="fas fa-chevron-right" aria-hidden="true"></i><span class="element-invisible">'.JText::_('HIKA_BLOCK_WIDTH').'</span></button>';
		if(!$widthOnly)
			$html .= '<button type="button" onclick="blockCols(\''.$blockId.'\', \''.$delay.'\'); return false;" class="hikashop_block_cols hikabtn hikabtn-primary" style="display:inline-block;" data-toggle="hk-tooltip" title="'.JText::_('HIKA_BLOCK_COLS').'"><i class="fas fa-columns" aria-hidden="true"></i><span class="element-invisible">'.JText::_('HIKA_BLOCK_COLS').'</span></button>';
		if($hideBtn)
			$html .= '<button type="button" onclick="window.formCustom.toggleBlock(\''.$blockId.'\', window.productDragOptionsKey, true); return false;" class="hikabtn hikabtn-primary btn-small block-hide-btn" style="display:inline;" data-toggle="hk-tooltip" title="'.JText::_('HIKA_TOGGLE_BLOCK').'"><i class="fa fa-eye" aria-hidden="true"></i><span class="element-invisible">'.JText::_('HIKA_TOGGLE_BLOCK').'</span></button>';
		return $html;
	}
