...
class DiscountViewDiscount extends hikashopView {
	...
	public function listing($extendedData = true) {
		...
		if($manage) {
			...
			$currentType = hikaInput::get()->getCmd('filter_type', '');
			if(empty($currentType) || !in_array($currentType, array('coupon', 'discount'), true)) {
				$currentType = (string)$app->getUserState($this->paramBase.'.filter_type', '');
			}
			$importUrl = hikashop_completeLink('discount&task=import&tmpl=component' . (in_array($currentType, array('coupon', 'discount'), true) ? '&filter_type='.$currentType : ''));
