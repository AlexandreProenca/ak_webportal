...
class uploadViewupload extends hikashopView {
	...
	public function galleryimage() {
		...
		$this->popupTitle = JText::_('HIKA_SELECT_IMAGE');
		if(!empty($this->uploadConfig['type']) && $this->uploadConfig['type'] == 'file') {
			$this->popupTitle = JText::_('SELECT_A_FILE');
			$uploadFolder = ltrim(JPath::clean(html_entity_decode($config->get('uploadsecurefolder'))),DS);
			$uploadFolder = rtrim($uploadFolder,DS).DS;
			$basePath = JPATH_ROOT.DS.$uploadFolder.DS;
		}
		...
		$pageInfo->limit->value = $app->getUserStateFromRequest( $this->paramBase.'.list_limit', 'limit', 50, 'int' );
		...
		$app = JFactory::getApplication();
		if(empty($folder))
			$folder = $app->getUserState('com_hikashop.gallery_folder', '');
		$destFolder = rtrim($folder, '/\\');
		if(!$galleryHelper->validatePath($destFolder))
			$destFolder = '';
		if(!empty($destFolder)) $destFolder .= '/';
		$app->setUserState('com_hikashop.gallery_folder', rtrim($destFolder, '/'));
		...
		$galleryOptions = array(
			...
			'type' => (empty($this->uploadConfig['type']) ? 'image' : $this->uploadConfig['type'])
		);
