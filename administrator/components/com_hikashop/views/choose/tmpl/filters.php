...
<div style="clear:both"/>
	<table class="adminlist table table-striped" cellpadding="1">
		...
		<tbody>
			...
			?>
				<tr class="<?php echo empty($row->selected) ? "row$k" : 'selectedrow'; ?>" id="content<?php echo htmlspecialchars($row->filter_namekey, ENT_QUOTES, 'UTF-8'); ?>" onclick="applyContent('<?php echo htmlspecialchars($row->filter_namekey, ENT_QUOTES, 'UTF-8')."','row$k'"?>);" style="cursor:pointer;">
					<td>
					<?php echo htmlspecialchars($row->filter_namekey, ENT_QUOTES, 'UTF-8'); ?>
					</td>
					<td>
