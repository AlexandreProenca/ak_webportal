...
<?php } ?>
	<tr>
		<td class="hk_tbl_key"<?php echo $this->docTip('address_form_layout');?>><?php echo JText::_('ADDRESS_FORM_LAYOUT'); ?></td>
		<td>
	<div style="display:flex; align-items:center; gap: 10px;">
		<?php
			$isEnabled = $this->config->get('address_form_layout_enabled', 0);
			echo JHTML::_('hikaselect.booleanlist', 'config[address_form_layout_enabled]', '', $isEnabled);
			echo $this->popup->display(
				'<button type="button" class="btn btn-primary"><i class="fas fa-th-large"></i> ' . JText::_('CONFIGURE_LAYOUT') . '</button>',
				'ADDRESS_FORM_LAYOUT',
				hikashop_completeLink('config&task=addresslayout&tmpl=component', true),
				'address_layout_popup',
				1200, 750, '', '', 'link'
			);
		?>
	</div>
...
<?php
	...
	if($joomla_params && (int)$joomla_params->get('allowUserRegistration') == 0){
