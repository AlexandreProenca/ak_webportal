...
<table class="hk_config_table table" style="width:100%">
	<tr>
		...
		<td>
			HikaShop <?php echo $this->config->get('level').' '.$this->config->get('version'); ?> [2607090929]
		</td>
	</tr>
	<tr>
		<td class="hk_tbl_key"><?php echo JText::_('HIKA_CHECK_FOR_UPDATES'); ?></td>
		<td>
<?php if(!defined('HIKASHOP_WORDPRESS')) { ?>
			<button type="button" class="btn btn-primary" id="hika_check_update_btn" onclick="hikaCheckUpdate(false); return false;">
				<?php echo JText::_('HIKA_CHECK_NOW'); ?>
			</button>
			<span id="hika_check_update_spinner" style="display:none;margin-left:10px;"><?php echo JText::_('HIKA_CHECKING'); ?>...</span>
<?php } ?>
<?php
	$currentLevel = strtolower($this->config->get('level', 'Starter'));
	if($currentLevel !== 'business') {
		if($currentLevel === 'starter') {
?>
			<button type="button" class="btn btn-success" onclick="hikaCheckUpdate('Essential'); return false;" style="margin-left:10px;">
				<?php echo JText::sprintf('HIKA_UPGRADE_TO_X', 'Essential'); ?>
			</button>
<?php
		}
?>
			<button type="button" class="btn btn-success" onclick="hikaCheckUpdate('Business'); return false;" style="margin-left:10px;">
				<?php echo JText::sprintf('HIKA_UPGRADE_TO_X', 'Business'); ?>
			</button>
			<span id="hika_upgrade_spinner" style="display:none;margin-left:10px;"><?php echo JText::_('HIKA_CHECKING'); ?>...</span>
<?php
	}
?>
<?php if(!defined('HIKASHOP_WORDPRESS')) { ?>
			<div id="hika_check_update_result" style="margin-top:10px;"></div>
<?php } ?>
<?php if($currentLevel !== 'business') { ?>
			<div id="hika_upgrade_result" style="margin-top:10px;"></div>
<?php } ?>
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td><?php
			$roundValues = array(
				...
				JHTML::_('select.option', 2, JText::_('ROUND_CALCULATIONS_WITHOUT_INCREMENT')),
			);
			echo JHTML::_('hikaselect.genericlist', $roundValues, 'config[round_calculations]', 'class="custom-select" size="1"', 'value', 'text', $this->config->get('round_calculations', 0));
		?></td>
	</tr>
	<tr>
...
<?php if(defined('HIKASHOP_WORDPRESS')) { ?>
	<tr>
		<td class="hk_tbl_key"><?php echo JText::_('HIKASHOP_WP_FLOATING_CART'); ?></td>
		<td><?php
			echo JHTML::_('hikaselect.booleanlist', 'config[wp_floating_cart]', '', $this->config->get('wp_floating_cart', 0));
			?> <a class="btn btn-small" href="<?php
				echo admin_url('admin.php?page=hikashop-widget-options&module_name=mod_hikashop_cart&options_view=cartmodules&options_field=hikashopcartmodule&numeric_id=90000&tmpl=popup');
			?>" data-hk-popup="vex" data-vex='{"x":1200,"y":800}' onclick="window.hikashop.openBox(this); return false;" title="<?php echo JText::_('HIKASHOP_OPTIONS'); ?>"><span class="dashicons dashicons-admin-generic" style="vertical-align:middle"></span></a>
		</td>
	</tr>
<?php } ?>
...
<table class="hk_config_table table" style="width:100%">
	...
	<tr>
		<td class="hk_tbl_key"<?php echo $this->docTip('invoice_exclude_free_orders');?>><?php echo JText::_('INVOICE_EXCLUDE_FREE_ORDERS'); ?></td>
		<td><?php
			echo JHTML::_('hikaselect.booleanlist', 'config[invoice_exclude_free_orders]', '', $this->config->get('invoice_exclude_free_orders', 0));
		?></td>
	</tr>
	<tr>
	...
	<tr>
		<td class="hk_tbl_key"<?php echo $this->docTip('withdrawal_period');?>><?php echo JText::_('HIKA_DAYS_AFTER_SHIPPING_FOR_WITHDRAWAL'); ?></td>
		<td>
			<input class="inputbox" type="text" name="config[withdrawal_period]" value="<?php echo $this->config->get('withdrawal_period', 14); ?>" />
		</td>
	</tr>
</table>
	</div></div>
...
?>
	...
	<tr>
		<td class="hk_tbl_key"<?php echo $this->docTip('export_fields');?>><?php echo JText::_('EXPORT_FIELDS'); ?></td>
		<td>
<?php
			echo $this->popup->display(
				'<button class="btn btn-primary" onclick="return false;">'.JText::_('PRODUCT').'</button>',
				JText::_('EXPORT_FIELDS').' - '.JText::_('PRODUCT'),
				hikashop_completeLink('config&task=export_fields&type=product&tmpl=component', true),
				'export_fields_product',
				400, 550, '', '', 'link'
			);
			echo $this->popup->display(
				'<button class="btn btn-primary" onclick="return false;">'.JText::_('HIKASHOP_ORDER').'</button>',
				JText::_('EXPORT_FIELDS').' - '.JText::_('HIKASHOP_ORDER'),
				hikashop_completeLink('config&task=export_fields&type=order&tmpl=component', true),
				'export_fields_order',
				400, 550, '', '', 'link'
			);
			echo $this->popup->display(
				'<button class="btn btn-primary" onclick="return false;">'.JText::_('SHIPPING_METHODS').'</button>',
				JText::_('EXPORT_FIELDS').' - '.JText::_('SHIPPING_METHODS'),
				hikashop_completeLink('config&task=export_fields&type=shipping&tmpl=component', true),
				'export_fields_shipping',
				400, 550, '', '', 'link'
			);
			echo $this->popup->display(
				'<button class="btn btn-primary" onclick="return false;">'.JText::_('PAYMENT_METHODS').'</button>',
				JText::_('EXPORT_FIELDS').' - '.JText::_('PAYMENT_METHODS'),
				hikashop_completeLink('config&task=export_fields&type=payment&tmpl=component', true),
				'export_fields_payment',
				400, 550, '', '', 'link'
			);
?>
		</td>
...
<table class="hk_config_table table" style="width:100%">
	...
	<tr>
		<td class="hk_tbl_key"<?php echo $this->docTip('withdrawal_notification_email');?>><?php echo JText::_('HIKA_WITHDRAWAL_NOTIFICATION_EMAIL_ADDRESS'); ?></td>
		<td>
			<input class="inputbox" type="text" placeholder="<?php echo $this->escape($this->config->get('from_email')); ?>" name="config[withdrawal_notification_email]" size="40" value="<?php echo $this->escape($this->config->get('withdrawal_notification_email')); ?>">
		</td>
	</tr>
	<tr>
...
<script>
function hikaCheckUpdate(upgradeLevel) {
	var isUpgrade = (upgradeLevel !== false);
	var spinnerEl = document.getElementById(isUpgrade ? 'hika_upgrade_spinner' : 'hika_check_update_spinner');
	var resultEl = document.getElementById(isUpgrade ? 'hika_upgrade_result' : 'hika_check_update_result');
	var btnEl = isUpgrade ? null : document.getElementById('hika_check_update_btn');

	if(isUpgrade) {
		var h = window.location.hostname;
		if(h === 'localhost' || h === '127.0.0.1' || h === '::1' || h.endsWith('.local') || h.endsWith('.test')) {
			<?php
				if(defined('HIKASHOP_WORDPRESS'))
					$localhostInstaller = JText::_('HIKA_WP_INSTALLER');
				else
					$localhostInstaller = JText::_('HIKA_JOOMLA_INSTALLER');
			?>
			if(resultEl) resultEl.innerHTML = '<div class="alert alert-warning">' + <?php echo json_encode(JText::sprintf('HIKA_LOCALHOST_UPGRADE', $localhostInstaller)); ?> + '</div>';
			return;
		}
	}
	...
	var url = '<?php echo hikashop_completeLink("config&task=check_update&tmpl=raw&" . hikashop_getFormToken() . "=1", false, true); ?>';
	if(isUpgrade) {
		url += '&upgrade_level=' + encodeURIComponent(upgradeLevel);
	}
	...
	fetch(url)
		...
		.then(function(data) {
			...
			if(data.update_available) {
				...
				html += <?php echo json_encode(JText::_('HIKA_UPDATE_AVAILABLE')); ?> + ': <strong>' + data.new_version + '</strong>';
				html += ' <button type="button" class="btn btn-primary" onclick="hikaInstallUpdate(\'' + data.package_url + '\'); return false;">';
				html += <?php echo json_encode(JText::_('HIKA_INSTALL_UPDATE')); ?>;
				html += '</button></div>';
				resultEl.innerHTML = html;
			} else if(data.domain_not_found) {
				var html = '<div class="alert alert-danger">';
				html += <?php echo json_encode(JText::_('HIKA_DOMAIN_NOT_FOUND')); ?>;
				html += '</div>';
				resultEl.innerHTML = html;
			} else {
				resultEl.innerHTML = '<div class="alert alert-success">' + <?php echo json_encode(JText::_('HIKA_UP_TO_DATE')); ?> + '</div>';
			}
		})
		.catch(function() {
			if(spinnerEl) spinnerEl.style.display = 'none';
			if(btnEl) btnEl.disabled = false;
			if(resultEl) resultEl.innerHTML = '<div class="alert alert-danger">' + <?php echo json_encode(JText::_('HIKA_ERROR_CONNECTING')); ?> + '</div>';
		});
}

function hikaInstallUpdate(packageUrl) {
	if(!confirm(<?php echo json_encode(JText::_('HIKA_CONFIRM_INSTALL')); ?>)) return;

	var resultEl = document.getElementById('hika_check_update_result');
	var upgradeEl = document.getElementById('hika_upgrade_result');
	var targetEl = (resultEl && resultEl.innerHTML) ? resultEl : ((upgradeEl && upgradeEl.innerHTML) ? upgradeEl : resultEl);
	if(targetEl) targetEl.innerHTML = '<div class="alert alert-info">' + <?php echo json_encode(JText::_('HIKA_INSTALLING')); ?> + '</div>';

	var url = '<?php echo hikashop_completeLink("config&task=install_update&tmpl=raw&" . hikashop_getFormToken() . "=1", false, true); ?>';

	fetch(url, {
		method: 'POST',
		headers: {'Content-Type': 'application/x-www-form-urlencoded'},
		body: 'package_url=' + encodeURIComponent(packageUrl) + '&<?php echo hikashop_getFormToken(); ?>=1'
	})
		.then(function(r) { return r.json(); })
		.then(function(data) {
			if(data.success) {
				targetEl.innerHTML = '<div class="alert alert-success">' + <?php echo json_encode(JText::_('HIKA_UPDATE_INSTALLED')); ?> + '</div>';
				setTimeout(function(){ window.location.reload(); }, 2000);
			} else {
				targetEl.innerHTML = '<div class="alert alert-danger">' + <?php echo json_encode(JText::_('HIKA_ERROR_INSTALLING')); ?> + ': ' + (data.error || '') + '</div>';
			}
		})
		.catch(function() {
			if(targetEl) targetEl.innerHTML = '<div class="alert alert-danger">' + <?php echo json_encode(JText::_('HIKA_ERROR_CONNECTING')); ?> + '</div>';
		});
}
</script>
