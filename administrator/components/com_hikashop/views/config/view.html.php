...
class configViewConfig extends hikashopView
{
	...
	public function display($tpl = null) {
		...
		$function = (string)$this->getLayout();
		if(method_exists($this, $function))
			$this->$function();
		parent::display($tpl);
	}

	function export_fields($tpl = null) {
		$this->type = hikaInput::get()->getCmd('type', 'product');
		$this->columns = array();

		if($this->type) {
			$classType = 'class.'.$this->type;
			$class = hikashop_get($classType);
			if(method_exists($class, 'getExportColumns')) {
				$this->columns = $class->getExportColumns(false);
			}
		}
	}

	public function address_layout($tpl = null) {
		$config =& hikashop_config();
		$this->assignRef('config', $config);

		$fieldClass = hikashop_get('class.field');
		$this->fields = $fieldClass->getData('all_unpublished', 'address');
		$this->fieldClass = $fieldClass;
		$this->allAddressFields = $fieldClass->getData('all', 'address');
		foreach($this->allAddressFields as $f) {
			if(is_string($f->field_options)) {
				$f->field_options = hikashop_unserialize($f->field_options);
			}
		}

		$sampleAddress = new stdClass();
		$sampleAddress->address_firstname = 'John';
		$sampleAddress->address_lastname = 'Doe';
		$sampleAddress->address_title = 'Mr';
		$sampleAddress->address_company = 'Acme Corp';
		$sampleAddress->address_vat = '123456789';
		$sampleAddress->address_street = '123 Main Street';
		$sampleAddress->address_street2 = 'Empire State Building';
		$sampleAddress->address_city = 'New York';
		$sampleAddress->address_post_code = '10001';
		$sampleAddress->address_country = 'country_United_States_of_America_223';
		$sampleAddress->address_state = 'state_New_York_4296';
		$sampleAddress->address_telephone = '123-456-7890';

		$configJson = $config->get('address_form_layout', '');
		$layoutConfig = json_decode($configJson, true);

		if(empty($layoutConfig)) {
			$layoutConfig = $fieldClass->getAddressLayoutDefault($this->fields);
			$configJson = json_encode($layoutConfig);
		}

		$this->assignRef('layoutConfig', $layoutConfig);
		$this->assignRef('layoutConfigJson', $configJson);

		$fieldClass->prepareFields($this->fields, $sampleAddress, 'address', 'index.php?option=com_hikashop&ctrl=checkout&task=state');
		$this->assignRef('sampleAddress', $sampleAddress);
	}
	...
	public function config($tpl = null) {
		...
		if(!empty($plugin)) {
			...
			$plugin = $pluginClass->getByName($plugin->type,$plugin->name, false);
		...
		$aclcats['withdrawal'] = array('view','manage','delete');
...
';
	...
	public function language() {
		...
		if(is_file($path)){
			...
			$file->content = file_get_contents(hikashop_getLanguageFile('en-GB'));
		}
		if($loadLatest OR hikaInput::get()->getString('task') == 'latest'){
			...
			if(defined('HIKASHOP_WORDPRESS'))
				$doc->addScript(HIKASHOP_UPDATEURL.'languageload&code='.hikaInput::get()->getString('code').'&type=wordpress');
			$showLatest = false;
		...
		if(defined('HIKASHOP_WORDPRESS')) {
			...
			if(is_file($wp_path))
				$wp_content = file_get_contents($wp_path);
		}

		$this->assignRef('wp_content', $wp_content);
