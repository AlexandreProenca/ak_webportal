	...
	protected function getMenuData($cid) {
		...
		if(empty($cid)) {
			...
			if(is_array($default_params))
				$element->hikashop_params = $default_params;
			return $element;
		}
	...
	function options(&$params) {
		...
		if(empty($this->element)) {
			...
			if(isset($this->element['modules']) && $this->type != $this->menu) {
				...
				$overrideSubDir = defined('HIKASHOP_WORDPRESS') ? 'hikashop' : 'html/com_hikashop';
			if(file_exists(JPATH_ROOT .'/templates/'.$template.'/'.$overrideSubDir.'/category/listing.php')){
					$app = JFactory::getApplication();
