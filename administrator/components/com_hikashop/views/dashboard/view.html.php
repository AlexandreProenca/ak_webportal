...
class dashboardViewDashboard extends hikashopView {
	...
	public function listing() {
		...
		if(JFactory::getUser()->authorise('core.admin', 'com_hikashop')) {
			...
			$this->toolbar[] = array('name' => 'link', 'icon' => 'th', 'alt' => 'HIKA_DASHBOARD_CONFIG', 'url' => hikashop_completeLink('dashboard&task=config'));
		}
	...
	public function widget($params) {
		$this->edit = isset($params->edit_view) ? $params->edit_view : true;
	...
	public function settings() {
		...
		$widget = hikaInput::get()->getString('widget');

		if(empty($widget)) {
			$this->assignRef('error', 'No widget specified');
			return;
		}

		$statisticsClass = hikashop_get('class.statistics');
		$dashboard = $statisticsClass->getDashboardWidgets('cpanel');

		if(isset($dashboard[$widget])) {
			$form = $statisticsClass->getForm($dashboard[$widget], $widget);
			$this->assignRef('form', $form);
			$this->assignRef('widget', $widget);
			$this->assignRef('widget_label', $dashboard[$widget]['label']);
			$this->assignRef('widget_type_label', $dashboard[$widget]['type_label']);

			$params = hikaInput::get()->getString('params');
			$paramsRegistry = new JRegistry();
			if(!empty($params)) {
				$paramsRegistry->loadString($params);
			}
			$this->assignRef('params', $paramsRegistry);
		} else {
	...
	public function config() {
		...
		$statisticsClass = hikashop_get('class.statistics');
		$this->available_widgets = $statisticsClass->getDashboardWidgets('cpanel');

		foreach($this->available_widgets as $key => &$stat) {
			if(!empty($stat['type_label'])) {
				$stat['original_label'] = $stat['label'];
				$stat['label'] = JText::_($stat['type_label']);
			}
		}
		unset($stat);

		$config = hikashop_config();
		$layoutConfig = $config->get('dashboard_layout_cpanel');
		$this->current_layout = !empty($layoutConfig) ? json_decode($layoutConfig, true) : array();

		if(empty($this->current_layout) && !empty($this->available_widgets)) {
			$rows = array();
			foreach($this->available_widgets as $key => $stat) {
				$slot = isset($stat['slot']) ? (int)$stat['slot'] : 0;
				$order = isset($stat['order']) ? (int)$stat['order'] : 0;
				if(!isset($rows[$slot])) {
					$rows[$slot] = array();
				}
				$rows[$slot][] = array(
					'key' => $key,
					'order' => $order,
					'width' => isset($stat['class']) ? $stat['class'] : 'hkc-sm-12'
				);


				if(isset($this->available_widgets[$key]) && isset($this->available_widgets[$key]['original_label'])) {
					$rows[$slot][count($rows[$slot])-1]['params'] = array('custom_title' => $this->available_widgets[$key]['original_label']);
				}
			}
			ksort($rows);
			foreach($rows as $slot => $widgets) {
				...
				$this->current_layout[] = array('widgets' => $cleanWidgets);
			}
		}

		$presets = array(
			'' => JText::_('PLEASE_SELECT_SOMETHING'),
			'default' => JText::_('HIKA_DEFAULT'),
			'maps' => JText::_('HIKA_MAPS'),
			'orders' => JText::_('ORDERS'),
		);
		$options = array();
		foreach($presets as $value => $text) {
			$options[] = JHtml::_('select.option', $value, $text);
		}
		$presetsHtml = JHtml::_('select.genericlist', $options, 'preset', 'onchange="hikashop_dashboard.loadPreset(this.value);" class="custom-select" style="width:100%"', 'value', 'text', '');
		$this->assignRef('presets', $presetsHtml);

		hikashop_setTitle(JText::_('HIKA_DASHBOARD_CONFIG'), 'cog', 'dashboard&task=config');

		$this->toolbar[] = array('name' => 'save', 'url' => 'javascript:hikashop_dashboard.save();');
		$this->toolbar[] = array('name' => 'link', 'icon' => 'import', 'alt' => 'IMPORT', 'url' => 'javascript:hikashop_dashboard.import();');
		$this->toolbar[] = array('name' => 'link', 'icon' => 'export', 'alt' => 'HIKA_EXPORT', 'url' => 'javascript:hikashop_dashboard.export();');
		$app = JFactory::getApplication();
		$cancel_url = $app->getUserStateFromRequest($this->paramBase . '.cancel_redirect', 'cancel_redirect', hikashop_completeLink('dashboard'));
		$this->toolbar[] = array('name' => 'cancel', 'url' => $cancel_url);

		hikashop_loadJsLib('dashboard_config');
	}
