...
<div id="hikabase_<?php echo $table->table; ?>_massactions" style="display:none">
	<div id="<?php echo $table->table; ?>_triggers_original">
		<?php echo JHTML::_('select.genericlist', $table->typevaluesTriggers, "trigger[".$table->table."][type][__num__]", 'class="custom-select chzn-done not-processed" size="1" onchange="updateMassAction(\'trigger\',\''.$table->table.'\',__num__); refreshSelect(\''.$table->table.'\',\'trigger\', __num__) "', 'value', 'text','trigger'.$table->table.'__num__');
				echo '<span><a style="cursor:  pointer;" onClick="selectTrigger=document.getElementById(\'trigger'.$table->table.'type__num__\') ;selectTrigger.options[0].selected=\'selected\';updateMassAction(\'trigger\',\''.$table->table.'\',__num__); document.getElementById(\''.$table->table.'trigger__num__\').style.display=\'none\'; numTriggers[\''.$table->table.'\']--;" ><i class="fa fa-times-circle" style="margin-left :3px;"></i></a></span>';?>
...
<?php
				$count = 0;
				if(!empty($this->loadedData->massaction_triggers) && $table->table == $this->loadedData->massaction_table){
					$count = count($this->loadedData->massaction_triggers);
					foreach($this->loadedData->massaction_triggers as $k => $trigger){
						if(!empty($trigger->name) && $this->loadedData->massaction_table == $table->table){
							...
							echo JHTML::_('select.genericlist', $table->typevaluesTriggers, 'trigger['.$table->table.'][type]['.$k.']', 'class="custom-select chzn-done not-processed" size="1" onchange="updateMassAction(\'trigger\',\''.$table->table.'\','.$k.');"', 'value', 'text',$trigger->name);
							echo '<span><a style="cursor:pointer;" onClick="selectTrigger=document.getElementById(\'trigger'.$table->table.'type'.$k.'\') ;selectTrigger.options[0].selected=\'selected\'; updateMassAction(\'trigger\',\''.$table->table.'\','.$k.'); document.getElementById(\''.$table->table.'trigger'.$k.'\').style.display=\'none\';numTriggers[\''.$table->table.'\']--;" ><i class="fa fa-times-circle" style="margin-left :3px;"></i></a></span>';
							echo ($trigger->html ?? '').'</div>';
						}
					}
				}else{
					$this->doc->addScriptDeclaration( "window.hikashop.ready( function() { addHikaMassAction('".$table->table."','trigger'); });" );
				}
