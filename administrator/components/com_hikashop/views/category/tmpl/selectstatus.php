...
<form action="index.php?option=<?php echo HIKASHOP_COMPONENT ?>" method="post"  name="adminForm" id="adminForm">
	...
	<table class="adminlist table table-striped table-hover" cellpadding="1">
		...
		<tbody>
			...
			?>
				<tr class="<?php echo empty($row->selected) ? "row$k" : 'selectedrow'; ?>" id="content<?php echo htmlspecialchars($row->category_name, ENT_QUOTES, 'UTF-8'); ?>" onclick="applyContent('<?php echo htmlspecialchars($row->category_name, ENT_QUOTES, 'UTF-8')."','row$k'"?>);" style="cursor:pointer;">
					<td>
