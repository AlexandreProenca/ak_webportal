...
class VoteViewVote extends hikashopView{
	...
	function listing() {
		...
		if(!empty($rows)){
			...
			$userIds = array();
			foreach($rows as $v){
				if($v->vote_type == 'vendor')
					$vendorIds[(int)$v->vote_ref_id] = (int)$v->vote_ref_id;
				else
					$productIds[(int)$v->vote_ref_id] = (int)$v->vote_ref_id;
				if($v->vote_pseudo == '0' && !empty($v->vote_user_id))
					$userIds[(int)$v->vote_user_id] = (int)$v->vote_user_id;
			}

			$productNames = array();
			if(!empty($productIds)){
				$database->setQuery('SELECT product_id, product_name FROM '.hikashop_table('product').' WHERE product_id IN ('.implode(',', $productIds).')');
				$productNames = $database->loadObjectList('product_id');
			}
			$vendorNames = array();
			if(!empty($vendorIds) && defined('HIKAMARKET_COMPONENT')){
				$database->setQuery('SELECT vendor_id, vendor_name FROM `#__hikamarket_vendor` WHERE vendor_id IN ('.implode(',', $vendorIds).')');
				$vendorNames = $database->loadObjectList('vendor_id');
			}
			$usersData = array();
			if(!empty($userIds)){
			...
			foreach($rows as $k => $v){
				...
				$refId = (int)$v->vote_ref_id;
				if($v->vote_type == 'vendor' && isset($vendorNames[$refId]))
					$rows[$k]->item_name = $vendorNames[$refId]->vendor_name;
				elseif(isset($productNames[$refId]))
					$rows[$k]->item_name = $productNames[$refId]->product_name;
				else
					$rows[$k]->item_name = '';

				if($v->vote_pseudo == '0' && isset($usersData[(int)$v->vote_user_id])){
					$rows[$k]->username = $usersData[(int)$v->vote_user_id]->username;
					$rows[$k]->email = $usersData[(int)$v->vote_user_id]->email;
				}
			}
		}
