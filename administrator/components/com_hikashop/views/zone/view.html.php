...
class ZoneViewZone extends hikashopView
{
	...
	function listing(){
		...
		$order->reverse = false;
		if($pageInfo->filter->order->value == 'a.zone_ordering') {
	...
	function importcsv() {
		hikashop_setTitle(JText::_('IMPORT_SUBZONES'), $this->icon, $this->ctrl);
		$this->main_id = hikaInput::get()->getInt('main_id');
		$this->csv_content = hikaInput::get()->get('csv_content', '', 'raw');
	}
