...
class hikashopViewClass extends hikashopClass {
	...
	public function get($id, $default = null) {
		...
		if(defined('HIKASHOP_WORDPRESS')) {
			$themeDir = get_theme_root() . DS . $obj->template;
			$obj->override = $themeDir . DS . 'hikashop' . DS;
			if($obj->client_id == 1) {
				$obj->override .= 'administrator' . DS;
			}
					...
					if($file->content[$i+1] == '!' && $file->content[$i+2] == '-' && $file->content[$i+3] == '-' && $file->content[$i+4] == ' ' ) {
						if(!$in_block) {
							...
							if($end_name == 'EO '.$current_block->name) {
								...
								$current_block->code = '';
								$current_block->type = 'normal';
								$i = $i + 5 + $length_name + 4 - 1; // length of <!-- + space + EO + space + name + space + -->
								$in_block = false;
								break;
							}
						}
