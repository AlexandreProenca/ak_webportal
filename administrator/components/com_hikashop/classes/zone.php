...
class hikashopZoneClass extends hikashopClass{
	...
	public function delete(&$elements) {
		...
		$list = is_array($elements) ? $elements : array($elements);
		$namekeys = array();
		if(!empty($list))
			$namekeys = is_numeric(reset($list)) ? $this->getZones($list, 'zone_namekey', 'zone_id', true) : $list;

		$status = parent::delete($elements);
		if($status) {
			$this->deleteLinks($namekeys);
			$app->triggerEvent('onAfterZoneDelete', array(&$elements));
		}
		return $status;
	}

	protected function deleteLinks($namekeys) {
		$quoted = array();
		foreach($namekeys as $namekey) {
			if($namekey !== null && $namekey !== '')
				$quoted[] = $this->database->Quote($namekey);
		}
		if(empty($quoted))
			return;
		$in = implode(',', $quoted);
		$this->database->setQuery('DELETE FROM '.hikashop_table('zone_link').' WHERE zone_parent_namekey IN ('.$in.') OR zone_child_namekey IN ('.$in.')');
	...
	function getChildren($zone_namekey) {
		...
		$query = 'SELECT a.*, b.zone_child_namekey FROM '.hikashop_table('zone_link').' AS b LEFT JOIN '.hikashop_table('zone').' AS a ON b.zone_child_namekey=a.zone_namekey WHERE b.zone_parent_namekey  = '.$this->db->Quote($zone_namekey).' ORDER BY a.zone_ordering ASC, a.zone_name_english ASC';
	...
	public function &getNameboxData(&$typeConfig, &$fullLoad, $mode, $value, $search, $options) {
		...
		if(!empty($search)) {
			...
			$query = 'SELECT z.*, zp.zone_namekey as zone_parent_namekey, zp.zone_id as zone_parent_id, zp.zone_name_english as zone_parent_name_english, zp.zone_type as zone_parent_type '.
				...
				' ORDER BY zp.zone_ordering ASC, zp.zone_name_english ASC, z.zone_ordering ASC, z.zone_name_english ASC';
		...
		if(empty($options['zone_key'])) {
			...
			$query = 'SELECT zone_id, zone_namekey, zone_name, zone_name_english, zone_code_2, zone_code_3, zone_type '.
					...
					' ORDER BY zone_ordering ASC, zone_name_english ASC';
		} else {
			...
			if($type == 'namekey') {
				$query = 'SELECT z.* '.
					...
					' ORDER BY z.zone_ordering ASC, z.zone_name_english ASC';
			} else {
				$query = 'SELECT z.* '.
					' FROM '.hikashop_table('zone').' as z '.
					' INNER JOIN ' . hikashop_table('zone_link') . ' as zl ON z.zone_namekey = zl.zone_child_namekey '.
	...
	public function importFromCSV($parentNamekey, $csvData, $mapping, $options = array()) {
		if(empty($parentNamekey) || empty($csvData) || empty($mapping)) return false;

		$parent = $this->get($parentNamekey);
		if(!$parent) return false;

		$childType = 'state';
		if($parent->zone_type == 'country') $childType = 'state';
		elseif($parent->zone_type == 'state') $childType = 'city';
		elseif($parent->zone_type == 'tax') $childType = 'tax';
		...
		foreach($csvData as $row) {
			...
			if(!$hasData || empty($element->zone_name)) {
				$skipped++;
				continue;
			}
			...
			$wheres = array();
			$quotedName = $this->database->Quote($element->zone_name);
			$wheres[] = 'zone_name = '.$quotedName;
			$wheres[] = 'zone_name_english = '.$quotedName;

			if(!empty($element->zone_code_2)) {
				$wheres[] = 'zone_code_2 = '.$this->database->Quote($element->zone_code_2);
			}
			if(!empty($element->zone_code_3)) {
				$wheres[] = 'zone_code_3 = '.$this->database->Quote($element->zone_code_3);
			}

			$query = 'SELECT zone_id, zone_namekey FROM '.hikashop_table('zone').' WHERE ('.implode(' OR ', $wheres).') AND zone_type = '.$this->database->Quote($childType);
			$this->database->setQuery($query);
			$existing = $this->database->loadObject();

			if(!$existing && $childType == 'state') {
				$query = 'SELECT zone_id, zone_namekey FROM '.hikashop_table('zone').' WHERE ('.implode(' OR ', $wheres).') AND zone_type = "country"';
				$this->database->setQuery($query);
				$existing = $this->database->loadObject();
			}
			if($existing) {
				$zone_id = $existing->zone_id;
				$zone_namekey = $existing->zone_namekey;
				$skipped++;
			} else {
				if($this->save($element)) {
					$zone_id = $element->zone_id;
					$zone_namekey = $element->zone_namekey;
					$imported++;
				} else {
					$errors[] = JText::sprintf('ERROR_IMPORTING_ZONE_X', $element->zone_name);
					continue;
				}
			}

			$this->addChildren($parent->zone_namekey, array($zone_namekey));
		}
	...
	public function getImportSupportedColumns() {
		return array(
			'zone_name',
			'zone_name_english',
			'zone_code_2',
