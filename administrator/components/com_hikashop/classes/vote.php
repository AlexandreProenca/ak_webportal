...
class hikashopVoteClass extends hikashopClass {
	...
	function checkVote(&$element){
		...
		$user = hikashop_loadUser(true);
		$tokenAuthorisedForBuyed = false;
		if(
			!hikashop_isClient('administrator')
			&& $element->vote_type == 'product'
			&& $this->config->get('access_vote','public') == 'buyed'
			&& empty($user->user_cms_id)
			&& !empty($element->order_token)
		) {
			$token_user_id = $this->hasBoughtViaToken(
				$element->vote_ref_id,
				$element->order_token,
				!empty($element->order_id) ? (int)$element->order_id : 0
			);
			if(!empty($token_user_id)) {
				$tokenAuthorisedForBuyed = true;
				$element->vote_user_id = (int)$token_user_id;
				if(empty($element->vote_pseudo) || empty($element->vote_email)) {
					$orderClass = hikashop_get('class.order');
					$orderData = $orderClass->loadFullOrder(!empty($element->order_id) ? (int)$element->order_id : 0, true, false);
					if(!empty($orderData) && !empty($orderData->billing_address)) {
						if(empty($element->vote_pseudo)) {
							$pseudo = trim(@$orderData->billing_address->address_firstname . ' ' . @$orderData->billing_address->address_lastname);
							if($pseudo !== '')
								$element->vote_pseudo = $pseudo;
						}
						if(empty($element->vote_email) && !empty($orderData->customer->user_email)) {
							$element->vote_email = $orderData->customer->user_email;
						}
					}
				}
			}
		...
		if(hikashop_isClient('administrator')){
			...
			if($this->config->get('access_vote','public') != 'public' && empty($user->user_cms_id) && !$tokenAuthorisedForBuyed){
			...
			if($element->vote_type == 'product' && $this->config->get('access_vote','public') == 'buyed'){
				...
				} elseif($tokenAuthorisedForBuyed) {
					$hasBought = true;
				}
				if(!$hasBought){
	...
	function save(&$element){
		...
		unset($element->order_token, $element->order_id);
		...
		if($element->vote_rating == 0) {
			if($element->vote_type == 'product') {
				$product = $itemClass->get($element->vote_ref_id);
				if(!empty($product)) {
					$this->values = array(
						'average' => isset($product->product_average_score) ? $product->product_average_score : 0,
						'total' => isset($product->product_total_vote) ? $product->product_total_vote : 0
					);
				}
	...
	function updateAverage(&$element, $oldElement, &$data) {
		...
		$query = 'SELECT * FROM '.hikashop_table('vote').' WHERE vote_ref_id = '.(int)$ref_id.' AND vote_type=\'product\' AND vote_rating != 0 AND vote_published >=1';
		$db = JFactory::getDBO();
		$db->setQuery($query);
	...
	function saveForm(){
		...
		foreach($formData['vote'] as $column => $value){
			...
			if($column!='vote_comment' && $column!='vote_answer'){
...
';
	...
	function hasBought($vote_ref_id, $user_id){
		...
		if(!empty($order_ids)) {
			...
			$where_product = 'product_id = '.(int)$vote_ref_id;
			if(!empty($product_ids)) {
				foreach($product_ids as $k => $v)
					$product_ids[$k] = (int)$v;
				$where_product = '(' . $where_product . ' OR product_id IN ('.implode(',', $product_ids).'))';
			}
			$query = 'SELECT order_product_id FROM '.hikashop_table('order_product').' WHERE order_id IN ('.implode(',',$order_ids).') AND '.$where_product;
			$db->setQuery($query);
	...
	function hasBoughtViaToken($vote_ref_id, $order_token, $order_id = 0) {
		if(empty($order_token) || empty($vote_ref_id)) {
			return null;
		}
		$db = JFactory::getDBO();
		$config = hikashop_config();
		...
		if(!empty($order_id)) {
			$query = 'SELECT order_id, order_token, order_status, order_user_id FROM '.hikashop_table('order')
				. ' WHERE order_id = '.(int)$order_id.' AND order_type = '.$db->quote('sale');
		} else {
			$query = 'SELECT order_id, order_token, order_status, order_user_id FROM '.hikashop_table('order')
				. ' WHERE order_token = '.$db->quote((string)$order_token).' AND order_type = '.$db->quote('sale')
				. ' LIMIT 1';
		}
		$db->setQuery($query);
		$order = $db->loadObject();
		...
		if(!hash_equals((string)$order->order_token, (string)$order_token)) {
			return null;
		}
		...
		$query = 'SELECT product_id FROM '.hikashop_table('product').' WHERE product_parent_id = '.(int)$vote_ref_id;
		$db->setQuery($query);
		$variant_ids = $db->loadColumn();
		$where_product = 'product_id = '.(int)$vote_ref_id;
		if(!empty($variant_ids)) {
			foreach($variant_ids as $k => $v)
				$variant_ids[$k] = (int)$v;
			$where_product = '(' . $where_product . ' OR product_id IN ('.implode(',', $variant_ids).'))';
		}

		$query = 'SELECT order_product_id FROM '.hikashop_table('order_product')
		...
		return (int)$order->order_user_id;
	}
