...
class hikashopPaymentClass extends hikashopClass {
	...
	public function getExportColumns($filter = false) {
		...
		if($filter) {
			...
			$selected_fields = $config->get('payment_export_columns');
	...
	function save(&$element, $reorder = true) {
		...
		if($status){
			...
			if($translationHelper->isMulti()) {
				...
				$this->_addTranslatableParamsForCheck($element, 'payment', $columns);
	...
	public function fillListingColumns(&$rows, &$listing_columns, &$view, $type = null) {
		...
		foreach($rows as &$row) {
			...
			if(isset($row->payment_price) && bccomp(sprintf('%F',$row->payment_price), 0, 3)) {
