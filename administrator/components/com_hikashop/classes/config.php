...
class hikashopConfigClass extends hikashopClass{
	...
	function save(&$configObject,$default=false){
		...
		foreach($configObject as $namekey => $value){
			...
			if(($namekey == 'payment_log_file' || $namekey == 'cron_savepath') && (!preg_match('#\.log$#i', $value) || strpos($value, '..') !== false)) {
			...
			if($namekey == 'checkout_workflow' || strpos($namekey, 'dashboard_layout_') === 0)
	...
	public function getAddressFormat($originalOnly=false){
		if(!$originalOnly && !defined('HIKASHOP_WORDPRESS')){
			$db = JFactory::getDBO();
