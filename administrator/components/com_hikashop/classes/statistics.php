...
class hikashopStatisticsClass extends hikashopClass {
	...
	public function getDashboard($place = null) {
		return $this->_getDashboard($place, false);
	}

	public function getDashboardWidgets($place = null) {
		return $this->_getDashboard($place, true);
	}

	protected function _getDashboard($place = null, $all = false) {
		...
		$ret['sales_avg'] = array(
			...
			'type_label' => 'HIKA_AVG_ORDER_VALUE',
		...
		$ret['conversion_rate'] = array(
			...
			'type_label' => 'HIKA_CONVERSION_RATE_LABEL',
		...
		$ret['geo_sales'] = array(
			...
			'type_label' => 'HIKA_GEOLOCATION_MAP',
			...
			'query' => array(
				'get' => 'list',
				'select' => array(
					(isset($this->vars['METRIC']) && $this->vars['METRIC'] == 'amount' ? 'SUM(hk_order.order_full_price) AS value' : 'COUNT(hk_order.order_id) AS value'),
		...
		$ret['last_orders'] = array(
			...
			'type_label' => 'HIKA_ORDERS_LISTING',
			'vars' => array(
				'COUNTER' => 6
			),
			'prefix' => '<table style="width:100%" class="table table-striped table-hover table-bordered"><thead><tr><th>{ORDER_NUMBER}</th><th>{ORDER_STATUS}</th><th>{CUSTOMER}</th><th>{DATE}</th><th>{PRICE}</th></thead><tbody>',
			...
			$layoutConfig = $this->config->get('dashboard_layout_' . $place);
			if(!empty($layoutConfig)) {
				$layout = json_decode($layoutConfig, true);
				if(is_array($layout) && !$all) {
					$newRet = array();
					$usedKeys = array();
					...
					foreach($layout as $rowIndex => $row) {
						...
						$orderIndex = 0;
						foreach($row['widgets'] as $widgetConfig) {
							...
							$stat['slot'] = $rowIndex; // Use row index as slot for ordering/grouping
							$stat['order'] = $orderIndex++; // Set order based on layout position
							if(isset($widgetConfig['width'])) {
								$stat['class'] = $widgetConfig['width']; 
							}
							if(isset($widgetConfig['params']) && is_array($widgetConfig['params'])) {
								if(!isset($stat['vars'])) $stat['vars'] = array();
								$stat['vars'] = array_merge($stat['vars'], $widgetConfig['params']);
								if(!empty($widgetConfig['params']['custom_title'])) {
									$stat['label'] = $widgetConfig['params']['custom_title'];
								}
							}

							if(isset($stat['vars']['order_status']) && is_array($stat['vars']['order_status'])) {
								$statuses = array();
								foreach($stat['vars']['order_status'] as $s) {
									$s = trim($s);
									if($s !== '') $statuses[] = $this->db->Quote($s);
								}
								if(!empty($statuses)) {
									$stat['vars']['ORDER_STATUSES'] = 'IN (' . implode(',', $statuses) . ')';
								}
							}

							if(isset($stat['vars']['METRIC']) && $stat['vars']['METRIC'] == 'count') {
								if(isset($stat['query']['select']['value'])) {
									$stat['query']['select']['value'] = 'COUNT(hk_order.order_id) as value';
								}
							}

							$newRet[$key . '_' . $rowIndex . '_' . $orderIndex] = $stat;
						}
					}

					foreach($ret as $key => $stat) {
	...
	public function getAjaxData($name, $value) {
		...
		if(hikashop_isClient('administrator'))
			$statistics = $this->getDashboard('cpanel');
...
');
	...
	public function display($data, $doText = true) {
		...
		if((!empty($data['type_label']) && $data['type_label'] == 'HIKA_ORDERS_LISTING') || isset($data['vars']['COUNTER'])) {
			if(!empty($data['vars']['limit'])) {
				$data['query']['limit'] = (int)$data['vars']['limit'];
			}

			if(!empty($data['vars']['order_status'])) {
				$statuses = $data['vars']['order_status'];
				if(!is_array($statuses)) $statuses = explode(',', $statuses);

				$quotedStatuses = array();
				foreach($statuses as $s) {
					if(trim($s) !== '')
						$quotedStatuses[] = $this->db->Quote($s);
				}

				if(!empty($quotedStatuses)) {
					$data['query']['filters']['order_status'] = 'hk_order.order_status IN ('. implode(',', $quotedStatuses) .')';
				}
				unset($data['vars']['order_status']);
			}

			if(!empty($data['vars']['prefix'])) $data['prefix'] = $data['vars']['prefix'];
			if(!empty($data['vars']['dynamic'])) $data['dynamic'] = $data['vars']['dynamic'];
			if(!empty($data['vars']['suffix'])) $data['suffix'] = $data['vars']['suffix'];

			foreach(array('prefix', 'dynamic', 'suffix') as $key) {
				if(empty($data[$key])) continue;
				if(preg_match_all('#{([A-Z0-9_]+)}#', $data[$key], $matches)) {
					foreach($matches[1] as $k) {
						$data[$key] = str_replace('{'.$k.'}', JText::_($k), $data[$key]);
					}
				}
			}

			if(isset($data['dynamic']) && strpos($data['dynamic'], 'address_') !== false && isset($data['query']['tables'])) {
				$data['query']['tables'][] = 'LEFT JOIN '.hikashop_table('address').' AS hk_address ON hk_order.order_billing_address_id = hk_address.address_id';
				$data['query']['select'] .= ', hk_address.*';
			}

			if(isset($data['vars']['sort_col']) && isset($data['vars']['sort_dir'])) {
				$sortCol = $data['vars']['sort_col'];
				$sortDir = $data['vars']['sort_dir'];
				if(in_array($sortDir, array('ASC', 'DESC')) && preg_match('#^[a-z0-9_]+$#', $sortCol)) {
					$data['query']['order'] = 'hk_order.'.$sortCol.' '.$sortDir;
				}
			}
		...
		switch($data['type']) {
			...
			case 'list':
				...
				break;
			case 'report':
				if(!empty($data['vars']['report_id'])) {
					$widgetClass = hikashop_get('class.widget');
					$widget = $widgetClass->get($data['vars']['report_id']);
					if($widget) {
						$widget->edit_view = false;
						if(!empty($data['class'])) {
							$widget->dashboard_class = $data['class'];
						}
						$js = '';
						$title = isset($data['label']) ? JText::_($data['label']) : '';
						if(isset($data['vars']['custom_title']) && $data['vars']['custom_title'] !== '') {
							$title = $data['vars']['custom_title'];
						} elseif(!empty($widget->widget_name)) {
							$title = JText::_($widget->widget_name);
						}

						$ret = '<div class="hikashop_panel_heading">'.$title.'</div>';
						$ret .= '<div class="hikashop_panel_body">'.hikashop_getLayout('dashboard', 'widget', $widget, $js).'</div>';
					}
				}
				break;
			case 'raw':
...
<div class="cpanel-tile-block'.$extra_class.'">
	...
	protected function displayGraph($data) {
		...
		$currentGroup = isset($data['vars']['DATE_GROUP']) ? $data['vars']['DATE_GROUP'] : 'day';
		if(strpos($currentRange, ':') !== false)
			list($currentRange, $currentGroup) = explode(':', $currentRange, 2);

		$chartType = 'LineChart';
		if(isset($data['vars']['GRAPH_TYPE'])) {
			if($data['vars']['GRAPH_TYPE'] == 'area') $chartType = 'AreaChart';
			else if($data['vars']['GRAPH_TYPE'] == 'column') $chartType = 'ColumnChart';
		}

		$legendPos = 'bottom';
		if(isset($data['vars']['LEGEND_POS'])) {
			$legendPos = $data['vars']['LEGEND_POS'];
		}

		$curveStyle = '';
		if(isset($data['vars']['CURVE_STYLE']) && $data['vars']['CURVE_STYLE'] == 'function') {
			$curveStyle = ',curveType:"function"';
		}
...
window.localPage.chartsInit[window.localPage.chartsInit.length] = function() {
	...
	var options = {legend:{position:"'.$legendPos.'"},focusTarget:"category",animation:{duration:500,easing:"in"}'.$theme.$viewport.$curveStyle.'};
	var chart = new google.visualization.'.$chartType.'(document.getElementById("hikashop_chart_'.$id.'_div"));
...
<div id="hikashop_chart_'.$id.'_div" style="width:100%; height: 250px;"><div class="hikashop_empty_chart">'.JText::_('HIKA_LOADING_CHART').'</div></div>';
	...
	public function getForm($stat, $key = null) {
		$form = array();

		if(isset($stat['vars']['period'])) {
			$form['period'] = array(
				'type' => 'radio',
				'label' => 'HIKA_PERIOD',
				'values' => array(
		...
		if($key == 'last_orders' || (isset($stat['type_label']) && $stat['type_label'] == 'HIKA_ORDERS_LISTING')) {
			...
			$form['prefix'] = array(
				...
				'default' => isset($stat['prefix']) ? $stat['prefix'] : ''
			);
			$form['dynamic'] = array(
				'type' => 'textarea',
				'label' => 'DYNAMIC_DISPLAY',
				'default' => isset($stat['dynamic']) ? $stat['dynamic'] : ''
			);
			$form['suffix'] = array(
				'type' => 'textarea',
				'label' => 'SHIPPING_SUFFIX',
				'default' => isset($stat['suffix']) ? $stat['suffix'] : ''
			);
			...
			$db = JFactory::getDBO();
			$columns = $db->getTableColumns(hikashop_table('order'));
			$colOptions = array();
		...
		} elseif ($key == 'orders_history' || (isset($stat['type_label']) && $stat['type_label'] == 'HIKA_SALES_GRAPH')) {
			if(!isset($valid_order_statuses)) {
				$config = hikashop_config();
				$valid_order_statuses = explode(',',$config->get('order_statuses','confirmed,shipped'));
			}
			$form['order_status'] = array(
				'type' => 'order_status',
				'label' => 'ORDER_STATUS',
				'multiple' => true,
				'default' => $valid_order_statuses
			);

			$ranges = $this->getDateRangeList();
			$rangeOptions = array();
			foreach($ranges as $k => $v) {
				if(strpos($k, ':') !== false) list($k, $trash) = explode(':', $k, 2);
				$rangeOptions[$k] = $v;
			}

			$form['DATE_RANGE'] = array(
				'type' => 'list',
				'label' => 'HIKA_DEFAULT_PERIOD',
				'values' => $rangeOptions,
				'default' => 'this.year'
			);
			...
			$form['DATE_GROUP'] = array(
				'type' => 'list',
				'label' => 'HIKA_DEFAULT_GROUP',
				'values' => array(
					'day' => 'HIKA_PERIOD_BY_DAY',
					'week' => 'HIKA_PERIOD_BY_WEEK',
					'month' => 'HIKA_PERIOD_BY_MONTH',
					'year' => 'HIKA_PERIOD_BY_YEAR'
				),
				'default' => 'month'
			);
			...
			$form['GRAPH_TYPE'] = array(
				'type' => 'list',
				'label' => 'HIKA_GRAPH_TYPE',
				'values' => array(
					'line' => 'HIKA_GRAPH_LINE',
					'area' => 'HIKA_GRAPH_AREA',
					'column' => 'HIKA_GRAPH_COLUMN',
				),
				'default' => 'line'
			);
			$form['METRIC'] = array(
				...
				'label' => 'HIKA_METRIC',
				'values' => array(
					'amount' => 'HIKA_METRIC_AMOUNT',
					'count' => 'HIKA_METRIC_COUNT'
				),
				'default' => 'amount'
			);
			$form['CURVE_STYLE'] = array(
				'type' => 'list',
				'label' => 'HIKA_CURVE_STYLE',
				'values' => array(
					'none' => 'HIKA_CURVE_LINEAR',
					'function' => 'HIKA_CURVE_SMOOTH'
				),
				'default' => 'none'
			);
			$form['LEGEND_POS'] = array(
				'type' => 'list',
				'label' => 'HIKA_LEGEND_POS',
				'values' => array(
					'none' => 'HIKA_POS_NONE',
					'in' => 'HIKA_INSIDE',
					'top' => 'HIKA_POS_TOP',
					'bottom' => 'HIKA_POS_BOTTOM',
					'right' => 'HIKA_POS_RIGHT'
				),
				'default' => 'none'
		);

	} elseif($key == 'geo_sales' || (isset($stat['type_label']) && $stat['type_label'] == 'HIKA_GEOLOCATION_MAP')) {
		$form['METRIC'] = array(
			'type' => 'list',
			'label' => 'HIKA_METRIC',
			'values' => array(
		...
		$form['REGION'] = array(
			...
			'default' => 'world',
			'description' => 'HIKA_GEO_REGION_DESC'
		);
		$form['COLOR_LOW'] = array(
			'type' => 'text',
			'label' => 'HIKA_COLOR_LOW',
			'default' => '#E5F5E0'
		);
		$form['COLOR_HIGH'] = array(
			'type' => 'text',
			'label' => 'HIKA_COLOR_HIGH',
			'default' => '#31A354'
		);

	} elseif($key == 'report') {
			$db = JFactory::getDBO();
			$db->setQuery('SELECT widget_id, widget_name FROM #__hikashop_widget ORDER BY widget_name');
			$rows = $db->loadObjectList();
			$reports = array();
			foreach($rows as $row) {
				$reports[$row->widget_id] = $row->widget_name;
			}
	...
	public function saveDashboard($place, $layout) {
		if(empty($place) || !preg_match('#^[_a-zA-Z0-9]+$#', $place)) {
			return false;
		}

		$config = hikashop_config();
		$config->set('dashboard_layout_' . $place, json_encode($layout));
		return $config->save();
	}
}
