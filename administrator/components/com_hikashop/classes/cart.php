...
class hikashopCartClass extends hikashopClass {
	...
	public function get($element, $default = null, $options = array()) {
		...
		if(isset(self::$cache['get'][$element]->cart_type) && self::$cache['get'][$element]->cart_type == 'ucp')
			$options['skip_user_check'] = true;

		if(!hikashop_isClient('administrator') && empty($options['skip_user_check'])) {
			if(isset(self::$cache['get'][$element]->cart_type) && self::$cache['get'][$element]->cart_type == 'wishlist' && !$this->config->get('enable_wishlist')) {
			...
			$share = (isset(self::$cache['get'][$element]->cart_type) && self::$cache['get'][$element]->cart_type == 'wishlist') ? self::$cache['get'][$element]->cart_share : '';
	...
	public function save(&$element) {
		...
		if(!hikashop_isClient('administrator') && !empty($element->cart_id)) {
			...
			if(isset($element->old->cart_type) && $element->old->cart_type == 'wishlist' && !empty($element->old->user_id) && (empty($this->user) || $element->old->user_id != $this->user->user_id))
				return false;
		...
		if(isset($element->cart_type) && !in_array($element->cart_type, array('cart', 'wishlist', 'auction', 'ucp')))
		...
		$cache = JCache::getInstance('page', array('defaultgroup' => 'com_hikashop'));
		$cache->clean();
	...
	public function &getFullCart($cart_id = 0, $options = array()) {
		if(!empty($options['ucp'])) {
			$options['skip_user_check'] = true;
			if(empty($options['auto_select_addresses']))
				$options['auto_select_addresses'] = false;
		}
		...
		if($cart->cart_type != 'wishlist') {
			...
			if(empty($address) && !empty($cart->cart_billing_address_id))
			...
			if(empty($address) && !empty($cart->cart_shipping_address_ids))
		...
		if(!isset($cart->cart_type) || $cart->cart_type != 'wishlist') {
		...
		foreach($cart->products as &$product) {
			...
			if(!empty($product->cart_product_option_parent_id) && !empty($product->cart_product_quantity) && $this->config->get('group_options', 0)) {
				if(isset($product->prices[0]->unit_price->price_value))
					$product->prices[0]->unit_price->price_value /= $product->cart_product_quantity;
				if(isset($product->prices[0]->unit_price->price_value_with_tax))
					$product->prices[0]->unit_price->price_value_with_tax /= $product->cart_product_quantity;
				if(isset($product->prices[0]->unit_price->price_value_without_discount))
					$product->prices[0]->unit_price->price_value_without_discount /= $product->cart_product_quantity;
				if(isset($product->prices[0]->unit_price->price_value_without_discount_with_tax))
					$product->prices[0]->unit_price->price_value_without_discount_with_tax /= $product->cart_product_quantity;
			}
		...
		if(!empty($cart->cart_coupon) && $cart->cart_type != 'wishlist') {
			...
			if(is_array($cart->cart_coupon)) {
				foreach($cart->cart_coupon as $coupon) {
					$cart->coupon = $discountClass->loadAndCheck($coupon, $cart->full_total, $parent_zones, $cart->products, true, $cart);
					if(!empty($cart->coupon))
						break;
				}
			} else {
				$cart->coupon = $discountClass->loadAndCheck($cart->cart_coupon, $cart->full_total, $parent_zones, $cart->products, true, $cart);
			}
	...
	public function addProduct($cart_id, &$products, $options = array()) {
		...
		if(!empty($cart->cart_type) && !in_array($cart->cart_type, array('cart', 'ucp')) && empty($this->user))
			return false;
	...
	public function needsShipping($cart = null) {
		if((int)$this->config->get('force_shipping', 0))
			return true;
		...
		$package = $this->getWeightVolume($cart);
		return !empty($package['weight']['value']) && (float)$package['weight']['value'] > 0;
	}
	...
	protected function checkQuantities(&$products, &$cart, $options = array()) {
		...
		$query = 'SELECT p.*, pp.product_published as parent_product_published, pp.product_quantity as parent_product_quantity, '.
				...
				' (SELECT MAX(child.product_published) FROM ' . hikashop_table('product') . ' AS child WHERE child.product_parent_id = p.product_id) as child_max_published '.
		...
		foreach($db_products as $pid => $p) {
			if($p->product_type == 'main' && isset($p->child_max_published) && !$p->child_max_published)
				$db_products[$pid]->product_quantity = 0;
		}
		...
		if(!empty($limits['min']) || !empty($limits['max'])) {
			...
			foreach($cart->cart_products as $cart_product) {
				...
				if(!empty($options['product_being_removed']) && (int)$i == (int)$options['product_being_removed']) {
					continue;
				}
