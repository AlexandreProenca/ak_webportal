...
class hikashopAddressClass extends hikashopClass {
	...
	const SHIPPING_PREVIEW_ID = -987654321;
	...
	function get($element, $default = null) {
		...
		$app = JFactory::getApplication();
		if((int)$element < 0) {
			if((int)$element == self::SHIPPING_PREVIEW_ID) {
				$preview = $app->getUserState(HIKASHOP_COMPONENT.'.shipping_address_preview', null);
				if(!empty($preview))
					$cachedElements[$element] = hikashop_copy($preview);
			} else {
	...
	public function save(&$addressData, $order_id = 0, $type = 'shipping') {
		...
		$skip_user_check = !empty($addressData->skip_user_check);
		unset($addressData->skip_user_check);
		...
		if(!empty($addressData->address_id) && $addressData->address_id > 0) {
			...
			if(!$skip_user_check && !hikashop_isClient('administrator') && (!in_array($oldData->address_user_id, $allowed) || !$oldData->address_published)) {
				$do = false;
			}
	...
	public function getCurrentUserAddress($type = '', $user = null) {
		...
		if(empty($user) || empty($user_id))
			$user_id = (int)hikashop_loadUser(false);
