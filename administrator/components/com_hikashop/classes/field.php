...
class hikashopFieldClass extends hikashopClass {
	...
	function &getData($area, $type, $notcoreonly = false, $categories = null) {
		...
		} elseif($area == 'all_unpublished') {
		} elseif($area != 'all') {
			$clauses = explode(';', trim((string)$area,';'));
...
<?php
	...
	function prepareFields(&$fields, &$data, $type = 'user', $url = 'checkout&task=state', $test = false) {
		...
		foreach($fields as $namekey => $field) {
			...
			if(!is_array($fields[$namekey]->field_options)) {
	...
	function getAddressLayoutDefault($fields) {
		$assignedKeys = array();
		$defaultRows = array(
			array('fields' => array(
				array('namekey' => 'address_title', 'width' => 1),
				array('namekey' => 'address_firstname', 'width' => 1),
				array('namekey' => 'address_lastname', 'width' => 2)
			)),
			array('fields' => array(
				array('namekey' => 'address_company', 'width' => 2),
				array('namekey' => 'address_vat', 'width' => 2)
			)),
			array('fields' => array(
				array('namekey' => 'address_street', 'width' => 4)
			)),
			array('fields' => array(
				array('namekey' => 'address_street2', 'width' => 4)
			)),
			array('fields' => array(
				array('namekey' => 'address_post_code', 'width' => 1),
				array('namekey' => 'address_city', 'width' => 3)
			)),
			array('fields' => array(
				array('namekey' => 'address_country', 'width' => 2),
				array('namekey' => 'address_state', 'width' => 2)
			)),
			array('fields' => array(
				array('namekey' => 'address_telephone', 'width' => 4),
			))
		);

		foreach($defaultRows as $row) {
			foreach($row['fields'] as $f) {
				$assignedKeys[$f['namekey']] = true;
			}
		}

		foreach($fields as $field) {
			if(!isset($assignedKeys[$field->field_namekey]) && $field->field_type != 'hidden') {
				$defaultRows[] = array('fields' => array(
					array('namekey' => $field->field_namekey, 'width' => 4)
				));
			}
		}
	...
	function handleZoneListing(&$fields,&$rows){
		...
		if(!empty($values)){
			$query = 'SELECT * FROM '.hikashop_table('zone').' WHERE zone_namekey IN ('.implode(',',$values).') ORDER BY zone_ordering ASC, zone_name_english ASC';
			$this->database->setQuery($query);
...
if(!window.hikashopFieldsJs["'.$type.$suffix_type.'"]) window.hikashopFieldsJs["'.$type.$suffix_type.'"] = {};';
	...
	function allowed($column, $type = 'user') {
		$restricted = array(
			'user'=>array('user_partner_price'=>1,'user_partner_paid'=>1,'user_created_ip'=>1,'user_partner_id'=>1,'user_partner_lead_fee'=>1,'user_partner_click_fee'=>1,'user_partner_percent_fee'=>1,'user_partner_flat_fee'=>1,'affiliation_category_restriction'=>1),
	...
	function get($field_id,$default=null){
		...
		if(!empty($field) && !is_array($field->field_options)) {
...
class hikashopFieldText extends hikashopFieldItem {
	...
	function display($field, $value, $map, $inside, $options = '', $test = false, $allFields = null, $allValues = null) {
		...
		if(empty($js) && hikashop_isClient('site')){
			$js = ' onblur="if(document.formvalidator) document.formvalidator.validate(this); if(window.hikashop.updateFieldError) window.hikashop.updateFieldError(this);"';
		}
		...
		$errorSpan = '';
		$inputId = $this->prefix.@$field->field_namekey.$this->suffix;
		$errorSpanId = $inputId . '_error';
		$size .= ' aria-describedby="' . $errorSpanId . '"';
		$errorSpan = '<span id="' . $errorSpanId . '" class="hikashop_field_error_msg" style="display:none;" role="alert"></span>';
		...
		return $html.'<input id="'.$inputId.'"'.$size.$js.' '.$options.' type="'.$this->type.'" name="'.$map.'" value="'.$value.'" />'.$errorSpan;
	}
...
class hikashopFieldLink extends hikashopFieldText {
	...
	function display($field, $value, $map, $inside, $options = '', $test = false, $allFields = null, $allValues = null) {
		...
		if(strlen(trim((string)$value)) < 1 && !empty($field->field_default))
			$value = $field->field_default;
		$field->field_default = '';
		$link = $value;
		$text = $value;
...
class hikashopFieldImage extends hikashopFieldFile {
	function show(&$field, $value, $class='hikashop_custom_image_link', $token='') {
		if(in_array($class,array('admin_email', 'user_email')))
			return parent::show($field, $value, $class, $token);
		...
		if(!empty($token))
			$extra .= '&order_token='.$token;
		return '<img class="'.$class.' hikashop_custom_field_preview_image" src="'.hikashop_completeLink('order&task=download&field_table='.$field->field_table.'&field_namekey='.urlencode(base64_encode($field->field_namekey)).'&name='.urlencode(base64_encode($value))).$extra.'" alt="'.htmlspecialchars($value, ENT_COMPAT, 'UTF-8').'" />';
	}
...
class hikashopFieldAjaxfile extends hikashopFieldItem {
	...
	function display($field, $value, $map, $inside, $options = '', $test = false, $allFields = null, $allValues = null){
		...
		if(!empty(hikashopView::$currentFormId))
			$vars['form_id'] = hikashopView::$currentFormId;
		$options = array(
			'upload' => true,
			'tooltip' => true,
			'gallery' => false,
		...
		if(!empty($field->field_options['max_filesize'])) {
			$options['maxFileSize'] = $field->field_options['max_filesize'];
		}
	...
	function _showOne(&$field, $value, $class = 'hikashop_custom_file_link', $token = '') {
		...
		$name = $value;
		if(strpos($name,'<span class="searchtext">') !== false) {
			$name = str_replace(array('<span class="searchtext">','</span>'),'',$name);
		}

		$download_link = 'order&task=download&field_table='.$field->field_table.'&field_namekey='.urlencode(base64_encode($field->field_namekey)).'&name='.urlencode(base64_encode($name));
		if(!empty($token)) {
...
class hikashopFieldTextarea extends hikashopFieldItem {
	...
	function display($field, $value, $map, $inside, $options = '', $test = false, $allFields = null, $allValues = null){
		...
		$options .= ' data-validation-text="'.str_replace('"', '&quot;', $message).'"';
		$errorSpanId = $inputId . '_error';
		$options .= ' aria-describedby="' . $errorSpanId . '"';
		$errorSpan = '<span id="' . $errorSpanId . '" class="hikashop_field_error_msg" style="display:none;" role="alert"></span>';
		...
		return '<textarea id="'.$inputId.'" name="'.$map.'" '.$cols.' '.$rows.' '.$js.' '.$options.'>'.$value.'</textarea>'.$html.$errorSpan;
	}
...
class hikashopFieldDropdown extends hikashopFieldItem {
	...
	function show(&$field,$value){
		...
		if($value !== null && isset($field->field_value[$value])) $value = $field->field_value[$value]->value;
	...
	function display($field, $value, $map, $inside, $options = '', $test = false, $allFields = null, $allValues = null){
		...
		$string .= '<select id="'.$inputId.'" name="'.$map.'" '.$arg.$options.'>';
		if(!empty($field->field_value)) {
		...
		$string .= '<span id="' . $errorSpanId . '" class="hikashop_field_error_msg" style="display:none;" role="alert" data-validation-text="'.str_replace('"', '&quot;', $message).'"></span>';
...
class hikashopFieldZone extends hikashopFieldSingledropdown {
	function display($field, $value, $map, $inside, $options = '', $test = false, $allFields = null, $allValues = null){
		...
		if($field->field_options['zone_type'] == 'country'){
			...
			if(strpos($options, 'data-prevent-change="true"') === false && empty($field->field_options['data-prevent-change'])) {
				if(!empty($options) && stripos($options,'onchange="')!==false){
					$options = preg_replace('#onchange="#i','onchange="'.$changeJs,$options);
				}else{
					$options = ' onchange="'.$changeJs.'"'; // It's important to keep the space at the beginning
				}
			}
...
class hikashopFieldMultipledropdown extends hikashopFieldDropdown{
	...
	function show(&$field,$value){
		...
		foreach($value as $val){
			if($val !== null && isset($field->field_value[$val])) $val = $field->field_value[$val]->value;
