...
class hikashopWidgetClass extends hikashopClass {
	...
	function get($cid=0,$default=''){
		if(!empty($cid)){
			...
			if(!empty($widget->widget_params)){
				...
				$this->_sanitizeWidgetParams($widget->widget_params);
			}
			return $widget;
		}
		$filters=array();
		...
		if(!empty($widgets)){
			foreach($widgets as $k => $widget){
				if(!empty($widget->widget_params)){
					...
					$this->_sanitizeWidgetParams($widgets[$k]->widget_params);
				}
			}
		}
		return $widgets;
	...
	protected function _sanitizeWidgetParams(&$params) {
		if(!is_object($params)) return;

		$stringProps = array('content', 'display', 'date_type', 'date_group', 'periodType', 'proposedPeriod', 
			'compare_with', 'period_compare', 'region', 'REGION', 'format', 'map_source', 'product_data', 
			'product_order_by', 'orders_order_by', 'orders_total_calculation', 'orders_with_tax', 'customers', 'customers_order',
			'partners', 'partners_order', 'content_view', 'COLOR_LOW', 'COLOR_HIGH');

		foreach($stringProps as $prop) {
			if(isset($params->$prop) && !is_string($params->$prop) && !is_null($params->$prop)) {
				$params->$prop = '';
			}
		}
	}
...
<?php
		...
		switch($widget->widget_params->display){
			...
			case 'listing':
				...
				if($widget->widget_params->content=='products' || $widget->widget_params->content=='categories' || $widget->widget_params->content=='discounts'){
					...
					if($widget->widget_params->content=='categories'){
						$ids=$this->_getBestCategories($filters, $widget->widget_params->product_data, $limit, $reverse, $widget);
					...
					if($widget->widget_params->content=='products'){
						$ids=$this->_getBestProducts($filters, $widget->widget_params->product_data, $limit, $reverse, false, $leftjoin, $widget);
						$widget->elements=$ids;
						if($widget->widget_params->product_data=='sales'){
				...
				if($widget->widget_params->content=='customers' || $widget->widget_params->content=='partners' || $widget->widget_params->content=='orders' || $widget->widget_params->content=='sales'){
					...
					if(in_array($widget->widget_params->content, array('orders','sales'))) {
						$listingSum = 'a.order_full_price';
						$excludeShipping = !empty($widget->widget_params->orders_total_calculation) && $widget->widget_params->orders_total_calculation == 'exclude_fees';
						$excludeCoupon = !empty($widget->widget_params->orders_include_coupon) && $widget->widget_params->orders_include_coupon == 'exclude';
						$excludeTax = !empty($widget->widget_params->orders_with_tax) && $widget->widget_params->orders_with_tax == 'without_tax';

						if($excludeShipping)
							$listingSum .= '-a.order_shipping_price';
						if($excludeCoupon)
							$listingSum .= '+a.order_discount_price';
						if($excludeTax)
							$listingSum .= '-(SELECT SUM(subprod.order_product_tax*subprod.order_product_quantity) FROM '.hikashop_table('order_product').' AS subprod WHERE a.order_id=subprod.order_id)-a.order_shipping_tax-a.order_payment_tax';

						if($excludeShipping || $excludeCoupon || $excludeTax)
							$select .= ', ('.$listingSum.') AS listing_total';
					}
				}
				if(in_array($widget->widget_params->content,array('orders','sales'))) {
			...
			case 'map':
				if($widget->widget_params->content=='orders' || $widget->widget_params->content=='sales' || $widget->widget_params->content=='taxes'){
					...
					$select.='z.zone_code_2, z.zone_name_english, '.$sum;
	...
	function _getProductPriceExpression($widget = null, $alias = 'prod') {
		$with_tax = false;
		$include_shipping = false;
		$include_coupon = true;

		if(!empty($widget->widget_params)) {
			$p = $widget->widget_params;
			if(isset($p->with_tax)) $with_tax = !empty($p->with_tax);
			if(isset($p->include_shipping)) $include_shipping = !empty($p->include_shipping);
			if(isset($p->include_coupon)) $include_coupon = !empty($p->include_coupon);
			if(isset($p->orders_with_tax)) $with_tax = ($p->orders_with_tax !== 'without_tax');
			if(isset($p->orders_total_calculation)) $include_shipping = ($p->orders_total_calculation !== 'exclude_fees');
			if(isset($p->orders_include_coupon)) $include_coupon = ($p->orders_include_coupon !== 'exclude');
		}

		if(!$include_coupon) {
			$price = 'CASE WHEN '.$alias.'.order_product_price_before_discount > 0 THEN '.$alias.'.order_product_price_before_discount ELSE '.$alias.'.order_product_price END';
		} else {
			$price = $alias.'.order_product_price';
		}
		...
		if($with_tax) {
			if(!$include_coupon) {
				$expr .= '+CASE WHEN '.$alias.'.order_product_price_before_discount > 0 THEN '.$alias.'.order_product_tax_before_discount ELSE '.$alias.'.order_product_tax END';
			} else {
				$expr .= '+'.$alias.'.order_product_tax';
			}
		}

		if($include_shipping) {
			$expr .= '+'.$alias.'.order_product_shipping_price';
			if($with_tax)
				$expr .= '+'.$alias.'.order_product_shipping_tax';
		}

		return '(' . $expr . ')*' . $alias . '.order_product_quantity';
	}

	function _getBestCategories($filters, $content='', $limit='', $reverse=false, $widget=null){
		$db = JFactory::getDBO();
		$order='DESC';
		if($reverse){ $order= 'ASC'; }
		if($content=='sales'){
			$dataType=$this->_getProductPriceExpression($widget);
		...
		$currencyGroup = ($content == 'sales') ? ', a.order_currency_id' : '';
		$currencySelect = ($content == 'sales') ? ', a.order_currency_id' : '';
		$query='SELECT *, SUM('.$dataType.') as Total'.$currencySelect.' FROM '.hikashop_table('order').' as a LEFT JOIN '.hikashop_table('order_product ').' as prod ON a.order_id=prod.order_id LEFT JOIN '.hikashop_table('product').' AS p ON p.product_id=prod.product_id ' .
				...
				$filters.'	GROUP BY c.category_id'.$currencyGroup.' ORDER BY Total '.$order;
		$db->setQuery($query);
		$rows = $db->loadObjectList();

		if($content == 'sales') {
			$rows = $this->_convertCategoryCurrencies($rows, $order, $limit);
		} else {
			$keyed = array();
			foreach($rows as $row) {
				$keyed[$row->category_id] = $row;
			}
			$rows = $keyed;
		}
	...
	function _getBestProducts($filters, $content, $limit='', $reverse=false, $statusForce=false, $leftjoin='', $widget=null){
		$db = JFactory::getDBO();
		$order='DESC';
		$dataType='';
		if($reverse){ $order= 'ASC'; }
		if($content=='sales'){
			$dataType='SUM('.$this->_getProductPriceExpression($widget).')';
		}else if($content=='clicks'){
			$dataType='p.product_hit';
		...
		if($content=='orders'){
			...
			if(!empty($ids)){
				...
				$priceExpr = $this->_getProductPriceExpression($widget);
				$db->setQuery('SELECT prod.product_id, '.$priceExpr.' AS total, a.order_currency_id FROM #__hikashop_order_product AS prod LEFT JOIN #__hikashop_order AS a on a.order_id=prod.order_id '
				.$leftjoin.' '.$filters.' AND prod.product_id IN ('.implode(',',$product_ids).')');
				$products = $db->loadObjectList();

				$currencyClass = hikashop_get('class.currency');
				$display_currency = hikashop_getCurrency();
				$config =& hikashop_config();
				$main_currency = (int)$config->get('main_currency', 1);
				$currencyIds = array($main_currency => $main_currency, $display_currency => $display_currency);
				foreach($products as $p) {
					if(!empty($p->order_currency_id))
						$currencyIds[(int)$p->order_currency_id] = (int)$p->order_currency_id;
				}
				$null = null;
				$currencyClass->getCurrencies($currencyIds, $null);

				foreach($ids as $k => $id) {
					foreach($products as $p) {
						if($p->product_id == $id->product_id) {
							$converted = (float)$p->total;
							if(!empty($p->order_currency_id) && (int)$p->order_currency_id !== $display_currency) {
								$converted = $currencyClass->convertUniquePrice($converted, $p->order_currency_id, $display_currency);
							}
							$ids[$k]->Total += $converted;
						}
					}
					$ids[$k]->order_currency_id = $display_currency;
				}
			}
		}else{
			$query='SELECT *, '.$dataType.' as Total, count(a.order_id) AS quantity, a.order_currency_id FROM '.hikashop_table('order_product').' AS prod LEFT JOIN '.hikashop_table('order').' AS a on a.order_id=prod.order_id '
				.$leftjoin.' '.$filters.' GROUP BY prod.product_id, prod.order_product_name, a.order_currency_id ORDER BY Total '.$order;
			$db->setQuery($query);
			$ids = $db->loadObjectList();

			if($content == 'sales')
				$ids = $this->_convertProductCurrencies($ids, $order, $limit);
		}
		return $ids;
	}

	function _convertCategoryCurrencies($rows, $order = 'DESC', $limit = '') {
		if(empty($rows))
			return array();
		...
		$currencyIds = array($main_currency => $main_currency, $display_currency => $display_currency);
		foreach($rows as $row) {
			if(!empty($row->order_currency_id))
				$currencyIds[(int)$row->order_currency_id] = (int)$row->order_currency_id;
		}
		$null = null;
		...
		foreach($rows as $row) {
			...
			$converted = (float)$row->Total;

			if(!empty($row->order_currency_id) && (int)$row->order_currency_id !== $display_currency) {
				$converted = $currencyClass->convertUniquePrice($converted, $row->order_currency_id, $display_currency);
			}

			if(!isset($merged[$key])) {
				$row->Total = $converted;
				$row->order_currency_id = $display_currency;
				$merged[$key] = $row;
			} else {
				$merged[$key]->Total += $converted;
			}
		}

		uasort($merged, function($a, $b) use ($order) {
			if($order == 'DESC')
				return $b->Total <=> $a->Total;
			return $a->Total <=> $b->Total;
		});

		if(!empty($limit) && preg_match('/LIMIT\s+(\d+)/i', $limit, $m)) {
			$merged = array_slice($merged, 0, (int)$m[1], true);
		}

		return $merged;
	}

	function _convertProductCurrencies($rows, $order = 'DESC', $limit = '') {
		if(empty($rows))
			return $rows;
		...
		$merged = array();
		foreach($rows as $row) {
			$key = (int)$row->product_id . '_' . $row->order_product_name;
			$converted = (float)$row->Total;
		...
		if(!empty($limit) && preg_match('/LIMIT\s+(\d+)/i', $limit, $m)) {
			$merged = array_slice($merged, 0, (int)$m[1]);
		}
