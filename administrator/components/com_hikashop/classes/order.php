...
class hikashopOrderClass extends hikashopClass {
	...
	var $skipLockedCheck = false;
	var $skipEmailSending = false;
	...
	public function getExportColumns($filter = false) {
		$db = JFactory::getDBO();
		$tables = array(
			'order', 'user', 'address', 'order_product'
		);
		...
		foreach($tables as $table) {
			...
			$keys = array_keys($columnsArray);

			if($table == 'address') {
				foreach($keys as $key) {
					$columns['shipping_'.$key] = 'shipping_'.$key;
					$columns['billing_'.$key] = 'billing_'.$key;
				}
		...
		$columns['order_full_tax'] = 'order_full_tax';


		$query = 'SELECT tax_namekey FROM '.hikashop_table('tax');
		$db->setQuery($query);
		$taxes = $db->loadColumn();
		if(!empty($taxes)) {
			foreach($taxes as $tax) {
				$key = 'order_tax_amount_'.str_replace(array(' ', '(', ')', '%'), '', $tax);
				$columns[$key] = $key;
			}
		}
		...
		foreach($userFields as $field) {
			if($field->field_type == "customtext")
				continue;
			$columns[$field->field_namekey] = $field->field_namekey;
		}

		if($filter) {
			$config = hikashop_config();
			$selected_fields = $config->get('order_export_columns');
	...
	public function save(&$order) {
		...
		if(!empty($order->order_status) && !$this->skipLockedCheck) {
			...
			if(in_array($order->order_status, $locked_statuses)) {
				if($order->order_status != $order->old->order_status) {
					if(!in_array($order->order_status, $locked_statuses)) {
						...
						$app->enqueueMessage(JText::_('STATUS_CHANGE_NOT_ALLOWED'), 'error');
						return false;
					}
				}
				if(!empty($order->product)) unset($order->product);
				...
				foreach($blacklist as $field) {
					if(isset($order->$field) && $order->$field != $order->old->$field) {
						$app = JFactory::getApplication();
						$app->enqueueMessage(JText::sprintf('FIELD_CHANGE_NOT_ALLOWED', $field), 'error');
						return false;
					}
				}
			}
		...
		if(!empty($order->order_status)) {
			if($order->order_status == 'shipped' && (empty($order->old->order_status) || $order->old->order_status != 'shipped')) {
				if(empty($order->order_shipped)) $order->order_shipped = time();
			}
			if($order->order_status == 'delivered' && (empty($order->old->order_status) || $order->old->order_status != 'delivered')) {
				if(empty($order->order_delivered)) $order->order_delivered = time();
			}
		}
		...
		if(!$new && (isset($order->order_shipping_price) || isset($order->order_payment_price) || isset($order->order_discount_price))) {
			if(isset($order->order_shipping_tax_namekey) || isset($order->order_discount_tax_namekey) || isset($order->order_payment_tax_namekey)) {
				...
				if(isset($order->order_payment_tax_namekey) && is_string($order->order_payment_tax_namekey)) {
				...
				} else if(isset($order->order_payment_tax_namekey) && is_array($order->order_payment_tax_namekey)) {
					foreach($order->order_payment_tax_namekey as $namekey => $value) {
						if(!isset($order->order_tax_info[$namekey])) {
							$order->order_tax_info[$namekey] = new stdClass();
							$order->order_tax_info[$namekey]->tax_namekey = $namekey;
						}
						$order->order_tax_info[$namekey]->tax_amount_for_payment = $value;
					}
				}
				if(isset($order->order_discount_tax_namekey) && is_string($order->order_discount_tax_namekey)) {
				...
				} else if(isset($order->order_discount_tax_namekey) && is_array($order->order_discount_tax_namekey)) {
					foreach($order->order_discount_tax_namekey as $namekey => $value) {
						if(!isset($order->order_tax_info[$namekey])) {
							$order->order_tax_info[$namekey] = new stdClass();
							$order->order_tax_info[$namekey]->tax_namekey = $namekey;
						}
						$order->order_tax_info[$namekey]->tax_amount_for_coupon = $value;
					}
				}
			}
			$recalculate = true;
		...
		if($new && $order->order_type == 'sale' && empty($order->order_token)) {
			...
			$order->order_token = JUserHelper::genRandomPassword(32);
		}
		...
		if(!empty($order->order_status) && empty($order->order_invoice_id) && empty($order->old->order_invoice_id) && $order_type == 'sale' && !$already_invoice_number) {
			...
			if(in_array($order->order_status, $valid_statuses) && ($total > 0 || !$excludeFreeOrders)) {
				...
				$app->triggerEvent('onAfterInvoiceCreate', array(&$order));
			}
		}
		...
		if(!empty($order->cart->products)) {
				...
				if(!empty($order->order_payment_params->recurring) && !empty($order->order_payment_params->recurring['products'])) {
					...
					if(!empty($recurring_order_products)) {
						...
						$query = 'UPDATE '.hikashop_table('order').
							' SET order_payment_params = '.$this->database->Quote((string)serialize($order->order_payment_params)).
			...
			if(!empty($order->order_discount_code) && $order_type == 'sale') {
				$query = 'UPDATE '.hikashop_table('discount').
					...
					' WHERE discount_code='.$this->database->Quote((string)$order->order_discount_code).' AND discount_type=\'coupon\' LIMIT 1';
				$this->database->setQuery($query);
				$this->database->execute();
			}
			...
			if(!in_array($order->old->order_status, $cancelled_statuses) && in_array($order->order_status, $cancelled_statuses)) {
				...
				$query = 'UPDATE '.hikashop_table('discount').' SET discount_used_times = discount_used_times - 1 WHERE discount_used_times > 0 AND discount_code='.$this->database->Quote((string)$code).' AND discount_type=\'coupon\' LIMIT 1';
				$this->database->setQuery($query);
				$this->database->execute();
			}
		...
		if($new) {
			...
			if(!$send_email || $this->skipEmailSending)
				return $order->order_id;
	...
	public function saveForm($task = '') {
		...
		if( (empty($task) || $task == $currentTask) && !empty($data[$currentTask]) ) {
			...
			if(!empty($billing_address) && !empty($order_id)){
				...
				if(!empty($oldAddress->address_user_id))
					$billing_address->address_user_id = $oldAddress->address_user_id;
				elseif(!empty($oldOrder->order_user_id))
					$billing_address->address_user_id = (int)$oldOrder->order_user_id;
		...
		if( (empty($task) || $task == $currentTask) && !empty($data[$currentTask]) ) {
			...
			if(!empty($shipping_address) && !empty($order_id)){
				...
				elseif(!empty($oldOrder->order_user_id))
					$shipping_address->address_user_id = (int)$oldOrder->order_user_id;
		...
		if( (empty($task) || $task == $currentTask) && !empty($data[$currentTask]) && !isset($forbidden[$currentTask]) ) {
			...
			if(isset($data['order']['order_discount_tax_namekeys']) && is_array($data['order']['order_discount_tax_namekeys'])) {
				$order->order_discount_tax_namekey = $this->_buildTaxNamekeyMap($data['order']['order_discount_tax_namekeys'], (float)@$order->order_discount_tax);
				$do = true;
			} elseif(isset($data['order']['order_discount_tax_namekey'])) {
			...
			if(isset($data['order']['order_shipping_tax_namekeys']) && is_array($data['order']['order_shipping_tax_namekeys'])) {
				$order->order_shipping_tax_namekey = $this->_buildTaxNamekeyMap($data['order']['order_shipping_tax_namekeys'], (float)@$order->order_shipping_tax);
				$do = true;
			} elseif(isset($data['order']['order_shipping_tax_namekey'])) {
			...
			if(isset($data['order']['order_payment_tax_namekeys']) && is_array($data['order']['order_payment_tax_namekeys'])) {
				$order->order_payment_tax_namekey = $this->_buildTaxNamekeyMap($data['order']['order_payment_tax_namekeys'], (float)@$order->order_payment_tax);
				$do = true;
			} elseif(isset($data['order']['order_payment_tax_namekey'])) {
			...
			if(isset($productData['many']) && $productData['many'] == true) {
				...
				foreach($productData as $key => $value) {
					...
					if(isset($items_fields[$key]))
						continue;
					if($key == 'tax_namekeys' && is_array($value)) {
						foreach($value as $vk => $vv)
							$value[$vk] = $safeHtmlFilter->clean($vv, 'string');
						$product->tax_namekeys = $value;
						continue;
					}
					if(is_array($value))
						$value = implode(',', $value);
	...
	private function _buildTaxNamekeyMap($namekeys, $totalTax) {
		...
		foreach($namekeys as $namekey) {
			if(empty($namekey) || $namekey == '-1')
				continue;
			$tax = $taxClass->get($namekey);
			if(!$tax)
				continue;
			$validKeys[] = $namekey;
			$totalRate += (float)$tax->tax_rate;
		}
		...
		foreach($validKeys as $namekey) {
			$tax = $taxClass->get($namekey);
			if($totalRate > 0) {
				$map[$namekey] = round($totalTax * (float)$tax->tax_rate / $totalRate, 5);
			} else {
				$map[$namekey] = 0;
			}
		}
	...
	public function recalculateFullPrice(&$order, $products = null) {
		...
		foreach($products as $i => $product) {
			...
			} elseif(!empty($product->order_product_tax) && !empty($order->order_tax_info)) {
				$tax_info = $order->order_tax_info;
				if(is_string($tax_info))
					$tax_info = hikashop_unserialize($tax_info);
				if(!empty($tax_info)) {
					$qty = ($product->order_product_code == 'order additional') ? 1 : (int)$product->order_product_quantity;
					$productTax = (float)$product->order_product_tax * $qty;
					$productBase = (float)$product->order_product_price * $qty;

					$valid = array();
					foreach($tax_info as $t) {
						if(!empty($t->tax_namekey) && $t->tax_namekey != '-1')
							$valid[] = $t;
					}

					if(count($valid) == 1) {
						$first_tax = reset($valid);
						if(!isset($taxes[$first_tax->tax_namekey])) {
							$taxes[$first_tax->tax_namekey] = 0;
							$bases[$first_tax->tax_namekey] = 0;
						}
						$taxes[$first_tax->tax_namekey] += $productTax;
						$bases[$first_tax->tax_namekey] += $productBase;
					} elseif(count($valid) > 1) {
						$sumOrderTax = 0.0;
						foreach($valid as $t) {
							$sumOrderTax += (float)@$t->tax_amount;
						}
						if($sumOrderTax > 0) {
							foreach($valid as $t) {
								$share = (float)@$t->tax_amount / $sumOrderTax;
								if(!isset($taxes[$t->tax_namekey])) {
									$taxes[$t->tax_namekey] = 0;
									$bases[$t->tax_namekey] = 0;
								}
								$taxes[$t->tax_namekey] += $productTax * $share;
								$bases[$t->tax_namekey] += $productBase * $share;
							}
						} else {
							$fallback_namekey = '';
							if(!empty($product->product_id)) {
								$query = 'SELECT t.tax_namekey FROM '.hikashop_table('product').' AS p'
									.' LEFT JOIN '.hikashop_table('product').' AS main_p ON p.product_parent_id > 0 AND main_p.product_id = p.product_parent_id'
									.' JOIN '.hikashop_table('category').' AS cat ON cat.category_id = IF(p.product_tax_id > 0, p.product_tax_id, main_p.product_tax_id)'
									.' JOIN '.hikashop_table('taxation').' AS tn ON cat.category_namekey = tn.category_namekey'
									.' JOIN '.hikashop_table('tax').' AS t ON tn.tax_namekey = t.tax_namekey'
									.' WHERE p.product_id = '.(int)$product->product_id
									.' AND tn.taxation_published = 1'
									.' ORDER BY tn.taxation_id ASC';
								$this->database->setQuery($query, 0, 1);
								$fallback_namekey = (string)$this->database->loadResult();
							}
							if(empty($fallback_namekey)) {
								$first_tax = reset($valid);
								$fallback_namekey = $first_tax->tax_namekey;
							}
							if(!empty($fallback_namekey) && $fallback_namekey != '-1') {
								if(!isset($taxes[$fallback_namekey])) {
									$taxes[$fallback_namekey] = 0;
									$bases[$fallback_namekey] = 0;
								}
								$taxes[$fallback_namekey] += $productTax;
								$bases[$fallback_namekey] += $productBase;
							}
						}
					}
				}
	...
	public function loadFullOrder($order_id, $additionalData = false, $checkUser = true) {
		...
		if(hikashop_isClient('administrator')) {
			...
			if(@$order->customer->user_type === 'guest' || empty($order->customer->user_cms_id) || (int)$order->customer->user_cms_id == 0) {
				$token = hikaInput::get()->getVar('order_token');

				if(empty($token))
					$token = $app->getUserState('com_hikashop.order_token');

				if(empty($order->order_token) || empty($token) || !hash_equals($order->order_token, $token)) {
					return null;
				}
	...
	public function loadOrderNotification(&$order, $type = 'order_status_notification', $params = null) {
		...
		if((@$order->customer->user_type === 'guest' || empty($order->customer->user_cms_id) || (int)$order->customer->user_cms_id == 0) && !empty($order->order_token)) {
	...
	public function delete(&$elements) {
		...
		$config = hikashop_config();
		$locked_statuses = $config->get('order_locked_statuses','');
		if(!empty($locked_statuses)) {
			$locked_statuses = explode(',', $locked_statuses);
			$escapedStatuses = array();
			foreach($locked_statuses as $ls) {
				$escapedStatuses[] = $this->database->Quote($ls);
			}
			$query = 'SELECT order_id FROM '.hikashop_table('order').' WHERE order_id IN ('.implode(',',$elements).') AND order_status IN ('.implode(',',$escapedStatuses).')';
			$this->database->setQuery($query);
			$locked_orders = $this->database->loadColumn();

			if(!empty($locked_orders)) {
				$elements = array_diff($elements, $locked_orders);
				$app = JFactory::getApplication();
				$app->enqueueMessage(JText::_('LOCKED_ORDERS_CANNOT_BE_DELETED'), 'notice');
	...
	public function &getNameboxData($typeConfig, &$fullLoad, $mode, $value, $search, $options) {
		...
		if(!empty($search)) {
			...
			$where['search'] = '('.implode(' LIKE '.$searchVal.' OR ', $searchMap).' LIKE '.$searchVal.' OR '.implode(' = '.$thid->db->Quote((string)$search).' OR ', $searchMap).' = '.$thid->db->Quote((string)$search).')';
		}
	...
	public function modifyOrder(&$order_id, $order_status, $payment_name = 'ucp', $history = null, $email = null, $payment_params = null) {
		if($payment_name !== null && !is_string($payment_name)) {
		...
		$order->history->history_payment_method = $payment_name;
		...
		if(!empty($id)) {
			$mailClass = hikashop_get('class.mail');
			$dbOrder = $this->get($id);
			$message = str_replace('<br/>', "\r\n", JText::sprintf('PAYMENT_NOTIFICATION_STATUS', $payment_name, $payment_status)) . ' ' .
				JText::sprintf('ORDER_STATUS_CHANGED', $mail_status) .
				"\r\n".JText::sprintf('NOTIFICATION_OF_ORDER_ON_WEBSITE', $dbOrder->order_number, HIKASHOP_LIVE);
			if(is_string($email))
				$message.= "\r\n\r\n" . $email;
			$orderMail = $this->loadNotification((int)$id, 'payment_notification', $message);
			if(empty($orderMail) || empty($orderMail->mail))
				return;
			if(empty($orderMail->mail->subject))
				$orderMail->mail->subject = JText::sprintf('PAYMENT_NOTIFICATION_FOR_ORDER', $payment_name, $payment_status, $dbOrder->order_number);

			if(!empty($orderMail->mail->dst_email) && filter_var($orderMail->mail->dst_email, FILTER_VALIDATE_EMAIL)) {
				$replyName = (string)(@$orderMail->mail->dst_name);
				$mailClass->mailer->addReplyTo($orderMail->mail->dst_email, $replyName);
			}
