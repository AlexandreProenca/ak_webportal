...
class hikashopProductClass extends hikashopClass{
	...
	public function checkAlias(&$element) {
		if(!empty($element->product_alias)) {
			$query = 'SELECT product_id FROM '.hikashop_table('product').' WHERE product_alias='.$this->database->Quote($element->product_alias).' AND product_parent_id = 0';
			$this->database->setQuery($query);
	...
	public function getExportColumns($filter = false) {
		$db = JFactory::getDBO();
		if(!HIKASHOP_J30){
		...
		$columns = array_keys($columnsArray);
		...
		$db = JFactory::getDBO();
		$query = 'SELECT characteristic_id, characteristic_value, characteristic_alias FROM '.hikashop_table('characteristic').' WHERE characteristic_parent_id = 0';
		$db->setQuery($query);
		$characteristics = $db->loadObjectList();
		if(!empty($characteristics)) {
			foreach($characteristics as $characteristic) {
				$label = $characteristic->characteristic_value;
				if(!empty($characteristic->characteristic_alias))
					$label = $characteristic->characteristic_alias;
				$columns['char_'.$characteristic->characteristic_id] = $label;
			}
		}

		if($filter) {
			$config = hikashop_config();
			$selected_fields = $config->get('product_export_columns');
			if(!empty($selected_fields)) {
				$allowed = explode(',', $selected_fields);
	...
	public function backSaveForm() {
		...
		if(!empty($product->product_alias)) {
			$query = 'SELECT product_id FROM '.hikashop_table('product').' WHERE product_alias='.$this->db->Quote($product->product_alias).' AND product_parent_id = 0';
			$this->db->setQuery($query);
		...
		if($status) {
			...
			$this->warnIfNoPublishedVariant($product);
		} else {
			hikaInput::get()->set('fail', $product);
	...
	protected function warnIfNoPublishedVariant(&$product) {
		if(empty($product->product_id))
			return;
		$parent_id = !empty($product->product_parent_id) ? (int)$product->product_parent_id : (int)$product->product_id;
		$this->db->setQuery('SELECT COUNT(*) AS total, MAX(product_published) AS has_published FROM '.hikashop_table('product').' WHERE product_parent_id = '.$parent_id);
		$info = $this->db->loadObject();
		if(!empty($info) && (int)$info->total > 0 && empty($info->has_published)) {
			$app = JFactory::getApplication();
			$app->enqueueMessage(JText::_('PRODUCT_NO_PUBLISHED_VARIANT_WARNING'), 'warning');
		}
	}
	...
	public function backSaveVariantForm() {
		...
		if(hikashop_acl('product/variant/bundle') && hikashop_level(1)) {
			$options = @$formVariant['bundle'];
			$product->bundle = array();
			if(!empty($options)) {
				$k = 0;
				foreach($options as $pid => $r) {
	...
	function getProducts($ids, $mode = 'id') {
		...
		foreach($related as $rel){
			...
			if(isset($all_products[$rel->product_id]) && isset($all_products[$rel->product_id]->{$type})){
				$all_products[$rel->product_id]->{$type}[] = $rel->product_related_id;
			}
			if(isset($all_products[$rel->product_id]) && $type == 'bundle')
	...
	public function addCharacteristic($product_id, $characteristic_id, $characteristic_value_id, $vendor_id = 0) {
		...
		$query = 'SELECT c.characteristic_id, c.characteristic_value, c.characteristic_parent_id FROM ' . hikashop_table('characteristic') . ' AS c '.
			' WHERE (c.characteristic_parent_id = '.(int)$characteristic_id;
		if(!empty($vendor_id))
			$query .= ' AND c.characteristic_vendor_id IN (0, '.(int)$vendor_id.')';
		$query .= ')';
	...
	public function getAllValuesMatches($characteristics, $variants, $mainProduct = null) {
		...
		$showOutOfStock = $config->get('show_out_of_stock', 1);
		foreach($variants as $k => $variant) {
			$isNeeded = $variant->product_published
				&& $variant->product_id != $variant->product_parent_id
				&& ($variant->product_quantity != 0 || ($showOutOfStock && ($waitlist || $wishlist)));
			if(!$isNeeded) {
				continue;
			}
			$chars = array();
	...
	public function searchProducts($options = array()) {
		...
		$filters = array(
			'b.product_published = 1'
		);

		if($options['product_type'] == 'all') {
			$filters[] = "b.product_type IN ('main', 'variant')";
		} elseif(is_array($options['product_type'])) {
			$filters[] = 'b.product_type IN (' . implode(',', array_map(array($db, 'Quote'), $options['product_type'])) . ')';
		} else {
			$filters[] = 'b.product_type = ' . $db->Quote($options['product_type']);
		}
		...
		if(!empty($options['search'])) {
			$searchFields = array('product_name', 'product_description');
			$searchMap = array('b.product_name', 'b.product_description', 'b.product_id', 'b.product_code');
			$searchVal = '\'%' . hikashop_getEscaped($options['search'], true) . '%\'';
			$translationHelper = hikashop_get('helper.translation');
			$multi = $translationHelper->isMulti(true);

			$fieldClass = hikashop_get('class.field');
			$customFields = $fieldClass->getData('frontcomp', 'product');
			if(!empty($customFields)) {
				foreach($customFields as $field) {
					if(!empty($field->field_namekey)) {
						$searchMap[] = 'b.' . $field->field_namekey;
						$searchFields[] = $field->field_namekey;
					}
				}
			}

			$searchMap = array_unique($searchMap);
			$searchFields = array_unique($searchFields);

			$searchFilter = '(' . implode(" LIKE $searchVal OR ", $searchMap) . " LIKE $searchVal)";

			if($multi) {
				$translatedProductIds = $translationHelper->searchTranslation(
					$options['search'],
					'product',
					$searchFields,
					!empty($options['search_all_languages'])
				);

				if(!empty($translatedProductIds)) {
					hikashop_toInteger($translatedProductIds);
					$searchFilter = '(' . $searchFilter . ' OR b.product_id IN (' . implode(',', $translatedProductIds) . '))';
				}
			}
		...
		if(!is_array($categoryIds)) {
			$categoryIds = !empty($categoryIds) ? array((int)$categoryIds) : array();
		}
		hikashop_toInteger($categoryIds);

		if(!empty($categoryIds) && $options['include_children']) {
			$categoryClass = hikashop_get('class.category');
			$categoryFilters = array('a.category_published=1');
			hikashop_addACLFilters($categoryFilters, 'category_access', 'a');
			$children = $categoryClass->getChildren($categoryIds, true, $categoryFilters, '', 0, 0);
			if(!empty($children)) {
				foreach($children as $child) {
					$categoryIds[] = (int)$child->category_id;
				}
			}
		...
		if(!empty($categoryIds)) {
			...
			$joinOn = 'a.product_id = b.product_id';

			if($options['match_parent_category'])
				$joinOn .= ' OR a.product_id = b.product_parent_id';

			$from = hikashop_table('product_category') . ' AS a LEFT JOIN ' .
					hikashop_table('product') . ' AS b ON (' . $joinOn . ')';
			$filters[] = 'a.category_id IN (' . implode(',', $categoryIds) . ')';
			$orderTable = ($options['order_by'] == 'ordering') ? 'a' : 'b';
		} else {
			$select = 'SELECT DISTINCT b.*';
			$from = hikashop_table('product') . ' AS b';
			$orderTable = 'b';
		}

		hikashop_addACLFilters($filters, 'product_access', 'b');

		$allowedOrderFields = array('ordering', 'product_name', 'product_id', 'product_code',
									'product_created', 'product_modified', 'product_hit', 'product_quantity',
									'product_sales', 'product_sort_price');
		$orderBy = in_array($options['order_by'], $allowedOrderFields) ? $options['order_by'] : 'ordering';
		if($orderBy === 'ordering' && empty($categoryIds)) {
			$orderBy = 'product_id';
		}
		$orderDir = strtoupper($options['order_dir']) === 'DESC' ? 'DESC' : 'ASC';
		$order = ' ORDER BY ' . $orderTable . '.' . $orderBy . ' ' . $orderDir . ', b.product_id ASC';

		$where = ' WHERE (' . implode(') AND (', $filters) . ')';

		$countQuery = 'SELECT COUNT(DISTINCT b.product_id) FROM ' . $from . $where;
		...
		$idsQuery = 'SELECT DISTINCT b.product_id FROM ' . $from . $where . $order;
		$db->setQuery($idsQuery, $offset, $limit);
		...
		$db->setQuery('SELECT b.* FROM ' . hikashop_table('product') . ' AS b ' .
					  'WHERE b.product_id IN (' . implode(',', $productIds) . ') ORDER BY FIELD(b.product_id, ' . implode(',', $productIds) . ')');
		$rows = $db->loadObjectList();

		$enrichOptions = array(
			'currency_id' => $options['currency_id'],
			'zone_id' => $options['zone_id'],
			'load_badges' => $options['load_badges'],
			'load_custom_item_fields' => false,
			'bundle_qty' => true
		);
		$this->loadProductsListingData($rows, $enrichOptions);

		if(!empty($options['init_variants'])) {
			$variantIds = array();
			$parentIds = array();
			foreach($rows as $row) {
				if($row->product_type == 'variant' && $row->product_parent_id > 0) {
					$variantIds[] = $row->product_id;
					$parentIds[] = $row->product_parent_id;
				}
			}
			...
			if(!empty($variantIds)) {
				...
				if(!empty($parentIds)) {
					$db->setQuery('SELECT * FROM ' . hikashop_table('product') . ' WHERE product_id IN (' . implode(',', $parentIds) . ')');
					$parents = $db->loadObjectList('product_id');

					$parentList = array_values($parents);
					$this->loadProductsListingData($parentList, $enrichOptions);
					$parents = array();
					foreach($parentList as $p) {
						$parents[$p->product_id] = $p;
					}
				}
				...
				$query = 'SELECT hk_variant.*, hk_characteristic.* '.
					...
					' WHERE hk_variant.variant_product_id IN (' . implode(',', $variantIds) . ') '.
					' ORDER BY hk_variant.ordering, hk_characteristic.characteristic_value';
				$db->setQuery($query);
				$characteristics = $db->loadObjectList();

				foreach($rows as &$row) {
					if($row->product_type != 'variant' || empty($row->product_parent_id) || !isset($parents[$row->product_parent_id]))
						continue;

					$parent = $parents[$row->product_parent_id];

					$row->characteristics = array();
					if(!empty($characteristics)) {
						foreach($characteristics as $characteristic) {
							if($row->product_id == $characteristic->variant_product_id) {
								$row->characteristics[] = $characteristic;
							}
						}
					}

					$this->checkVariant($row, $parent);
				}
				unset($row);
			}
		}
	...
	public function searchProductsAndVariants($options = array()) {
		$options['product_type'] = isset($options['product_type']) ? $options['product_type'] : 'all';
		$options['match_parent_category'] = true;
		$options['init_variants'] = true;
	...
	function save(&$element, $stats = false, $noAfterTrigger = false) {
		...
		$new = false;
		if(empty($element->product_id)) {
			if(!isset($element->product_quantity) || $element->product_quantity === '' || $element->product_quantity === null) {
	...
	function checkSortPriceOnView(&$element) {
		if(empty($element->all_prices) || empty($element->product_id) || !empty($element->product_type) && $element->product_type != 'main')
			return;

		$tmp = new stdClass();
		$tmp->prices = $element->all_prices;
		$tmp->product_sort_price = 0;
		$tmp->product_type = 'main';
		$this->selectSortPrice($tmp);

		if($tmp->product_sort_price != hikashop_toFloat($element->product_sort_price)) {
			$update = new stdClass();
			$update->product_id = (int)$element->product_id;
			$update->product_sort_price = $tmp->product_sort_price;
			parent::save($update);
			$element->product_sort_price = $tmp->product_sort_price;
		}
	}
	...
	function updateRelated($element, $status, $type = 'related') {
		if($element->product_type == 'variant' && $type != 'bundle')
			return true;
	...
	public function &getNameboxData(&$typeConfig, &$fullLoad, $mode, $value, $search, $options) {
		...
		if($tree_mode) {
			if(empty($search)) {
				$query = 'SELECT c.*, 0 as `base_depth`' .
					...
					' ORDER BY c.category_depth ASC, c.category_ordering ASC, c.category_name ASC';
		...
		if(!empty($lookup_categories) && empty($search)) {
			$query = 'SELECT p.*, c.category_id FROM ' . hikashop_table('product') . ' AS p '.
				...
				' ORDER BY c.category_depth ASC, c.category_ordering ASC, c.category_name ASC, p.product_name ASC';
	...
	function loadFullProduct($product_id, $options = array()) {
		$product_id = (int)$product_id;
		if(empty($product_id))
			return null;
		...
		$selected_variant_id = isset($options['selected_variant_id']) ? (int)$options['selected_variant_id'] : 0;
		...
		$this->addAlias($element);
		if(!$element->product_published || $element->product_type == 'trash')
			return null;
		...
		$this->loadFullProductData($element, array_merge($options, array(
	...
	function loadFullProductData(&$element, $options = array()) {
		$product_id = isset($options['product_id']) ? (int)$options['product_id'] : (int)$element->product_id;
		$selected_variant_id = isset($options['selected_variant_id']) ? (int)$options['selected_variant_id'] : 0;
		$config = hikashop_config();

		$this->loadPageProductOptions($element, $product_id);
		$this->loadPageProductBundles($element, $product_id);

		$ids = array($product_id);
		if(!empty($element->options))
			$ids = array_merge($ids, array_keys($element->options));

		$this->loadPageProductVariants($element, $ids, $selected_variant_id);
		$this->loadPageProductCharacteristics($element, $ids, $product_id, $selected_variant_id);
		$this->loadPageProductFiles($element, $ids);
		$this->loadPageProductPrices($element, $ids, $options);
		$this->loadPageProductCustomFields($element, $options);
		$this->loadPageProductCategories($element);
		...
		if(empty($element->variants) && !empty($element->has_unpublished_variant)) {
			$element->no_available_variant = true;
			$element->product_quantity = 0;
		}
	...
	function loadPageProductOptions(&$element, $product_id) {
		$database = JFactory::getDBO();
		$filters = array(
	...
	function loadPageProductBundles(&$element, $product_id) {
		if(!hikashop_level(1))
			return;
		...
		$query = 'SELECT p.*, pr.product_related_quantity ' .
			...
			' WHERE pr.product_id = ' . (int)$product_id.' AND pr.product_related_type = ' . $database->Quote('bundle').
			' ORDER BY pr.product_related_ordering ASC, pr.product_related_id ASC';
		$database->setQuery($query);
	...
	function loadPageProductVariants(&$element, &$ids, $selected_variant_id = 0) {
		$database = JFactory::getDBO();
		$product_id = (int)$element->product_id;
		$filters = array(
			'product_parent_id IN ('.implode(',', $ids).')'
		...
		if(!empty($variants)) {
			foreach($variants as $key => $variant) {
				if(empty($variant->product_published)) {
					if($variant->product_parent_id == $product_id) {
						$element->has_unpublished_variant = true;
					} elseif(!empty($element->options)) {
						foreach($element->options as $k => $optionElement) {
							if($variant->product_parent_id == $optionElement->product_id) {
								$element->options[$k]->has_unpublished_variant = true;
								break;
							}
						}
					}
	...
	function loadPageProductCharacteristics(&$element, &$ids, $product_id, $selected_variant_id = 0) {
		$config = hikashop_config();
		$database = JFactory::getDBO();
	...
	function loadPageProductFiles(&$element, &$ids) {
		$database = JFactory::getDBO();
	...
	function loadPageProductPrices(&$element, &$ids, $options = array()) {
		$config = hikashop_config();
		$currency_id = isset($options['currency_id']) ? (int)$options['currency_id'] : hikashop_getCurrency();
		$main_currency = (int)$config->get('main_currency', 1);
		$zone_id = isset($options['zone_id']) ? (int)$options['zone_id'] : hikashop_getZone(null);
	...
	function loadPageProductCustomFields(&$element, $options = array()) {
		$fieldsClass = hikashop_get('class.field');
		$fields = $fieldsClass->getFields('frontcomp', $element, 'product', 'checkout&task=state');

		$element->_custom_fields = $fields;
		$element->_item_fields = null;

		$config = hikashop_config();
		$default_params = $config->get('default_params');

		if(hikashop_level(2)) {
			$catalogue = isset($options['catalogue']) ? (int)$options['catalogue'] : (int)$config->get('catalogue', 0);
			if(!$catalogue) {
				$display_item_fields = true;
				if(!$config->get('display_add_to_cart_for_free_products') || (!$config->get('display_add_to_wishlist_for_free_products', 1) && $config->get('enable_wishlist', 1))) {
					if(empty($element->variants)) {
						if(empty($element->prices))
							$display_item_fields = false;
					} else {
						$variants_are_all_free = empty($element->prices);
				...
				if($display_item_fields) {
					$element->_item_fields = $fieldsClass->getFields('frontcomp', $element, 'item', 'checkout&task=state');

					foreach($element->_item_fields as $fieldName => $oneExtraField) {
	...
	function loadPageProductCategories(&$element) {
		static $cache = array();
		$pid = ($element->product_type == 'variant' && !empty($element->product_parent_id)) ? (int)$element->product_parent_id : (int)$element->product_id;

		if(isset($cache[$pid])) {
			$element->_categories = $cache[$pid];
			return;
		}
		...
		$query = 'SELECT b.* '.
			...
			' ORDER BY a.ordering ASC, a.product_category_id ASC';
		$database->setQuery($query);
		$element->_categories = $database->loadObjectList('category_id');
		$cache[$pid] = $element->_categories;
	}
}
