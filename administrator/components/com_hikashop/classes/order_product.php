...
class hikashopOrder_productClass extends hikashopClass {
	...
	public function save(&$products) {
		...
		foreach($products as $product) {
			...
			$line = array(
				...
				$this->database->Quote((string)$product->order_product_name),
				$this->database->Quote((string)$product->order_product_code),
				$this->database->Quote((string)@$product->order_product_price),
				$this->database->Quote((string)@$product->order_product_tax),
				$this->database->Quote((string)@$product->order_product_options),
				(int)@$product->cart_product_id,
				$this->database->Quote((string)@$product->order_product_tax_info),
				(int)@$product->order_product_wishlist_id,
				(int)@$product->order_product_wishlist_product_id,
				$this->database->Quote((string)@$product->order_product_shipping_id),
				$this->database->Quote((string)@$product->order_product_shipping_method),
				(float)@$product->order_product_shipping_price,
				(float)@$product->order_product_shipping_tax,
				$this->database->Quote((string)@$product->order_product_shipping_params),
				(float)@$product->order_product_weight,
				$this->database->Quote((string)@$product->order_product_weight_unit),
				(float)@$product->order_product_width,
				(float)@$product->order_product_length,
				(float)@$product->order_product_height,
				$this->database->Quote((string)@$product->order_product_dimension_unit),
				...
				$this->database->Quote((string)@$product->order_product_discount_info),
			...
			if(!empty($itemFields)) {
				foreach($itemFields as $field) {
					...
					$line[] = $this->database->Quote((string)@$product->$namekey);
				}
			}
	...
	public function update(&$product) {
		...
		if(!empty($product->tax_namekeys) && is_string($product->tax_namekeys)) {
			$product->tax_namekeys = explode(',', $product->tax_namekeys);
		}
		if(!empty($product->tax_namekeys) && is_array($product->tax_namekeys)) {
			$taxClass = hikashop_get('class.tax');
			$taxInfoArray = array();
			$basePrice = (float)@$product->order_product_price;
			foreach($product->tax_namekeys as $namekey) {
				if(empty($namekey) || $namekey == '-1')
					continue;
				$taxRate = $taxClass->get($namekey);
				if(!$taxRate)
					continue;
				$tax = new stdClass();
				$tax->tax_namekey = $namekey;
				$tax->tax_amount = round($basePrice * (float)$taxRate->tax_rate, 5);
				$tax->amount = $basePrice;
				$taxInfoArray[] = $tax;
			}
			if(!empty($taxInfoArray)) {
				$product->order_product_tax_info = $taxInfoArray;
				$totalTax = 0;
				foreach($taxInfoArray as $t) {
					$totalTax += $t->tax_amount;
				}
				$product->order_product_tax = round($totalTax, 5);
			}
		} elseif(!empty($product->tax_namekey) && $product->tax_namekey != '-1') {
			$tax = new stdClass();
		...
		$nonColumns = array(
			'change', 'tax_namekey', 'tax_namekeys', 'bundle_done',
			'order_product_total_price', 'order_product_total_price_no_vat',
			'cart_product_id', 'cart_product_option_parent_id',
			'no_update_qty', 'discount',
		);
		foreach($nonColumns as $p) {
			unset($product->$p);
		}
