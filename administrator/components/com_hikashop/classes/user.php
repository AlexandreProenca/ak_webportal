...
class hikashopUserClass extends hikashopClass {
	...
	public function get($id, $type = 'hikashop', $geoloc = false, $preferType = null) {
		static $data = array();
		...
		$cacheKey = $type.'_'.$id.(!empty($preferType) ? '_'.$preferType : '');
		if(!empty($data[$cacheKey]))
		...
		switch($type) {
			...
			case 'email':
				...
				$orderBy = ' ORDER BY FIELD(a.user_type, ' .
					$this->database->Quote(!empty($preferType) ? $preferType : 'registered') . ') DESC, a.user_id DESC';
				break;
			case 'cms':
			default:
				$field = 'user_cms_id';
		...
		$query = 'SELECT '.$select.' FROM '.hikashop_table('user').' AS a LEFT JOIN '.hikashop_table('users', false).' AS b ON a.user_cms_id = b.id ' . $geo . ' WHERE a.' . $field . ' = ' . $id . $orderBy . ' LIMIT 1';
	...
	public function save(&$element, $skipJoomla = false) {
		...
		if(empty($element->user_type) && isset($element->user_cms_id))
			$element->user_type = empty($element->user_cms_id) ? 'guest' : 'registered';
	...
	public function registerGuest($user_id, $registerData) {
		...
		$hikaUser->user_type = 'registered';
		$promotedId = $this->save($hikaUser);
		if(empty($promotedId)) {
			$ret['status'] = false;
			$ret['messages'][] = array(JText::_('USER_WITH_SAME_EMAIL_ADDRESS_ALREADY_EXISTS'), 'error');
			return $ret;
		}
