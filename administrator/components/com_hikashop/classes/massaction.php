...
class hikashopMassactionClass extends hikashopClass{
	...
	public static $cancelledStatic = false;
	public static $cancelMessageStatic = '';
	...
	function getSpecialActions() {
		...
		if($special === null) {
			...
			JFactory::getApplication()->triggerEvent('onMassactionSpecialActions', array(&$special));
		}
		return $special;
	}

	function canUseSpecialActions() {
		return JFactory::getUser()->authorise('core.admin');
	}

	function hasSpecialAction(&$massaction) {
		$special = $this->getSpecialActions();
		if(empty($special) || empty($massaction->massaction_actions) || !is_array($massaction->massaction_actions))
			return false;
		foreach($massaction->massaction_actions as $action) {
			if(isset($action->name) && in_array($action->name, $special))
				return true;
		}
		return false;
	}

	function stripSpecialActions(&$massaction) {
		if($this->canUseSpecialActions())
			return;
		$special = $this->getSpecialActions();
		if(empty($special) || empty($massaction->massaction_actions) || !is_array($massaction->massaction_actions))
			return;
		foreach($massaction->massaction_actions as $k => $action) {
			if(isset($action->name) && in_array($action->name, $special))
				unset($massaction->massaction_actions[$k]);
		}
	}
	...
	function runActions($massaction_id, $type) {
		...
		$this->stripSpecialActions($massaction);
	...
	function saveForm(){
		...
		if($this->hasSpecialAction($element) && !$this->canUseSpecialActions()) {
	...
	function batch() {
		...
		$this->stripSpecialActions($element);
	...
	function trigger($trigger,$elements=array()){
		...
		$massactions = $this->loadMassactions();
		...
		foreach($massactions[$trigger] as $massaction){
			...
			if(self::$cancelledStatic) {
				$result = array(
					'cancelled' => true,
					'message' => self::$cancelMessageStatic
				);
				self::$cancelledStatic = false;
				self::$cancelMessageStatic = '';
				return $result;
			}
		}
		return true;
	...
	function initDefaultDiv(&$value, $key, $type, $table, &$loadedData, $html){
		if($type == 'filter'){
			...
			if(!is_int($key)){
				$filters_html = '<div id="'.$table.$type.'__num__'.$value->name.'">';
				$filters_html .= $html.'</div>';
				$loadedData->massaction_triggers[$key]->name = '';
				return $filters_html;
			}else{
				$loadedData->massaction_triggers[$key]->html = '<div id="'.$table.$type.'area_'.$key.'" class="hikamassactionarea">';
				$loadedData->massaction_triggers[$key]->html .= $html.'</div>';
			}
					...
					case 'price':
						...
						if($taxedPrice){
							...
							foreach($productClass->all_products as $p) {
								if(!empty($p->product_parent_id) && empty($p->product_tax_id) && !isset($productClass->all_products[$p->product_parent_id]))
									$parentIds[$p->product_parent_id] = $p->product_parent_id;
							}
							if(!empty($parentIds)) {
								$database->setQuery('SELECT * FROM '.hikashop_table('product').' WHERE product_id IN ('.implode(',', $parentIds).')');
								$parents = $database->loadObjectList('product_id');
								if(!empty($parents)) {
									foreach($parents as $pid => $parent)
										$productClass->all_products[$pid] = $parent;
								}
							}
							...
							foreach($rows as $k => $row){
								...
								elseif(isset($productClass->all_products[$row->price_product_id]->product_parent_id) && isset($productClass->all_products[$productClass->all_products[$row->price_product_id]->product_parent_id]->product_tax_id) && !empty($productClass->all_products[$productClass->all_products[$row->price_product_id]->product_parent_id]->product_tax_id))
				...
				switch($switch_table){
					...
					case 'usergroups':
						if(defined('HIKASHOP_WORDPRESS')) {
							$database->setQuery('SELECT user_id, user_cms_id FROM ' . hikashop_table('user') . ' WHERE user_id IN (' . implode(',', $ids) . ')');
							$hk_users = $database->loadObjectList();
							$rows = array();
							$wp_roles = wp_roles();
							$role_rid_map = array();
							$rid = 1;
							foreach($wp_roles->role_names as $slug => $rname) {
								$role_rid_map[$slug] = $rid;
								$rid++;
							}
							foreach($hk_users as $hk_user) {
								$wp_user = get_userdata((int)$hk_user->user_cms_id);
								if(empty($wp_user) || empty($wp_user->roles)) continue;
								foreach($wp_user->roles as $role_slug) {
									if(!isset($role_rid_map[$role_slug])) continue;
									$obj = new stdClass();
									$obj->title = translate_user_role($wp_roles->role_names[$role_slug]);
									$obj->user_id = (int)$hk_user->user_id;
									$obj->usergroups_id = $role_rid_map[$role_slug];
									$rows[] = $obj;
								}
							}
							return $rows;
		...
		if(!empty($params->action)){
			...
			foreach($params->action as $keyTable=>$table){
			    ...
			    if($keyTable == 'usergroups') {
			        $component = false;
			        if(defined('HIKASHOP_WORDPRESS')) {
			            $columnsArrays[$keyTable] = array('title' => 'varchar', 'usergroups_id' => 'int');
