...
class hikashopCurrencyClass extends hikashopClass{
	...
	protected function _shouldReplacePriceForSite($currentPrice, $newPrice) {
		static $multisites = null;
		if($multisites === null) {
			$multisites = file_exists(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_multisites'.DS.'helpers'.DS.'utils.php');
		}

		$currentSiteId = '';
		if($currentPrice !== null && !empty($currentPrice->price_site_id)) {
			$currentSiteId = ($currentPrice->price_site_id === '[unselected]' || $currentPrice->price_site_id === '0') ? '' : $currentPrice->price_site_id;
		}
		$newSiteId = '';
		if(!empty($newPrice->price_site_id)) {
			$newSiteId = ($newPrice->price_site_id === '[unselected]' || $newPrice->price_site_id === '0') ? '' : $newPrice->price_site_id;
		}

		if($currentPrice === null) {
			if(!$multisites && !empty($newSiteId)) {
				return false;
			}
			return true;
		}
	...
	function round($price, $round = 2, $increment = 0, $force = false) {
		...
		$roundMode = (int)$config->get('round_calculations', 0);
		if(!$roundMode && !$force)
			return $price;
	...
	function getTaxType($reset = false) {
		...
		$user_address = $addressClass->getCurrentUserAddress($this->_getTaxAddressType(), null);
	...
	private function _getTaxAddressType() {
		$config =& hikashop_config();
	...
	function _matchPostCode(&$taxPlan) {
		...
		$address_type = $this->_getTaxAddressType();
	...
	function getProductsPrices(&$rows, $options = array()) {
		...
		if(!empty($prices))
			$this->filterPricesByZone($prices, $zone_id);
		...
		foreach($rows as $k => $element) {
			...
			if(!empty($prices)) {
				...
				foreach($prices as $price) {
					...
					$currentPrice = isset($matches[$price->price_min_quantity]) ? $matches[$price->price_min_quantity] : null;
					if($this->_shouldReplacePriceForSite($currentPrice, $price)) {
	...
	function getProductPrices(&$element, &$ids, $options) {
		...
		$query = 'SELECT p.*, CASE WHEN p.price_site_id = 0 OR p.price_site_id = \'[unselected]\' THEN \'\' ELSE p.price_site_id END AS price_site_id FROM '.hikashop_table('price').' as p WHERE ('.implode(') AND (', $filters). ') ORDER BY price_site_id ASC, p.price_value DESC';
		$this->database->setQuery($query);
		$prices = $this->database->loadObjectList();
	...
	function filterPricesByZone(&$prices, $zone_id) {
		$hasZoneRestriction = false;
		foreach($prices as $price) {
	...
	function _removePrices(&$elementPrices, &$prices, $main_currency) {
		foreach($elementPrices as $currency => $currencyPrices) {
			...
			foreach($currencyPrices as $quantityPrices) {
				...
				$unique_price_k = null;
				foreach($quantityPrices as $k){
					$currentPrice = ($unique_price_k !== null) ? $prices[$unique_price_k] : null;
					if($this->_shouldReplacePriceForSite($currentPrice, $prices[$k])) {
						$unique_price_k = $k;
					}
				}

				if($unique_price_k !== null) {
					foreach($quantityPrices as $k){
						if($k != $unique_price_k) {
							unset($prices[$k]);
						}
	...
	function format($number, $currency_id = 0, $format_override = '') {
		...
		$config = hikashop_config();
		$roundMode = (int)$config->get('round_calculations', 0);
		if($roundMode != 1 && !empty($locale['rounding_increment']) && $locale['rounding_increment'] > 0.00001){
