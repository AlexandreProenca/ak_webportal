...
class hikashopMenusClass extends hikashopClass {
	...
	function loadAllMenuItemIds($view = 'category', $layout = 'listing', $exclude = array()) {
		if(defined('HIKASHOP_WORDPRESS'))
			return array();
		...
		return is_array($cache[$key]) ? $cache[$key] : array();
	}

	function loadAMenuItemId($view = 'category', $layout = 'listing', $id = 0, $exclude = array(), $target_category_id = 0) {
		if(defined('HIKASHOP_WORDPRESS'))
			return 0;
		...
		if(!empty($id) && !empty($view)) {
			$filters = array(
				...
				'b.title IS NOT NULL',
				'a.client_id=0',
				'a.link LIKE \'index.php?option=com_hikashop&view='.($view=='manufacturer'?'category':$view).'&layout=%\''
			);
		...
		$menuIds = $this->loadAllMenuItemIds($view, $layout, $exclude);

		if($id){
			if(count($menuIds)){
				foreach($menuIds as $current_id){
					if($current_id==$id){
						return (int)$id;
					}
				}
		...
		if(in_array($view, array('product', 'manufacturer', 'category'))) {
			if(count($menuIds))
				return $this->_findBestCategoryMenuId($view, $menuIds, $target_category_id);
			return 0;
		}

		if(count($menuIds)){
			return (int)reset($menuIds);
		}
		return 0;
	}

	protected function _findBestCategoryMenuId($view, $menuIds, $target_category_id = 0) {
		$app = JFactory::getApplication();
		$config = hikashop_config();

		$brandIds = $this->_getManufacturerCategoryIds();
		$target_category_id = (int)$target_category_id;

		if($target_category_id > 0 && HIKASHOP_J30) {
			foreach($menuIds as $current_id) {
				$menuItem = $app->getMenu()->getItem($current_id);
				if(!$menuItem) continue;
				$categoryParams = $menuItem->getParams()->get('hk_category');
				if(isset($categoryParams->category) && (int)$categoryParams->category === $target_category_id)
		...
		if($target_category_id > 0 && HIKASHOP_J30) {
			$categoryClass = hikashop_get('class.category');
			$ancestorIds = array($target_category_id);
			$parents = $categoryClass->getParents($target_category_id);
			if(!empty($parents)) {
				foreach($parents as $p) {
					$ancestorIds[] = (int)$p->category_id;
				}
			}
			...
			foreach($menuIds as $current_id) {
				...
				$categoryParams = $menuItem->getParams()->get('hk_category');
				$menuCat = isset($categoryParams->category) ? (int)$categoryParams->category : 0;
				if($menuCat > 0 && !in_array($menuCat, $ancestorIds, true)
					&& in_array($menuCat, $brandIds, true)) {
					continue;
				}
				$filtered[] = $current_id;
			}
		...
		foreach($filtered as $current_id) {
			if(HIKASHOP_J30) {
				$menuItem = $app->getMenu()->getItem($current_id);
				if(!$menuItem) continue;
				$categoryParams = $menuItem->getParams()->get('hk_category');
				if(
					(isset($categoryParams->content_type) && $categoryParams->content_type == 'manufacturer')
					|| (isset($categoryParams->category) && in_array((int)$categoryParams->category, $brandIds, true))
				)
					return (int)$current_id;
			}
		...
		if(!empty($filtered))
			return (int)reset($filtered);
		return 0;
	}

	protected function _getManufacturerCategoryIds() {
		...
		if($brandIds === null) {
			...
			$raw = $this->database->loadColumn();
			$brandIds = is_array($raw) ? array_map('intval', $raw) : array();
		}
		return $brandIds;
	}

	function getBestCategoryMenuItemId($category_id) {
		if(defined('HIKASHOP_WORDPRESS'))
			return 0;

		$menuIds = $this->loadAllMenuItemIds('category', 'listing');
		if(empty($menuIds))
			return 0;
		...
		foreach($menuIds as $menuId) {
			...
			if(HIKASHOP_J30) {
				...
				if(!empty($menuItem)) {
					$categoryParams = $menuItem->getParams()->get('hk_category');
					if(!empty($categoryParams->selectparentlisting))
						$rootCategory = (int)$categoryParams->selectparentlisting;
				}
			}

			if(empty($rootCategory)) {
				$options = $config->get('menu_'.$menuId, null);
				if(!empty($options['selectparentlisting']))
					$rootCategory = (int)$options['selectparentlisting'];
			}

			if(!empty($rootCategory)) {
				$depth = array_search($rootCategory, $ancestorIds);
				if($depth !== false && $depth > $bestDepth) {
					$bestDepth = $depth;
					$bestMenuId = (int)$menuId;
				}
			}

			if($bestDepth < 0 && empty($bestMenuId))
				$bestMenuId = (int)$menuId;
		}
	...
	function createMenu(&$moduleOtpions,$id){
		...
		$alias = 'hikashop-menu-for-module-'.(int)$id;
		$this->database->setQuery('SELECT id FROM '.hikashop_table('menu',false).' WHERE alias='.$this->database->Quote($alias));
		...
		if(empty($moduleOtpions['itemid'])){
			...
			$this->database->execute();
			$this->database->setQuery('SELECT id FROM '.hikashop_table('menu',false).' WHERE alias='.$this->database->Quote($element->alias));
			$moduleOtpions['itemid'] = $this->database->loadResult();
		}
		if(!empty($moduleOtpions['itemid'])){
