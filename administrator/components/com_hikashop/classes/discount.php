...
class hikashopDiscountClass extends hikashopClass {
	...
	public function loadAndCheck($coupon_code, &$total, $zones, &$products, $display_error = true, $cart = null) {
		$coupon = $this->load($coupon_code);
		if(empty($coupon))
			return null;
		return $this->check($coupon, $total, $zones, $products, $display_error, $cart);
	}

	public function check(&$coupon, &$total, $zones, &$products, $display_error = true, $cart = null) {
		if(empty($coupon))
			return null;
	...
	public function afterShippingProcessing(&$cart){
		...
		foreach($taxes as $tax) {
			...
			if(isset($cart->full_total->prices[0]->taxes[$tax->tax_namekey])) {
				...
				$cart->full_total->prices[0]->taxes[$tax->tax_namekey] = clone($tax);
		}
	}
