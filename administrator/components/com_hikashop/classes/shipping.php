...
class hikashopShippingClass extends hikashopClass {
	...
	function save(&$element, $reorder = true) {
		...
		if(!empty($element->shipping_params)
			&& !is_string($element->shipping_params)
			&& isset($element->shipping_params->shipping_warehouse_filter)
			&& is_array($element->shipping_params->shipping_warehouse_filter)) {
			$element->shipping_params->shipping_warehouse_filter = implode(
				',',
				array_map('intval', $element->shipping_params->shipping_warehouse_filter)
			);
		}
		...
		if($translationHelper->isMulti()) {
			...
			$this->_addTranslatableParamsForCheck($element, 'shipping', $columns);
	...
	public function getExportColumns($filter = false) {
		...
		if($filter) {
			...
			$selected_fields = $config->get('shipping_export_columns');
		...
		if(!empty($shipping_groups) && count($shipping_groups) > 1) {
			...
			foreach($shipping_groups as $key => &$group) {
				...
				foreach($rates as $rate) {
					if(empty($rate->shipping_published))
						continue;
					if($this->matchWarehouseFilter($rate, $key, $shipping_group_struct))
			...
			foreach($rates as $i => $rate) {
				if(empty($rate->shipping_params->shipping_warehouse_filter))
					continue;
				if(!$this->matchWarehouseFilter($rate, $key)) {
					$rates[$i] = null;
					unset($rates[$i]);
				}
			}
	...
	private function matchWarehouseFilter($rate, $key, $shipping_group_struct = null) {
		if(empty($rate->shipping_params->shipping_warehouse_filter))
			return true;

		$filter = $rate->shipping_params->shipping_warehouse_filter;
		$exclude = !empty($rate->shipping_params->shipping_warehouse_filter_type);

		if($shipping_group_struct === null) {
			if(is_array($key)) {
				$shipping_group_struct = $key;
			} else {
				$shipping_group_struct = array('' => $key);
				if(is_string($key) && preg_match_all('#([a-zA-Z])*([0-9]+)#iu', $key, $parsed_keys)) {
					$shipping_group_struct = array_combine($parsed_keys[1], $parsed_keys[2]);
				}
			}
		}
		$group_warehouse = isset($shipping_group_struct['']) ? (int)$shipping_group_struct[''] : 0;

		if(is_array($filter))
			$filter = implode(',', array_map('intval', $filter));
		$filter_str = (string)$filter;

		$filter_warehouses = array();
		$filter_components = array();

		if(preg_match_all('#([a-zA-Z])*([0-9]+)#iu', $filter_str, $matches)) {
			for($i = 0; $i < count($matches[0]); $i++) {
				$letter = $matches[1][$i];
				$number = (int)$matches[2][$i];
				if($letter === '') {
					$filter_warehouses[] = $number;
				} else {
					$filter_components[$letter] = $number;
				}
			}
		}

		if(empty($filter_warehouses))
			$filter_warehouses = array(0);

		if($exclude) {
			$warehouse_match = !in_array($group_warehouse, $filter_warehouses);
		} else {
			$warehouse_match = in_array($group_warehouse, $filter_warehouses) || in_array(0, $filter_warehouses);
		}

		if(!$warehouse_match)
			return false;

		foreach($filter_components as $k => $v) {
	...
	function fillListingColumns(&$rows, &$listing_columns, &$view, $type = null) {
		...
		foreach($rows as &$row) {
			...
			if(!empty($row->plugin_params->shipping_warehouse_filter)) {
				$warehouse_filter = $row->plugin_params->shipping_warehouse_filter;
				if(is_array($warehouse_filter))
					$warehouse_ids = array_map('intval', $warehouse_filter);
				else
					$warehouse_ids = array_map('intval', explode(',', $warehouse_filter));
				$list = array();
				...
				$mode = !empty($row->plugin_params->shipping_warehouse_filter_type) ? JText::_('EXCLUDE') : JText::_('INCLUDE');
				$restrictions[] = JText::_('WAREHOUSE') . ' (' . $mode . '): ' . implode(', ', $list);
			}
	...
	function _addTranslatableParamsForCheck(&$element, $type, &$columns) {
		...
		foreach($plugin->pluginConfig as $key => $config) {
			...
			if(isset($element->old)) {
				$oldParams = null;
				if(!empty($element->old->$params_field)) {
					$oldParams = $element->old->$params_field;
					if(is_string($oldParams))
						$oldParams = hikashop_unserialize($oldParams);
				}
				$element->old->$prop = '';
				if(!empty($oldParams) && isset($oldParams->$key))
					$element->old->$prop = $oldParams->$key;
			}
		}
	}
}
