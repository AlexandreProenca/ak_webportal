...
class hikashopFileClass extends hikashopClass {
		...
		if(!hikashop_isClient('administrator')) {
			if(empty($file->file_free_download)) {
				...
				if(empty($user_id) && !empty($token)) {
					...
					if(!empty($user) && empty($user->user_cms_id) && hash_equals($order->order_token, $token)) {
	...
	function sendFile(&$file, $is_resume = true, $path = null, $options = array()) {
		...
		if($real_path !== false && strpos($real_filename, $real_path) !== 0) {
			$secure_path = $this->getPath('file');
			$real_secure = realpath($secure_path);
			$real_root = (JPATH_ROOT != '') ? realpath(JPATH_ROOT) : false;
			if(($real_secure === false || strpos($real_filename, $real_secure) !== 0) && ($real_root === false || strpos($real_filename, $real_root) !== 0))
				return false;
		}
		...
		if(!hikashop_isClient('administrator')) {
			...
			if(!empty($field)) {
				...
				if(!$found) {
					...
					$user_condition = '(a.order_user_id = '.(int)hikashop_loadUser().' OR a.order_token = '.$this->database->Quote($token).')';
					switch($field_table) {
						case 'order':
