...
class hikashopCategoryClass extends hikashopClass {
	...
	function delete(&$elements) {
		...
		foreach($elements as $element) {
			...
			if($data->category_type == 'product') {
				$query = 'SELECT product_id FROM '.hikashop_table('product_category').' WHERE category_id='.(int)$element;
				$this->database->setQuery($query);
			...
			if($data->category_right - $data->category_left != 1 ) {
				$query = 'UPDATE '.$table.' SET '.$parent.' = '.(int)$data->$parent.' WHERE '.$parent.' = '.(int)$element;
				$this->database->setQuery($query);
	...
	function buildBreadcrumbItems($category_id, $parent = 0, $url_itemid = '') {
		$items = array();
		if(empty($category_id))
			return $items;

		$categories = $this->getParents($category_id, $parent);
		if(empty($categories))
			return $items;

		array_shift($categories);

		$cat_itemid = $url_itemid;
		if(!defined('HIKASHOP_WORDPRESS')) {
			$menusClass = hikashop_get('class.menus');
			$catMenuId = $menusClass->getBestCategoryMenuItemId($category_id);
			if(!empty($catMenuId))
				$cat_itemid = '&Itemid='.$catMenuId;
		}

		foreach($categories as $category) {
			$this->addAlias($category);
			$items[] = (object)array(
				'name' => hikashop_translate($category->category_name),
				'link' => hikashop_completeLink('category&task=listing&cid='.(int)$category->category_id.'&name='.$category->alias.$cat_itemid)
			);
		}
	...
	public function &getNameboxData($typeConfig, &$fullLoad, $mode, $value, $search, $options) {
		...
		if($typeConfig['mode'] == 'list')
			...
			$order = ' ORDER BY c.category_parent_id ASC, c.category_ordering ASC';
