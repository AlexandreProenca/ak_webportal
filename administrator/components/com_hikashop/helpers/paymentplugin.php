...
';
	...
	function getPaymentURL(&$order, &$method) {
		return null;
	}
	...
	function modifyOrder(&$order_id, $order_status, $history = null, $email = null, $payment_params = null) {
		...
		$id = is_object($order_id) ? @$order_id->order_id : $order_id;

		if(!empty($id) || is_object($order_id)) {
			$orderClass->modifyOrder($order_id, $order_status, $this->name, $history, $email, $payment_params);
			return;
		}
	...
	function getUcpGooglePayConfig() { return null; }
}
