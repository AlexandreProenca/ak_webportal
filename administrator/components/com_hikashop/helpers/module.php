...
#'.$main_div_name.' .hikashop_subcontainer { -moz-border-radius: 5px; -webkit-border-radius: 5px; -khtml-border-radius: 5px; border-radius: 5px; }';
	...
	public function setModuleData($modules) {
		if(empty($modules) || defined('HIKASHOP_WORDPRESS')) {
			$modules = array();
			return $modules;
		}
	...
	public function _getParams(&$obj) {
		...
		if (is_object( $menuParams )) {
			if(!defined('HIKASHOP_WORDPRESS') && HIKASHOP_J30 && (($menu->query['view'] == 'category' && (!$menuParams->get('hk_category',false) || !$menuParams->get('hk_product',false))) || ($menu->query['view'] == 'product' && !$menuParams->get('hk_product',false)))){
				$db = JFactory::getDBO();
