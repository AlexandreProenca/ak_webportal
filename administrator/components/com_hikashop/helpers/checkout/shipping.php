...
class hikashopCheckoutShippingHelper extends hikashopCheckoutHelperInterface {
	protected $params = array(
		...
		'dynamic_address_refresh' => array(
			'name' => 'DYNAMIC_ADDRESS_REFRESH',
			'type' => 'boolean',
			'default' => 1,
			'tooltip' => 'shipping_dynamic_address_refresh',
			'showon' => array(
				'key' => 'read_only',
				'values' => array(0)
			)
	...
	protected function applyAddressPreview(&$params) {
		if(empty($params['dynamic_address_refresh']))
			return;

		$previewData = hikaInput::get()->get('preview_address', array(), 'array');
		...
		$previewAddr = new stdClass();
		$allowedFields = array('address_country', 'address_state', 'address_city', 'address_post_code', 'address_street');
		$hasData = false;
		foreach($allowedFields as $field) {
			if(!empty($previewData[$field])) {
				$previewAddr->$field = htmlspecialchars(strip_tags(trim($previewData[$field])), ENT_QUOTES, 'UTF-8');
				$hasData = true;
			} else {
				$previewAddr->$field = '';
			}
		...
		$previewAddr->address_id = $addressClass::SHIPPING_PREVIEW_ID;
		$app = \Joomla\CMS\Factory::getApplication();
		$app->setUserState(HIKASHOP_COMPONENT.'.shipping_address_preview', hikashop_copy($previewAddr));

		$zoneClass = hikashop_get('class.zone');
		$zoneField = !empty($previewAddr->address_state) ? 'address_state' : 'address_country';
		$tax_zone_id = null;
		if(!empty($previewAddr->$zoneField)) {
			$zoneObj = $zoneClass->get($previewAddr->$zoneField);
			if(!empty($zoneObj))
				$tax_zone_id = $zoneObj->zone_id;
		}

		$addressClass = hikashop_get('class.address');
		$addressArray = array(&$previewAddr);
		$addressClass->loadZone($addressArray, 'parent');

		$cart->shipping_address = $previewAddr;
		...
		if(empty($cart->usable_methods->shipping) && !empty($shippingClass->errors)) {
			...
			$cart->usable_methods->shipping_errors = array();
		}
	...
	public function display(&$view, &$params) {
		...
		if(!isset($params['dynamic_address_refresh']))
			$params['dynamic_address_refresh'] = 1;

		$this->applyAddressPreview($params);
