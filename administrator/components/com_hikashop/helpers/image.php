...
class hikashopImageHelper {
	...
	public function __construct() {
		...
		if($override === null) {
			...
			if(!hikashop_isClient('cli') && !empty($app) && method_exists($app, 'getTemplate')) {
				try{
					$chromeSubDir = defined('HIKASHOP_WORDPRESS') ? 'hikashop' : 'html';
					$chromePath = JPATH_THEMES.DS.$app->getTemplate().DS.$chromeSubDir.DS.'hikashop_image.php';
					if(!file_exists($chromePath)) {
						$parentTemplate = hikashop_getParentTemplate();
						if(!empty($parentTemplate)) {
							$chromePath = JPATH_THEMES.DS.$parentTemplate.DS.$chromeSubDir.DS.'hikashop_image.php';
						}
					}
					if(file_exists($chromePath)) {
	...
	protected function extractICCProfile($filename) {
		...
		while($pos < $len && $counter < 1000) {
			...
			if($type == 0xe2) {
				$size = (ord($f[$pos + 2]) << 8) | ord($f[$pos + 3]);

				if($size >= 14 && substr($f, $pos + 4, 12) == "ICC_PROFILE\x00") {
					$chunk_no = ord($f[$pos + 16]);
					$chunk_cnt = ord($f[$pos + 17]);

					if($chunk_no >= 1 && $chunk_no <= $chunk_cnt) {
						$data_offset = $pos + 18;
						$data_size = $size - 16;
						$chunks[$chunk_no] = substr($f, $data_offset, $data_size);
				...
				$pos += $size + 2;
			} elseif(($type >= 0xe0 && $type <= 0xef) || in_array($type, array(0xc0, 0xc2, 0xc4, 0xdb, 0xda, 0xfe))) {
				$size = (ord($f[$pos + 2]) << 8) | ord($f[$pos + 3]);
				$pos += $size + 2;
			} else {
				$pos += 2;
	...
	protected function injectICCProfile($filename, $icc_data) {
		$f = @file_get_contents($filename);
		if($f === false || strlen($f) < 2)
			return false;

		$max_chunk_data = 65533 - 14;
		$chunk_cnt = (int)ceil(strlen($icc_data) / $max_chunk_data);
		$icc_segments = '';

		for($i = 1; $i <= $chunk_cnt; $i++) {
			$chunk = substr($icc_data, ($i - 1) * $max_chunk_data, $max_chunk_data);
			$chunk_size = strlen($chunk);

			$icc_segments .= "\xff\xe2";
			$icc_segments .= pack('n', $chunk_size + 16);
			$icc_segments .= "ICC_PROFILE\x00";
			$icc_segments .= chr($i) . chr($chunk_cnt);
			$icc_segments .= $chunk;
		}
		...
		$soi = substr($f, 0, 2);
		$rest = substr($f, 2);
		$f = $soi . $icc_segments . $rest;
		return (file_put_contents($filename, $f) !== false);
	}
	...
	public function checkSize(&$width, &$height, &$fileObj) {
		...
		if($imageSize === false)
			return;
		list($f_w, $f_h) = $imageSize;
		if(empty($width)) {
			if($f_h >= $height) {
				$scaled = $this->scaleImage($f_w, $f_h, 0, $height);
				if($scaled !== false)
					list($width, $height) = $scaled;
			} else {
		...
		if(empty($height)) {
			if($f_w >= $width) {
				$scaled = $this->scaleImage($f_w, $f_h, $width, 0);
				if($scaled !== false)
					list($width, $height) = $scaled;
			} else {
	...
	protected function getImage_GD($filename, &$extension) {
		...
		switch($extension) {
			...
			case 'jpeg':
				...
				$ret['icc_profile'] = $this->extractICCProfile($filename);
				break;
			case 'webp':
				case 'webp':
	...
	protected function createThumbRes_GD(&$source, $size, $options = array()) {
		...
		if(!empty($source['icc_profile'])) {
			$ret['icc_profile'] = $source['icc_profile'];
		}
	...
	protected function saveResImage_GD(&$res, $filename) {
		...
		switch($res['ext']) {
			...
			case 'jpeg':
				...
				if($status && !empty($res['icc_profile'])) {
					$this->injectICCProfile($filename, $res['icc_profile']);
				}
				break;
			case 'webp':
				if(empty($res['quality']))
	...
	public function getVideoThumbnail($url) {
		...
		$ret->id = null;
		$ret->type = null;
		$ret->thumbnail_url = null;

		if(preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $url, $match)) {
			$ret->id = $match[1];
			$ret->type = 'youtube';
			$ret->thumbnail_url = 'https://img.youtube.com/vi/'.$ret->id.'/mqdefault.jpg';
		} elseif(preg_match('#vimeo\.com/([0-9]+)#i', $url, $match)) {
			$ret->id = $match[1];
			$ret->type = 'vimeo';
			$data = $this->getRemoteData("http://vimeo.com/api/v2/video/".$ret->id.".json");
			$data = json_decode($data);
			if(!empty($data[0]->thumbnail_medium)) {
				$ret->thumbnail_url = $data[0]->thumbnail_medium;
			}
		} elseif(preg_match('#dailymotion\.com/video/([a-z0-9]+)#i', $url, $match)) {
			$ret->id = $match[1];
			$ret->type = 'dailymotion';
			$data = $this->getRemoteData("https://api.dailymotion.com/video/".$ret->id."?fields=thumbnail_url");
			$data = json_decode($data);
			if(!empty($data->thumbnail_url)) {
				$ret->thumbnail_url = $data->thumbnail_url;
			}
		}

		if(!empty($ret->thumbnail_url)) {
			$ret->success = true;
		}
		return $ret;
	}
	...
	function getThumbnail($filename, $size = null, $options = array(), $relativePath = true, $cachePath = null) {
		...
		if(!empty($filename) && preg_match('#^https?://#i', $filename) === 1) {
			$video = $this->getVideoThumbnail($filename);
			if($video->success) {
				$video_data = $video;
				$video_url = $filename;
				$ext = $this->getFileExtension($video->thumbnail_url);
				if(strpos($ext, '/') !== false || empty($ext)) $ext = 'jpg';
				$local_video_thumb = 'video_'.$video->type.'_'.$video->id.'.'.$ext;
				$full_local_path = $this->uploadFolder . $local_video_thumb;
				if(!file_exists($full_local_path)) {
					$thumb_data = $this->getRemoteData($video->thumbnail_url);
					if($thumb_data) {
						@file_put_contents($full_local_path, $thumb_data);
					}
				}
				if(file_exists($full_local_path)) {
					$filename = $local_video_thumb;
					$fullFilename = $full_local_path;
					$relativePath = true;
					$clean_filename = $full_local_path;
				}
			}
		}
		...
		if($extension == 'svg') {
			...
			if(!preg_match('#^https?://#i', $clean_filename)) {
			...
			if($cachePath !== false && is_file($cachePath . $destFolder . DS . $filename)) {
				...
				if(!empty($video_url)) {
					...
					if($video_data) {
						$ret->video_id = $video_data->id;
						$ret->video_type = $video_data->type;
					}
				}
				return $ret;
			}
...
';
	...
	function generateThumbnail($file_path, $module = false){
		...
		$dstFile = $this->uploadFolder.'thumbnail_'.$thumbnail_y.'x'.$thumbnail_x.DS.$file_path;
		if(is_file($dstFile)) {
			$srcFile = $this->uploadFolder.$file_path;
			if(filectime($srcFile) <= filectime($dstFile))
				return array($thumbnail_y, $thumbnail_x);
		...
		if($new !== false) {
			...
			if($ok && !$module){
				$ok = array($thumbnail_y, $thumbnail_x);
			}
		}
		return $ok;
	}
