...
class hikashopBridgePaginationHelper extends JPagination {
	...
	function getPagesLinks() {
		...
		$chromePath = JPATH_THEMES.DS.$app->getTemplate().DS.$chromeSubDir.DS.'pagination.php';
		if(!file_exists($chromePath)) {
			$parentTemplate = hikashop_getParentTemplate();
			if(!empty($parentTemplate)) {
				$chromePath = JPATH_THEMES.DS.$parentTemplate.DS.$chromeSubDir.DS.'pagination.php';
			}
		}
		if(file_exists($chromePath)) {
	...
	function _list_render($list){
		if (HIKASHOP_J40) {
			...
			$html .= "<a aria-label=\"".JText::_('JLIB_HTML_GOTO_POSITION_START')."\" href=\"".$this->_link($list['start']['base'])."\" class=\"pagenav_previous_chevron page-link\">".
			...
			if(isset($list['previous']['base']))
				$html .= "<a aria-label=\"".JText::_('JLIB_HTML_GOTO_POSITION_PREVIOUS')."\" href=\"".$this->_link($list['previous']['base'])."\" class=\"pagenav_previous_chevron page-link\">".
			...
			if(isset($list['next']['base']))
				$html .= "<a aria-label=\"".JText::_('JLIB_HTML_GOTO_POSITION_NEXT')."\" href=\"".$this->_link($list['next']['base'])."\" class=\"pagenav_next_chevron page-link\">".
			...
			if(isset($list['end']['base']))
				$html .= "<a aria-label=\"".JText::_('JLIB_HTML_GOTO_POSITION_END')."\" href=\"".$this->_link($list['end']['base'])."\" class=\"pagenav_end_chevron page-link\">".
	...
	function getLimitBox($minimum = -1) {
		...
		$ariaLabel = htmlspecialchars(JText::_('HIKA_ITEMS_PER_PAGE'), ENT_QUOTES, 'UTF-8');
		return JHTML::_('select.genericlist',  $limits, 'limit'.$this->hikaSuffix, 'class="chzn-done '.HK_FORM_SELECT_CLASS.' hikashop_pagination_dropdown" size="1" aria-label="'.$ariaLabel.'" onchange="this.form.submit()"', 'value', 'text', $viewall ? 0 : $this->limit);
	}
