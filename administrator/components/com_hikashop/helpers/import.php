...
class hikashopImportHelper extends stdClass
{
	...
	var $relatedCache = array();
	var $deferredDownloads = array();
	var $benchmark_import = false;
	...
	function __construct()
	{
		...
		$uploadFolder = ltrim((string)JPath::clean(html_entity_decode($config->get('uploadfolder'))),DS);
	...
	function importFromFolder($type, $delete, $uploadFolder) {
		...
		$uploadFolder = rtrim((string)JPath::clean(html_entity_decode($uploadFolder)),DS.' ').DS;
	...
	function importFromFile(&$importFile, $process = true){
		...
		$uploadFolder = (string)JPath::clean(html_entity_decode($config->get('uploadfolder')));
		$uploadFolder = trim($uploadFolder,DS.' ').DS;
		$uploadPath = (string)JPath::clean(HIKASHOP_ROOT.$uploadFolder);
	...
	function handleContent(&$contentFile){
		...
		if($this->benchmark_import) {
			hikashop_writeToLog('Import benchmark: Starting import with ' . count($this->importLines) . ' lines, batch size ' . $this->perBatch);
		...
		if($this->benchmark_import) {
			hikashop_writeToLog('Import benchmark: CSV parsing + DB inserts took ' . round((microtime(true) - $parseStart) * 1000, 2) . 'ms');
			$downloadStart = microtime(true);
		}

		$this->_deleteUnecessaryVariants();

		$this->_processDeferredDownloads();

		if($this->benchmark_import) {
			hikashop_writeToLog('Import benchmark: Deferred downloads took ' . round((microtime(true) - $downloadStart) * 1000, 2) . 'ms (' . count($this->deferredDownloads) . ' files)');
			hikashop_writeToLog('Import benchmark: TOTAL IMPORT took ' . round((microtime(true) - $importStart) * 1000, 2) . 'ms for ' . $this->totalTry . ' products');
		}
	...
	function _checkData(&$product,$main=false){
		...
		if(!empty($product->price_value_with_tax)){
			...
			$tax_id_for_conversion = 0;
			if(!empty($product->product_tax_id)){
				$tax_id_for_conversion = $product->product_tax_id;
			}elseif(!empty($product->product_parent_id)){
				$parent_id = (int)$product->product_parent_id;
				if(!empty($parent_id)) {
					if(!isset($this->parentTaxCache))
						$this->parentTaxCache = array();
					if(isset($this->parentTaxCache[$parent_id])) {
						$tax_id_for_conversion = $this->parentTaxCache[$parent_id];
					} else {
						$this->db->setQuery('SELECT product_tax_id FROM '.hikashop_table('product').' WHERE product_id = '.$parent_id);
						$parent_tax = $this->db->loadResult();
						if(!empty($parent_tax))
							$tax_id_for_conversion = $parent_tax;
						$this->parentTaxCache[$parent_id] = $tax_id_for_conversion;
					}
				}
			}
			if(empty($tax_id_for_conversion))
				$tax_id_for_conversion = $currencyHelper->getTaxCategory();

			if($tax_id_for_conversion){
				if(strpos($product->price_value_with_tax,'|')===false){
					$product->price_value_with_tax = hikashop_toFloat($product->price_value_with_tax);
					$product->price_value = $currencyHelper->getUntaxedPrice($product->price_value_with_tax,hikashop_getZone(),$tax_id_for_conversion);
				}else{
					...
					foreach($price_value as $k => $price_value_one){
						...
						$price_value[$k] = $currencyHelper->getUntaxedPrice($price_value_one,hikashop_getZone(),$tax_id_for_conversion);
					}
			...
			foreach($product->files as $k => $file){
				...
				if(substr($file,0,7)=='http://'||substr($file,0,8)=='https://'){
					...
					if(!file_exists($this->uploadFolder.$name)){
						$this->deferredDownloads[] = array(
							'url' => $file,
							'destination' => $this->uploadFolder.$name,
							'type' => 'files'
						);
					}else{
			...
			if(!empty($product->categories)){
				...
				if($this->createCategories && !empty($product->categories_image)){
					...
					foreach($product->categories_image as $k => $image){
						...
						if(substr($image,0,7)=='http://'||substr($image,0,8)=='https://'){
							if(!hikashop_isExternalUrlSafe($image))
								continue;
							$parts = explode('/',$image);
							$name = array_pop($parts);
							if(!file_exists($this->uploadFolder.$name)){
	...
	function getSizeFile($url) {
		if (substr($url,0,4) == 'http') {
			if(!hikashop_isExternalUrlSafe($url))
				return false;
	...
	function _prefetchCategories() {
		$this->db->setQuery('SELECT COUNT(*) FROM '.hikashop_table('category'));
		$totalCategories = (int)$this->db->loadResult();

		if($totalCategories > 4000) {
			return;
		}

		foreach(array('product', 'tax', 'manufacturer') as $type) {
			$query = 'SELECT category_id, category_namekey, category_name, category_parent_id 
					  FROM '.hikashop_table('category').' WHERE category_type='.$this->db->Quote($type);
			$this->db->setQuery($query);
			$results = $this->db->loadObjectList();

			$this->categoryCache[$type] = array('name' => array(), 'namekey' => array());
			if(!empty($results)) {
				foreach($results as $cat) {
					if(!empty($cat->category_namekey)) {
						$this->categoryCache[$type]['namekey'][$cat->category_namekey] = $cat->category_id;
					}
					if(!empty($cat->category_name)) {
						$key = $cat->category_name . '__' . $cat->category_parent_id;
						$this->categoryCache[$type]['name'][$key] = $cat->category_id;
						if(!isset($this->categoryCache[$type]['name'][$cat->category_name])) {
							$this->categoryCache[$type]['name'][$cat->category_name] = $cat->category_id;
						}
					}
				}
			}
	...
	function _prefetchRelatedProducts(&$products) {
		$codes = array();
		foreach($products as $product) {
			foreach(array('related', 'options', 'bundle') as $type) {
				if(!empty($product->$type) && is_array($product->$type)) {
					foreach($product->$type as $code) {
						if(!empty($code) && !is_numeric($code) && !isset($this->relatedCache[$code])) {
							$codes[$code] = $this->db->Quote((string)$code);
						}
					}
				}
			}
		...
		if(!empty($codes)) {
			$query = 'SELECT product_id, product_code FROM '.hikashop_table('product').
					 ' WHERE product_code IN ('.implode(',', $codes).')';
			$this->db->setQuery($query);
			$results = $this->db->loadObjectList('product_code');
			if(!empty($results)) {
				foreach($results as $code => $r) {
					$this->relatedCache[$code] = $r->product_id;
				}
			}
		}
	}

	function _processDeferredDownloads() {
		if(empty($this->deferredDownloads)) {
			return;
		}
		...
		foreach($this->deferredDownloads as $download) {
			if(file_exists($download['destination'])) {
				continue;
			}

			if(!hikashop_isExternalUrlSafe($download['url'])) {
				$failCount++;
				continue;
			}

			$content = @file_get_contents($download['url'], false, $ctx);

			if(!empty($content)) {
				if(JFile::write($download['destination'], $content)) {
					$downloadCount++;

					if($download['type'] == 'images' && $imageHelper) {
						$filename = basename($download['destination']);
						$imageHelper->resizeImage($filename);
					}
				} else {
					$failCount++;
				}
		...
		if($downloadCount > 0 || $failCount > 0) {
			$app->enqueueMessage(JText::sprintf('Downloaded %d remote files. %d failed.', $downloadCount, $failCount), $failCount > 0 ? 'warning' : 'message');
		}
	...
	function _getCategory($code,$newId=0,$error=true,$type='product',$parent_id=0,$image=''){
		...
		if(!empty($newId)){
			$data[$code] = $newId;
			$data[$namekey] = $newId;
			if(!empty($type) && isset($this->categoryCache[$type])) {
				$this->categoryCache[$type]['name'][$code] = $newId;
				$this->categoryCache[$type]['name'][$namekey] = $newId;
			}
		}
		if(!isset($data[$namekey])){
			$foundInCache = false;
			if(!empty($this->categoryCache[$type])) {
				if(isset($this->categoryCache[$type]['namekey'][$code])) {
					$data[$namekey] = $this->categoryCache[$type]['namekey'][$code];
					$foundInCache = true;
				}
				elseif(!empty($parent_id) && isset($this->categoryCache[$type]['name'][$namekey])) {
					$data[$namekey] = $this->categoryCache[$type]['name'][$namekey];
					$foundInCache = true;
				}
				elseif(isset($this->categoryCache[$type]['name'][$code])) {
					$data[$namekey] = $this->categoryCache[$type]['name'][$code];
					$foundInCache = true;
				}
			}
	...
	function _getRelated($code){
		static $data=array();
		if(!isset($data[$code])){
			if(isset($this->relatedCache[$code])) {
				$data[$code] = $this->relatedCache[$code];
				return $data[$code];
			}
	...
	function _getAccess($access){
		...
		if(!isset($data[$access])){
			if(defined('HIKASHOP_WORDPRESS')) {
				...
				$rid = 1;
				$data[$access] = 0;
				foreach($wp_roles->role_names as $slug => $rname) {
					if(strtolower(translate_user_role($rname)) == strtolower($access) || strtolower($slug) == strtolower($access)) {
						$data[$access] = $rid;
						break;
					}
					$rid++;
	...
	function _handleSEO(&$products) {
		$config = hikashop_config();
		$kw_meta_desc_auto_fill = $config->get('auto_keywords_and_metadescription_filling',0);
		$alias_auto_fill = $config->get('alias_auto_fill',1);
		if($kw_meta_desc_auto_fill || $alias_auto_fill){
			...
			if(!empty($codes) || !empty($ids)){
				...
				$query = 'SELECT product_id, product_code, product_name, product_description, product_alias, product_meta_description, product_keywords FROM '.hikashop_table('product'). ' WHERE '.implode(' AND ',$filters);
				$this->db->setQuery($query);
				$already = $this->db->loadObjectList('product_id');
				$already_with_codes = array();
				if(!empty($already)) {
					foreach($already as $al) {
						$already_with_codes[$al->product_code] = $al;
					}
				}
				foreach($products as $product){

					$found_in_db = null;
					if(!empty($already) && !empty($product->product_id) && !empty($already[$product->product_id])) {
						$found_in_db = $already[$product->product_id];
					}elseif(!empty($already_with_codes) && !empty($product->product_code) && !empty($already_with_codes[$product->product_code])) {
						$found_in_db = $already_with_codes[$product->product_code];
					}
					if($kw_meta_desc_auto_fill) {
						if(empty($product->product_meta_description)) {
							if($found_in_db && empty($found_in_db->product_meta_description)) {
								if(empty($product->product_description) && !empty($found_in_db->product_description)) {
									$product->product_description = $found_in_db->product_description;
								}
								$seoHelper->autoFillMeta($product, "product");
							}
						}
						if(empty($product->product_keywords)) {
							if($found_in_db && empty($found_in_db->product_keywords)) {
								if(empty($product->product_description) && !empty($found_in_db->product_description)) {
									$product->product_description = $found_in_db->product_description;
								}
								$seoHelper->autoFillKeyword($product, "product");
							}
						}
					}
					if($alias_auto_fill && empty($product->product_alias)) {
							if($found_in_db && empty($found_in_db->product_alias)) {
								if(empty($product->product_name) && !empty($found_in_db->product_name)) {
									$product->product_name = $found_in_db->product_name;
								}
								$productClass->addAlias($product);
								if($config->get('sef_remove_id',0)){
									$int_at_the_beginning = (int)$product->alias;
	...
	function _insertProducts(&$products){
		...
		$this->_handleSEO($products);
		if($this->benchmark_import) {
			hikashop_writeToLog('Import benchmark: _handleSEO took ' . round((microtime(true) - $benchStart) * 1000, 2) . 'ms for ' . $productCount . ' products');
			$stepStart = microtime(true);
		}

		$this->_insertOneTypeOfProducts($products,'main');
		if($this->benchmark_import) {
			hikashop_writeToLog('Import benchmark: _insertOneTypeOfProducts(main) took ' . round((microtime(true) - $stepStart) * 1000, 2) . 'ms');
			$stepStart = microtime(true);
		}
		...
		if($this->benchmark_import) {
			hikashop_writeToLog('Import benchmark: _insertOneTypeOfProducts(variant) took ' . round((microtime(true) - $stepStart) * 1000, 2) . 'ms');
			$stepStart = microtime(true);
		}

		$this->_insertVariants($products);
		if($this->benchmark_import) {
			hikashop_writeToLog('Import benchmark: _insertVariants took ' . round((microtime(true) - $stepStart) * 1000, 2) . 'ms');
			$stepStart = microtime(true);
		}

		$this->_insertPrices($products);
		if($this->benchmark_import) {
			hikashop_writeToLog('Import benchmark: _insertPrices took ' . round((microtime(true) - $stepStart) * 1000, 2) . 'ms');
			$stepStart = microtime(true);
		}
		...
		if($this->benchmark_import) {
			hikashop_writeToLog('Import benchmark: _insertFiles took ' . round((microtime(true) - $stepStart) * 1000, 2) . 'ms');
			$stepStart = microtime(true);
		}

		$this->_insertCategories($products);
		if($this->benchmark_import) {
			hikashop_writeToLog('Import benchmark: _insertCategories took ' . round((microtime(true) - $stepStart) * 1000, 2) . 'ms');
			$stepStart = microtime(true);
		}

		$this->_prefetchRelatedProducts($products);
		$this->_insertRelated($products);
		$this->_insertRelated($products,'options');
		$this->_insertRelated($products,'bundle');
		if($this->benchmark_import) {
			hikashop_writeToLog('Import benchmark: _insertRelated (all types) took ' . round((microtime(true) - $stepStart) * 1000, 2) . 'ms');
			$stepStart = microtime(true);
		}

		$this->_insertTranslations($products);
		if($this->benchmark_import) {
			hikashop_writeToLog('Import benchmark: _insertTranslations took ' . round((microtime(true) - $stepStart) * 1000, 2) . 'ms');
			hikashop_writeToLog('Import benchmark: _insertProducts TOTAL took ' . round((microtime(true) - $benchStart) * 1000, 2) . 'ms for ' . $productCount . ' products');
		}
	...
	function _insertOneTypeOfProducts(&$products, $type = 'main') {
		...
		if(!empty($codes)){
			...
			$already = $this->db->loadObjectList('product_id');
			if(!empty($already)){
				$productCodeMap = array();
				foreach($products as $k => $product){
					if($product->product_type == $type) {
						$productCodeMap[$product->product_code] = $k;
					}
				}

				foreach($already as $code){
					$found = isset($productCodeMap[$code->product_code]) ? $productCodeMap[$code->product_code] : false;
					...
					if($found!==false){
						if($this->overwrite){
							if(!empty($products[$found]->product_type) && !empty($code->product_type) && $products[$found]->product_type==$code->product_type){
								...
								unset($productCodeMap[$code->product_code]);
							}
	...
	function getHtmlPage()
	{
		...
		return '<!DOCTYPE html>' .
			...
			'<style>'.
			'body { margin: 0; padding: 20px; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; background-color: #f4f6f8; }'.
			'.hk-import-card { max-width: 900px; margin: 0 auto; background: #fff; padding: 30px; box-shadow: 0 4px 20px rgba(0,0,0,0.05); border-radius: 8px; }'.
			'.hk-import-header { display: flex; align-items: center; justify-content: center; margin-bottom: 25px; }'.
			'.hk-import-title { margin: 0; color: #333; font-weight: 600; font-size: 24px; text-align: center; }'.
			'.hk-import-subtitle { color: #666; margin-top: 5px; font-size: 14px; text-align: center; }'.
			'</style>'.
			'</head><body onload="import'.ucfirst($this->importName).'()">'.
			'<div class="hk-import-card">'.
				'<div class="hk-import-header">'.
					'<h1 class="hk-import-title">'.
						'<span style="color:#297F93">' . JText::sprintf('PRODUCTS_FROM_X', $buff) . '</span>'.
					'</h1>'.
				'</div>'.
	...
	function getStartPage()
	{
		...
		<div style="text-align: center; padding: 40px 20px;">
			<div style="font-size: 18px; font-weight: 600; color: #297F93; margin-bottom: 20px;">Step 0: Initialization</div>
			<div style="background: #fff3cd; color: #856404; padding: 15px; border-radius: 6px; display: inline-block; margin-bottom: 30px; text-align: left;">
				<div style="font-weight: bold; margin-bottom: 5px; display: flex; align-items: center;">
					<span style="font-size: 18px; margin-right: 10px;">⚠️</span> Important
				</div>
				Make sure to backup your database before proceeding.
			</div>
			<div style="margin-top: 10px;">
				<a '.$this->linkstyle.' href="'.hikashop_completeLink('import&task=import&importfrom='.$buff.'&'.$this->token.'=1&import=1').'" style="background: #28a745; color: #fff; padding: 12px 24px; text-decoration: none; border-radius: 4px; font-weight: 500; font-size: 16px; margin: 0 10px;">'.JText::_('HIKA_NEXT').'</a>
				<a href="'.hikashop_completeLink('import&task=show').'" style="background: #6c757d; color: #fff; padding: 12px 24px; text-decoration: none; border-radius: 4px; font-weight: 500; font-size: 16px; margin: 0 10px;">'.JText::_('HIKA_BACK').'</a>
			</div>
		</div>';
	...
	function parseHeader($content) {
		...
		$columns = explode($separator, $header);
		$result = array();

		foreach($columns as $i => $col) {
			if(function_exists('mb_strtolower')) {
				$result[$i] = mb_strtolower(trim($col, '" '));
			} else {
				$result[$i] = strtolower(trim($col, '" '));
			}
		}
	...
	function getSupportedColumns() {
		...
		if(!HIKASHOP_J30) {
			$columnsTable = $this->db->getTableFields(hikashop_table('product'));
			$productColumns = array_keys(reset($columnsTable));
		} else {
			$productColumns = array_keys($this->db->getTableColumns(hikashop_table('product')));
		}

		foreach($productColumns as $col) {
			if($col !== 'product_id') {
				$key = 'HIKASHOP_' . strtoupper($col);
				$trans = JText::_($key);
				$columns[$col] = ($trans == $key) ? $col : $trans;
			}
		}
		...
		$null = null;
		if($fieldClass) {
			$customFields = $fieldClass->getFields('backend', $null, 'product');
			if (!empty($customFields)) {
				foreach ($customFields as $field) {
					$key = $field->field_namekey;
					$columns[$key] = JText::_($field->field_realname) . ' [' . JText::sprintf('CUSTOM_FIELDS_X',JText::_('PRODUCT')) . ']';
				}
			}
		}
		...
		if(isset($languages) && count($languages) > 0) {
			$translatable = array('product_name', 'product_description', 'product_meta_description', 'product_keywords', 'product_alias', 'product_page_title');

			if(!empty($customFields)) {
				foreach ($customFields as $field) {
					if(in_array($field->field_type, array('text', 'textarea', 'wysiwyg'))) {
						$translatable[] = $field->field_namekey;
					}
				}
			}
			...
			foreach($languages as $tag => $lang) {
				foreach($translatable as $col) {
					if(isset($columns[$col])) {
						$columns[$col . '|' . $tag] = $columns[$col] . ' [' . $tag . ']';
					}
				}
			}
		}
			...
			foreach($this->characteristics as $char) {
				if(empty($char->characteristic_parent_id)) {
					$key = function_exists('mb_strtolower') 
						? mb_strtolower(trim($char->characteristic_value, ' "'))
						: strtolower(trim($char->characteristic_value, ' "'));
					$columns[$key] = $char->characteristic_value . ' [' . JText::_('HIKA_CHARACTERISTIC') . ']';
				}
			}
		}
		...
		$app = JFactory::getApplication();
		JPluginHelper::importPlugin('hikashop');
		$app->triggerEvent('onGetImportSupportedColumns', array(&$columns));
	...
	function autoMatchColumns($csvColumns) {
		...
		$supportedKeys = array_keys($supportedColumns);
		$result = array();
		...
		$aliases = array(
			...
			'preis' => 'price_value',
			'precio' => 'price_value',
			'price_with_tax' => 'price_value_with_tax',
			'price_ttc' => 'price_value_with_tax',
			'price_incl_tax' => 'price_value_with_tax',

			'currency' => 'price_currency_id',
			'currency_code' => 'price_currency_id',
			...
			'manufacturer' => 'product_manufacturer_id',
			'brand' => 'product_manufacturer_id',
			'vendor' => 'product_vendor_id',
			'supplier' => 'product_vendor_id',
			'tax' => 'product_tax_id',
			'tax_class' => 'product_tax_id',
			'keywords' => 'product_keywords',
			'tags' => 'product_keywords',
			'url' => 'product_url',
			'link' => 'product_url',
			'image' => 'images',
			'picture' => 'images',
			...
			'active' => 'product_published',
			'status' => 'product_published',
			'parent' => 'product_parent_id',
			'parent_code' => 'product_parent_id',
			'parent_sku' => 'product_parent_id',
			'msrp' => 'product_msrp',
			'retail_price' => 'product_msrp',
			'rrp' => 'product_msrp',
		);

		foreach($csvColumns as $index => $csvCol) {
			$csvCol = trim($csvCol);
			$match = '';

			if(in_array($csvCol, $supportedKeys)) {
				$match = $csvCol;
			}

			if(empty($match) && isset($aliases[$csvCol])) {
				$match = $aliases[$csvCol];
			}

			if(empty($match)) {
				$normalized = preg_replace('/[\s\-]+/', '_', $csvCol);
				if(in_array($normalized, $supportedKeys)) {
					$match = $normalized;
				}
			}

			if(empty($match)) {
				$withoutPrefix = preg_replace('/^(product_|item_|article_)/', '', $csvCol);
				$withPrefix = 'product_' . $withoutPrefix;
				if(in_array($withPrefix, $supportedKeys)) {
					$match = $withPrefix;
				}
			}
