...
?><?php
if(defined('ABSPATH') && !defined('HIKASHOP_WORDPRESS')) {
	require_once(dirname(__DIR__, 2) . DIRECTORY_SEPARATOR . 'wordpress' . DIRECTORY_SEPARATOR . 'bootstrap.php');
}
...
if(!defined('HIKASHOP_J80')) define('HIKASHOP_J80',version_compare($jversion,'8.0.0','>=') ? true : false);
if(!defined('HIKASHOP_J90')) define('HIKASHOP_J90',version_compare($jversion,'9.0.0','>=') ? true : false);
...
define('HIKASHOP_PHP9',version_compare(PHP_VERSION,'9.0.0', '>=') ? true : false);
define('HIKASHOP_PHP10',version_compare(PHP_VERSION,'10.0.0', '>=') ? true : false);

define('HIKASHOP_VERSION', '6.5.0');
...
if(!function_exists('hikashop_getIP')) {
	function hikashop_getIP() {
		...
		$headers = array(
			...
			'HTTP_INCAP_CLIENT_IP', // Incapsula/Imperva
...
if(!function_exists('hikashop_getParentTemplate')) {
	function hikashop_getParentTemplate() {
		if(!HIKASHOP_J40)
			return null;
		$app = JFactory::getApplication();
...
if(!function_exists('hikashop_copy')) {
	function hikashop_copy($src, &$copied = null) {
		if(is_array($src)) {
			$array = array();
			foreach($src as $k => $v) {
				$array[$k] = hikashop_copy($v, $copied);
			}
			return $array;
		}
...
if(!function_exists('hikashop_isExternalUrlSafe')) {
	function hikashop_isExternalUrlSafe($url) {
		$parsed = parse_url($url);
		if(empty($parsed['host']) || !isset($parsed['scheme']) || !in_array(strtolower($parsed['scheme']), array('http', 'https')))
			return false;
...
if(!function_exists('hikashop_getLayout')) {
	function hikashop_getLayout($controller, $layout, $params, &$js, $backend = false) {
		...
		$hika_restore_app = null;
		if(!method_exists($app, 'getTemplate')) {
			try {
				$hika_restore_app = $app;
				$siteApp = JFactory::getContainer()->get(\Joomla\CMS\Application\SiteApplication::class);
				JFactory::$application = $siteApp;
				if(method_exists($siteApp, 'loadLanguage'))
		...
		if(method_exists($app, 'getDocument') && $app->getDocument() === null) {
			try {
				$docFactory = JFactory::getContainer()->get(\Joomla\CMS\Document\FactoryInterface::class);
				$stub_doc = $docFactory->createDocument('html');
				if(method_exists($app, 'loadDocument')) {
					$app->loadDocument($stub_doc);
				} elseif(property_exists($app, 'document')) {
					$app->document = $stub_doc;
				}
			} catch(Throwable $e) {
			}
		}
		...
		try {
			...
			if(!empty($parentTemplate)) {
				$folder	= JPATH_BASE.DS.'templates'.DS.$parentTemplate.DS.'html'.DS.HIKASHOP_COMPONENT.DS.$view->getName();
				$view->addTemplatePath($folder);
			}
			...
			if($hika_restore_app !== null)
				JFactory::$application = $hika_restore_app;
		}
	}
}

if(!function_exists('hikashop_setExplorer')) {
	function hikashop_setExplorer($task, $defaultId = 0, $popup = false, $type = '', $openByDefault = false) {
		$document = JFactory::getDocument();
		...
		$view->setLayout('default');
		ob_start();
		$view->display(null, $task, $defaultId, $popup, $type, $openByDefault);
		return ob_get_clean();
	}
}
...
if(!function_exists('hikashop_display')) {
	function hikashop_display($messages, $type = 'success', $return = false, $close = true) {
		...
		$app = JFactory::getApplication();
		$role = ($type == 'error' || $type == 'danger' || $type == 'warning') ? ' role="alert"' : '';
		if((hikashop_isClient('administrator') && (!HIKASHOP_BACK_RESPONSIVE || HIKASHOP_J40)) || (!hikashop_isClient('administrator') && !HIKASHOP_RESPONSIVE)) {
			$html = '<div id="hikashop_messages_'.$type.'" class="hikashop_messages hikashop_'.$type.'"'.$role.'><ul><li>'.implode('</li><li>',$display_messages).'</li></ul></div>';
		} else {
			$html = '<div class="alert alert-'.$type.' alert-block"'.$role.'>'.($close?'<button type="button" class="close" data-dismiss="alert" data-bs-dismiss="alert">×</button>':'').'<p>'.implode('</p><p>',$display_messages).'</p></div>';
		}
...
if(!function_exists('hikashop_frontendLink')) {
	function hikashop_frontendLink($link, $popup = false) {
		...
		if(!hikashop_isClient('administrator') && $config->get('activate_sef',0)){
			$link = ltrim(hikashop_route($link,false),'/');
		}
...
if(!function_exists('hikashop_completeLink')) {
	function hikashop_completeLink($link, $popup = false, $redirect = false, $js = false, $frontend = false) {
		...
		if($frontend && hikashop_isClient('administrator')){
			...
			$ret = hikashop_route($link,!$redirect);
		}
...
if(!function_exists('hikashop_contentLink')) {
	function hikashop_contentLink($link, $object, $popup = false, $redirect = false, $js = false, $frontend = false, $variant_id = 0) {
		$config = hikashop_config();
		...
		if($force_canonical){
			...
			if(!empty($object->product_canonical) || !empty($object->product_parent_id)){
				...
				} elseif(!empty($variant_id) && !empty($url)) {
					$main_alias = !empty($object->alias) ? $object->alias : (!empty($object->product_alias) ? $object->product_alias : '');
					if(!empty($main_alias) && !empty($object->product_id)) {
						$vid = (int)$variant_id;
						$rewritten = str_replace('/'.$object->product_id.'-'.$main_alias, '/'.$vid.'-'.$main_alias, $url);
						if($rewritten === $url) {
							$pattern = '~/' . preg_quote($main_alias, '~') . '(?=[/?#]|$)~';
							$replaced = preg_replace($pattern, '/'.$vid.'-'.$main_alias, $url, 1);
		...
		if(!empty($object->item_data_params)) {
			$separator = (strpos($url, '?') !== false) ? '&' : '?';
			$url .= $separator . $object->item_data_params;
		}
	...
	function hikashop_translate($name, $language_code = null, $latin_keys = true) {
		...
		if(!empty($language_code)) {
			...
			if($old_code != $language_code) {
				$savedState = hikashop_saveTranslationState();
				foreach($keys as $key) {
		...
		if(!empty($savedState)) {
			hikashop_restoreTranslationState($savedState);
		}
...
if(!function_exists('hikashop_footer')) {
	function hikashop_footer() {
		...
		<!-- version '.$config->get('level').' : '.$config->get('version').' [2607090929] -->';
			...
			if(!empty($app) && method_exists($app, 'getTemplate') && (hikashop_isClient('administrator') || defined('HIKASHOP_JOOMLA_LOADED'))) {
				try{
					...
					if(!is_file($override)) {
						$parentTemplate = hikashop_getParentTemplate();
						if(!empty($parentTemplate)) {
							$path = JPATH_THEMES.DS.$parentTemplate.DS.'html'.DS.'com_hikashop'.DS.'administrator'.DS;
							$override = str_replace($srcPath, $path, constant(strtoupper('HIKASHOP_'.$group))).$class.'.override.php';
						}
					}
				}catch (Exception $e) {
...
if(!function_exists('hikashop_getPluginController')) {
	function hikashop_getPluginController($ctrl) {
		...
		if(!is_dir($path)) {
			$alt_path = JPATH_PLUGINS.DS.'plg_'.$type.'_'.$name.DS;
			if(is_dir($alt_path))
				$path = $alt_path;
		}
...
if(!function_exists('hikashop_saveTranslationState')) {
	function hikashop_saveTranslationState() {
		$save = function() {
			return array(
		...
		$lang = JFactory::getLanguage();
		$saveCB = $save->bindTo($lang, 'JLanguage');
		return $saveCB();
	}
}

if(!function_exists('hikashop_restoreTranslationState')) {
	function hikashop_restoreTranslationState($state) {
		$restore = function($state) {
			$this->override = $state['override'];
			$this->strings = $state['strings'];
		};
		$lang = JFactory::getLanguage();
		$restoreCB = $restore->bindTo($lang, 'JLanguage');
		$restoreCB($state);
	}
}
...
if(!function_exists('hikashop_autoUpdateLanguages')) {
	function hikashop_autoUpdateLanguages() {
		$session = JFactory::getSession();
		if($session->get('lang_update_checked', false, 'hikashop'))
			return;
		$session->set('lang_update_checked', true, 'hikashop');
		...
		$langPath = hikashop_getLanguagePath(JPATH_ROOT);
		$isWP = defined('HIKASHOP_WORDPRESS');
		$languages = array();
		...
		if($isWP) {
			...
			$files = glob($langPath . DS . '*.com_hikashop.ini');
			if($files) {
				foreach($files as $file) {
					$basename = basename($file);
					$code = substr($basename, 0, strpos($basename, '.'));
					if(!empty($code) && $code !== 'en-GB')
						$languages[$code] = true;
				}
			}
		} else {
			jimport('joomla.filesystem.folder');
			$dirs = JFolder::folders($langPath);
			if($dirs) {
				foreach($dirs as $dir) {
					if($dir === 'en-GB' || $dir === 'overrides')
						continue;
					$file = $langPath . DS . $dir . DS . $dir . '.com_hikashop.ini';
					if(is_file($file))
						$languages[$dir] = true;
				}
			}
		...
		foreach(array_keys($languages) as $code) {
			$mainFile = hikashop_getLanguageFile($code);
			$needsUpdate = false;
			...
			if(!is_file($mainFile)) {
				...
				$langVersion = '';
				if($content && preg_match('/HIKA_LANG_VERSION\s*=\s*"([^"]+)"/', $content, $m))
					$langVersion = $m[1];
				if(empty($langVersion) || version_compare($langVersion, $currentVersion, '<'))
					$needsUpdate = true;
			}

			if($needsUpdate) {
				$url = HIKASHOP_UPDATEURL . 'languageload&raw=1&code=' . $code;
				$data = hikashop_autoUpdateLanguages_fetch($url);
				if(!empty($data)) {
					$dir = dirname($mainFile);
					if(!is_dir($dir))
						@mkdir($dir, 0755, true);
					@file_put_contents($mainFile, $data);
				}
			}

			if($isWP) {
				$wpFile = $langPath . DS . $code . '.wordpress.ini';
				if($needsUpdate || !is_file($wpFile)) {
					$url = HIKASHOP_UPDATEURL . 'languageload&raw=1&code=' . $code . '&type=wordpress';
					$data = hikashop_autoUpdateLanguages_fetch($url);
					if(!empty($data))
						@file_put_contents($wpFile, $data);
				}
			}
		}
	}
...
if(!function_exists('hikashop_autoUpdateLanguages_fetch')) {
	function hikashop_autoUpdateLanguages_fetch($url) {
	...
	function hikashop_loadJslib($name, $data = null) {
		...
		try {
			...
			switch($name) {
				...
				case 'dashboard_config':
					if($js) {
						...
						$doc->addScript(HIKASHOP_JS.'dashboard_config.js?v='.HIKASHOP_RESSOURCE_VERSION);
					}
					$ret = true;
					break;
				case 'sortable':
			...
			if($name != 'font-awesome') {
				hikashop_writeToLog('Error when loading library ' . $name . ' : ' . $e->getMessage());
...
if(!function_exists('hikashop_route')) {
	function hikashop_route($link, $xhtml = true) {
		$client = hikashop_isClient('administrator') ? 'administrator' : 'site';
		if(method_exists('JRoute', 'link')) {
			return JRoute::link($client, $link, $xhtml);
		}
...
if(!function_exists('hikashop_getLanguageFile')) {
	function hikashop_getLanguageFile($code) {
		if(defined('HIKASHOP_WORDPRESS'))
			return hikashop_getLanguagePath(JPATH_ROOT).DS.$code.'.com_hikashop.ini';
		return hikashop_getLanguagePath(JPATH_ROOT).DS.$code.DS.$code.'.com_hikashop.ini';
	}
}
...
if(!function_exists('hikashop_unserialize')) {
	function hikashop_unserialize($data) {
		...
		if(!preg_match('#^(?:[aOCsidbN][:;]|R:[0-9]+;)#', $data))
			return false;
	...
	function __construct($config = array(), $skip = false) {
		if(!empty($this->pluginCtrl) && is_array($this->pluginCtrl)) {
			...
			if(!is_dir($config['base_path'])) {
				$alt_path = JPATH_PLUGINS.DS.'plg_'.$this->pluginCtrl[0].'_'.$this->pluginCtrl[1].DS;
				if(is_dir($alt_path))
					$config['base_path'] = $alt_path;
			}
		}
		if(!$skip) {
...
window.saveorder = function ( n, task ) {
	...
	function loadTemplate($tpl = null){
		...
		if(!$active) {
			...
			if(hikashop_isClient('administrator') && $active < 2) {
				...
				if((hikaInput::get()->getInt('tp', 0) != 1) && (empty($_GET['tp']) || $_GET['tp']!=1))
					$showBorder = false;
			}
			...
			if(!$showBorder) {
				...
				$filename = $layout.(!empty($tpl)?'_':'').$tpl;

				$template = $app->getTemplate();
				$overrideTemplate = $template;
				$overrideSubDir = defined('HIKASHOP_WORDPRESS') ? 'hikashop' : ('html' . DS . HIKASHOP_COMPONENT);
				$overridePath = JPATH_THEMES . DS . $template . DS . $overrideSubDir . DS . $viewName . DS . $filename . '.php';
				if(!file_exists($overridePath)) {
					$parentTemplate = hikashop_getParentTemplate();
					if(!empty($parentTemplate)) {
						$parentPath = JPATH_THEMES . DS . $parentTemplate . DS . $overrideSubDir . DS . $viewName . DS . $filename . '.php';
						if(file_exists($parentPath)) {
							$overrideTemplate = $parentTemplate;
						}
					}
				}

				$editIcon = $this->getViewEditIcon($viewName, $filename, $overrideTemplate);

				$output = '<div class="hikashop_view_files_border"><div class="hikashop_view_files_title"><span>'.
					$viewName.' / '.$filename.'.php'.$editIcon.'</span></div><div class="hikashop_view_files_wrapper">'.
					parent::loadTemplate($tpl).
					'</div></div>';
			}
		}

		if(!empty($this->form_attribute_processing) && !empty($this->form_attribute_formId)) {
			$output = $this->addFormAttribute($output, $this->form_attribute_formId);
		}

		return $output;
	}

	function setFormProcessing($formName = '', $on = true) {
		$this->form_attribute_processing = $on && !empty($formName);
		$this->form_attribute_formId = $on ? $formName : '';
		self::$currentFormId = ($on && !empty($formName)) ? $formName : '';
	}

	function addFormAttribute($html, $formId = null) {
		if(empty($formId))
			$formId = !empty($this->currentFormName) ? $this->currentFormName : 'hikashop_product_form';
		return hikashop_add_form_attribute($html, $formId);
	}

	function getViewEditIcon($viewName, $filename, $template = null) {
		$client_id = hikashop_isClient('administrator') ? 1 : 0;

		if($template === null) {
			$app = JFactory::getApplication();
			$template = $app->getTemplate();

			$overridePath = JPATH_THEMES . DS . $template . DS . 'html' . DS . HIKASHOP_COMPONENT . DS . $viewName . DS . $filename . '.php';
			if(!file_exists($overridePath)) {
				$parentTemplate = hikashop_getParentTemplate();
				if(!empty($parentTemplate)) {
					$parentPath = JPATH_THEMES . DS . $parentTemplate . DS . 'html' . DS . HIKASHOP_COMPONENT . DS . $viewName . DS . $filename . '.php';
					if(file_exists($parentPath)) {
		...
		$viewId = $client_id . '|' . $template . '|component|' . HIKASHOP_COMPONENT . '|' . $viewName . '|' . $filename;

		$editUrl = JURI::root() . 'administrator/index.php?option=' . HIKASHOP_COMPONENT .
				   '&ctrl=view&task=edit&id=' . urlencode($viewId) . '&tmpl=component';

		$popupId = 'hk_view_edit_' . md5($viewId);

		hikashop_loadJsLib('vex');

		return ' <a href="' . htmlspecialchars($editUrl) . '" id="' . $popupId . '" ' .
			   'class="hikashop_view_edit_icon" ' .
			   'data-hk-popup="vex" data-vex=\'{"x":1300, "y":800}\' ' .
	...
	function loadHkLayout($layout, $params = array()) {
		...
		$parentTemplate = hikashop_getParentTemplate();
		if(!empty($parentTemplate)) {
			$parentFallback = JPATH_THEMES . '/' . $parentTemplate . '/html/' . $component . '/layouts';
		}

		$this->_path['template'] = array();
		$paths = array($layout_path, $fallback);
		if(!empty($parentFallback))
			$paths[] = $parentFallback;
		$this->_addPath('template', $paths);
...
class hikashopPlugin extends hikashopJoomlaPlugin {
	...
	function showPage($name = 'thanks') {
		...
		if(!file_exists($path)) {
			$parentTemplate = hikashop_getParentTemplate();
			if(!empty($parentTemplate)) {
				$path = JPATH_THEMES.DS.$parentTemplate.DS.$folder.DS.$this->name.'_'.$name.'.php';
			}
		}
		if(!file_exists($path)) {
		...
		if(!file_exists($path)) {
			$path = JPATH_PLUGINS .DS.'plg_'.$folder.'_'.$this->name.DS.$this->name.'_'.$name.'.php';
		}
	...
	function pluginConfiguration(&$elements) {
		...
		if(hikashop_isClient('administrator')) {
			$this->toolbar = array(
				'save',
				'apply',
				'cancel' => array('name' => 'link', 'icon' => 'cancel', 'alt' => JText::_('HIKA_CANCEL'), 'url' => hikashop_completeLink('plugins&plugin_type='.$this->type)),
			);
...
if(!defined('HIKASHOP_LIVE')) {
	$hika_live_root = rtrim(HikaURI::root(),'/').'/';
	if(!preg_match('#^https?://#i', $hika_live_root)) {
		$hika_live_site = '';
		try {
			$hika_app = JFactory::getApplication();
			if(method_exists($hika_app, 'getConfig'))
				$hika_live_site = (string) $hika_app->getConfig()->get('live_site', '');
			elseif(method_exists($hika_app, 'get'))
				$hika_live_site = (string) $hika_app->get('live_site', '');
			if(empty($hika_live_site) && !empty($_SERVER['HTTP_HOST'])) {
				$hika_scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
				$hika_live_site = $hika_scheme . '://' . $_SERVER['HTTP_HOST'] . (!empty($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : '/');
			}
		} catch(Exception $e) {
		}
		if(!empty($hika_live_site) && preg_match('#^https?://#i', $hika_live_site)) {
			$hika_live_root = rtrim($hika_live_site, '/') . '/';
			$hika_live_parts = parse_url($hika_live_root);
			if(!empty($hika_live_parts['host'])) {
				if(empty($_SERVER['HTTP_HOST']))
					$_SERVER['HTTP_HOST'] = $hika_live_parts['host'];
				if(empty($_SERVER['SERVER_PORT']))
					$_SERVER['SERVER_PORT'] = (string) (!empty($hika_live_parts['port']) ? $hika_live_parts['port'] : (!empty($hika_live_parts['scheme']) && $hika_live_parts['scheme'] === 'https' ? 443 : 80));
				if(empty($_SERVER['REQUEST_URI']))
					$_SERVER['REQUEST_URI'] = !empty($hika_live_parts['path']) ? $hika_live_parts['path'] : '/';
				if(empty($_SERVER['REQUEST_METHOD']))
					$_SERVER['REQUEST_METHOD'] = 'GET';
			}
			unset($hika_live_parts);
		}
		unset($hika_live_site, $hika_app, $hika_scheme);
	}
	define('HIKASHOP_LIVE', $hika_live_root);
	unset($hika_live_root);
}
if(!defined('HIKASHOP_ROOT'))
	define('HIKASHOP_ROOT', rtrim(JPATH_ROOT,DS).DS);
if(!defined('HIKASHOP_FRONT'))
	define('HIKASHOP_FRONT', rtrim(JPATH_SITE,DS).DS.'components'.DS.HIKASHOP_COMPONENT.DS);
if(!defined('HIKASHOP_BACK'))
	define('HIKASHOP_BACK', rtrim(JPATH_ADMINISTRATOR,DS).DS.'components'.DS.HIKASHOP_COMPONENT.DS);
if(!defined('HIKASHOP_HELPER'))
	define('HIKASHOP_HELPER', HIKASHOP_BACK.'helpers'.DS);
if(!defined('HIKASHOP_BUTTON'))
	define('HIKASHOP_BUTTON', HIKASHOP_BACK.'buttons');
if(!defined('HIKASHOP_CLASS'))
	define('HIKASHOP_CLASS', HIKASHOP_BACK.'classes'.DS);
if(!defined('HIKASHOP_INC'))
	define('HIKASHOP_INC', HIKASHOP_BACK.'inc'.DS);
if(!defined('HIKASHOP_VIEW'))
	define('HIKASHOP_VIEW', HIKASHOP_BACK.'views'.DS);
if(!defined('HIKASHOP_TYPE'))
	define('HIKASHOP_TYPE', HIKASHOP_BACK.'types'.DS);
if(!defined('HIKASHOP_MEDIA'))
	define('HIKASHOP_MEDIA', HIKASHOP_ROOT.'media'.DS.HIKASHOP_COMPONENT.DS);
if(!defined('HIKASHOP_DBPREFIX'))
...
if($css) {
	...
	if ($admin) {
		if (HIKASHOP_J50) {
			...
			$path = HIKASHOP_CSS.'backend_dark.css?t='.@filemtime(HIKASHOP_MEDIA.'css'.DS.'backend_dark.css');

			if($dark_mode=='2' && !defined('HIKASHOP_WORDPRESS')) {
			...
			if(defined('HIKASHOP_WORDPRESS')) {
				switch ($dark_mode) {
					case '1':
						...
						break;
					case '2':
						$doc->addCustomTag('<link rel="stylesheet" id="hk-dark-css" href="' . htmlspecialchars($path, ENT_QUOTES) . '" media="not all" />');
						$doc->addCustomTag('<script>(function(){if(document.body&&document.body.classList.contains("abovewp-dark-mode")){document.getElementById("hk-dark-css").media="all"}})()</script>');
						break;
					default:
						break;
				}
...
if(!function_exists('hikashop_add_form_attribute')) {
	function hikashop_add_form_attribute($html, $formId) {
		if(empty($html) || empty($formId))
			return $html;

		$parts = preg_split('#(<form[\s>].*?</form>)#is', $html, -1, PREG_SPLIT_DELIM_CAPTURE);
		if($parts === false)
			return $html;
		...
		foreach($parts as $i => $part) {
			...
			$result .= preg_replace_callback(
				...
				function($m) use ($formId) {
					$tag = $m[1];
					$attrs = $m[2];
					$selfClose = $m[3];
					if(preg_match('#\bform\s*=#i', $attrs))
						return $m[0];
					return '<' . $tag . $attrs . ' form="' . $formId . '"' . $selfClose . '>';
				},
				$part
			);
		}
