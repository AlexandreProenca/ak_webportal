...
class hikashopDatabaseHelper {
	...
	public function checkdb() {
		...
		$collation = 'utf8_bin';
		if(method_exists($this->db, 'getConnectionCollation')) {
			$dbCollation = $this->db->getConnectionCollation();
			if(stripos($dbCollation, 'utf8mb4') !== false)
				$collation = 'utf8mb4_bin';
		} elseif(method_exists($this->db, 'getCollation')) {
			$dbCollation = $this->db->getCollation();
			if(stripos($dbCollation, 'utf8mb4') !== false)
				$collation = 'utf8mb4_bin';
		}
		$query = 'SELECT DISTINCT order_status COLLATE '.$collation.' FROM `#__hikashop_order` WHERE order_type = \'sale\'';
		try{
			$this->db->setQuery($query);
		...
		if(defined('HIKASHOP_WORDPRESS')) {
			$query = 'INSERT IGNORE INTO `#__hikashop_user` (`user_email`,`user_cms_id`,`user_created`) SELECT `user_email`, `ID`,'.time().' FROM `#__users`';
		} else {
		...
		try{
			...
			if($result){
				$ret[] = array(
					'success',
					defined('HIKASHOP_WORDPRESS') ? 'WordPress users synchronized' : 'Joomla users synchronized'
				);
			}
		} catch(Exception $e) {
			$ret[] = array(
		...
		if(defined('HIKASHOP_WORDPRESS')) {
			$joinInfo = $this->_getUserEmailJoinCondition('hku.`user_email`', 'ju.`user_email`', 'user_email');
		} else {
			$joinInfo = $this->_getUserEmailJoinCondition('hku.`user_email`', 'ju.`email`', 'email');
		}
		$query = 'UPDATE `#__hikashop_user` AS hku LEFT JOIN `#__users` AS ju ON '.$joinInfo['sql'].' SET hku.`user_cms_id`=IFNULL(ju.`id`, 0) WHERE hku.`user_type`=\'registered\' AND (hku.`user_cms_id`!=ju.`id` OR ju.`id` IS NULL)';
		if($joinInfo['status'] == 'aligned') {
			$successMsg = 'User email column collation aligned with CMS users table ('.htmlspecialchars($joinInfo['hkCollation']).' -> '.htmlspecialchars($joinInfo['cmsCollation']).')';
			if(!empty($joinInfo['engineChanged'])) {
				$successMsg .= '. The hikashop_user table was also converted from MyISAM to InnoDB so the UNIQUE index on (user_email, user_type) would fit the new utf8mb4 byte budget.';
			}
			$ret[] = array('success', $successMsg);
		} elseif($joinInfo['status'] == 'convert') {
			$cmsColumn = defined('HIKASHOP_WORDPRESS') ? 'user_email' : 'email';
			$msg  = 'HikaShop and CMS users tables use different email column collations: ';
			$msg .= 'HikaShop user_email is on <strong>'.htmlspecialchars($joinInfo['hkCollation']).'</strong>, ';
			$msg .= 'CMS '.$cmsColumn.' is on <strong>'.htmlspecialchars($joinInfo['cmsCollation']).'</strong>. ';
			$msg .= 'The automatic ALTER TABLE alignment failed with the following MySQL error: <em>'.htmlspecialchars($joinInfo['error']).'</em>. ';
			$msg .= 'The user id synchronization still ran through a CONVERT() fallback but did not use the index. ';
			$msg .= 'To fix this permanently, ';
			if(stripos($joinInfo['error'], 'uplicate') !== false) {
				$msg .= 'some existing rows in hikashop_user have emails that only differ by case and would collide under the new case-insensitive collation. Identify them with <code>SELECT user_email, COUNT(*) FROM `#__hikashop_user` WHERE user_type=\'registered\' GROUP BY LOWER(user_email) HAVING COUNT(*)&gt;1;</code> and resolve them, then re-run: <code>'.htmlspecialchars($joinInfo['alter']).'</code>';
			} elseif(stripos($joinInfo['error'], 'denied') !== false || stripos($joinInfo['error'], 'privilege') !== false) {
				$msg .= 'grant the ALTER privilege to the database user or ask your hosting provider to run this query for you: <code>'.htmlspecialchars($joinInfo['alter']).'</code>';
			} else {
				$msg .= 'run this exact SQL query in PHPMyAdmin after addressing the underlying cause: <code>'.htmlspecialchars($joinInfo['alter']).'</code>';
			}
			$ret[] = array('warning', $msg);
		}
		$this->db->setQuery($query);
		try{
			$result = $this->db->execute();
	...
	public function _getUserEmailJoinCondition($hkExpr, $cmsExpr, $cmsColumnName) {
		$ret = array(
			'sql' => '',
			'status' => '',
			'hkCollation' => '',
		...
		$ret['hkCollation']  = $this->_probeColumnCollation('#__hikashop_user', 'user_email');
		$ret['cmsCollation'] = $this->_probeColumnCollation('#__users', $cmsColumnName);

		if(empty($ret['hkCollation']) || empty($ret['cmsCollation'])) {
			$ret['sql'] = 'CONVERT('.$hkExpr.' USING utf8mb4) = CONVERT('.$cmsExpr.' USING utf8mb4)';
			$ret['status'] = 'unknown';
			return $ret;
		}

		if($ret['hkCollation'] === $ret['cmsCollation']) {
			$ret['sql'] = $hkExpr.' = '.$cmsExpr;
			$ret['status'] = 'match';
			return $ret;
		}

		$charset = $this->_charsetForCollation($ret['cmsCollation']);
		if(!empty($charset)) {
			$alter = 'ALTER TABLE `#__hikashop_user` MODIFY `user_email` VARCHAR(255) CHARACTER SET '.$charset.' COLLATE '.$ret['cmsCollation'].' NOT NULL DEFAULT \'\'';
			$ret['alter'] = $alter;
			try {
			...
			if(stripos($ret['error'], 'too long') !== false) {
				try {
					$this->db->setQuery('ALTER TABLE `#__hikashop_user` ENGINE=InnoDB');
					$this->db->execute();
					$ret['engineChanged'] = true;
					try {
						$this->db->setQuery($alter);
						$this->db->execute();
						$ret['sql'] = $hkExpr.' = '.$cmsExpr;
						...
						$ret['error'] = $retryE->getMessage();
					}
				} catch(Exception $engineE) {
				}
			}
		}
	...
	protected function _probeColumnCollation($table, $column) {
		try {
			$this->db->setQuery('SHOW FULL COLUMNS FROM '.$this->db->quoteName($table).' LIKE '.$this->db->quote($column));
			$row = $this->db->loadAssoc();
			if(!empty($row) && !empty($row['Collation'])) {
	...
	protected function _charsetForCollation($collation) {
