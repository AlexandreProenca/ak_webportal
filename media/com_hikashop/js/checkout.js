...
var hikashopCheckout = {
	...
	onFormSubmit: function(el, btn) {
		...
		if(triggers !== false && triggers.length > 0) {
			console.log('[HikaShop Checkout] The checkoutFormSubmit event returned something. The form will not be submitted.');
	...
	checkScroll: function (fullReload) {
		...
		if(!els || !els.length){
			els = document.getElementsByClassName("alert-error");
		}
		// if none, leave the scroll as it is
		if(!els || !els.length)
			return;
