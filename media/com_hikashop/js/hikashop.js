...
var Oby = {
	...
	getFormData: function(target, encoded) {
		...
		// Collect orphaned inputs associated with the form via the HTML form attribute
		// (these are outside the form element in the DOM but linked to it by form="formId")
		if(target && target.id) {
			var scope = target.closest('.hikashop_product_page') || target.parentNode;
			var orphanSelector = 'input[form="'+target.id+'"], select[form="'+target.id+'"], textarea[form="'+target.id+'"]';
			var orphans = scope.querySelectorAll(orphanSelector);
			for(var oi = 0; oi < orphans.length; oi++) {
				if(!orphans[oi].name || orphans[oi].disabled)
					continue;
				var n = orphans[oi].name;
				if(ret.hasOwnProperty(n))
					continue;
				var etype = orphans[oi].tagName.toLowerCase() == 'input' ? orphans[oi].type.toLowerCase() : '';
				if(etype == 'file' || etype == 'submit')
					continue;
				if((etype == 'radio' || etype == 'checkbox') && !orphans[oi].checked)
					continue;
				ret[n] = orphans[oi].value;
			}
		}
		if(encoded)
...
var hikashop = {
	...
	openBox_vex: function(elem, url) {
		...
		if(settings.x && settings.y && href) {
			...
			var origAfterOpen = settings.afterOpen || null;
			settings.afterOpen = function(context) {
				if(origAfterOpen) origAfterOpen(context);
				context.width(settings.x + 'px');
				hikashop.vexA11y(context, triggerEl);
			};
		}
		...
		// Fix for CSS filter on body (e.g. Joomla a11y_contrast) breaking position:fixed.
		// When a CSS filter is set on the body, position:fixed elements are positioned
		// relative to the body instead of the viewport. We compensate by converting
		// the popup to position:absolute and setting its top to the current scroll position.
		var bodyStyle = window.getComputedStyle(document.body);
		if(bodyStyle.filter && bodyStyle.filter !== 'none') {
			var vexEls = document.querySelectorAll('.vex');
			for(var vi = 0; vi < vexEls.length; vi++) {
				vexEls[vi].style.position = 'absolute';
				vexEls[vi].style.top = window.scrollY + 'px';
				vexEls[vi].style.height = window.innerHeight + 'px';
				// Also fix the overlay inside
				var overlay = vexEls[vi].querySelector('.vex-overlay');
				if(overlay) {
	...
	vexA11y: function(context, triggerEl) {
		var el = context.get(0) || context[0];
		if(!el) return;
		var content = el.querySelector('.vex-content');
		if(!content) return;
		// ARIA attributes
		content.setAttribute('role', 'dialog');
		content.setAttribute('aria-modal', 'true');
		var title = triggerEl ? (triggerEl.getAttribute('title') || triggerEl.textContent || '') : '';
		if(title) content.setAttribute('aria-label', title.trim().substring(0, 100));
		// Make close button accessible
		var closeBtn = content.querySelector('.vex-close');
		if(closeBtn) {
			closeBtn.setAttribute('role', 'button');
			closeBtn.setAttribute('tabindex', '0');
			closeBtn.setAttribute('aria-label', window.hikashop.translations['HIKA_CLOSE'] || 'Close');
		...
		content.addEventListener('keydown', function(e) {
			...
			var focusable = content.querySelectorAll('a[href], button, textarea, input, select, iframe, [tabindex]:not([tabindex="-1"])');
			if(!focusable.length) return;
			var first = focusable[0], last = focusable[focusable.length - 1];
			if(e.shiftKey) {
				if(document.activeElement === first || document.activeElement === content) { e.preventDefault(); last.focus(); }
			} else {
				if(document.activeElement === last) { e.preventDefault(); first.focus(); }
			}
		});
		// Focus the first focusable element
		var firstFocusable = content.querySelector('iframe, button, [tabindex]:not([tabindex="-1"]), a[href], input, select, textarea');
	...
	refreshOneArea: function(refreshUrl, currentArea, el, refreshAreas, resp) {
		...
		o.xRequest(refreshUrl, {mode:'GET'}, function (xhr2) {
			...
			// When SEF or a proxy strips tmpl=raw, the response is the whole page with
			// every module instead of the raw block. Detect that so we inject only the
			// refreshed block and run only its own scripts: re-running the whole page's
			// template and third-party scripts after load blanks the page.
			var isFullPage = /<!DOCTYPE|<html[\s>]|<body[\s>]/i.test(response);

			var body = /<body.*?>([\s\S]*)<\/body>/i.exec(response);
			body = body ? body[1] : response;

			if(isFullPage) {
				// Parse detached (innerHTML never executes scripts), then keep only the
				// scripts that belong to the block being refreshed.
				div.innerHTML = body;
				var block = (currentArea && currentArea.id) ? div.querySelector('#' + currentArea.id) : null;
				if(!block)
					block = div.querySelector('.filter_refresh_div');
				if(block) {
					var blockScripts = block.querySelectorAll('script');
					for(var s = 0; s < blockScripts.length; s++) {
						var sn = blockScripts[s], stype = sn.getAttribute('type') || '';
						if(!sn.getAttribute('src') && stype.indexOf('application/json') == -1 && stype.indexOf('application/ld+json') == -1)
							scripts.push(sn.textContent);
						sn.parentNode.removeChild(sn);
					}
				}
			} else {
				// Raw block: strip and run every script it carries, as before.
				var text = body.replace(/<script[^>]*>([\s\S]*?)<\/script>/gi, function(all, code){
				...
				window.hikashop.updateElem(div, text);
			}

			var newElem = (currentArea && currentArea.id) ? div.querySelector('#' + currentArea.id) : null;
			if(!newElem)
				newElem = div.querySelector('.filter_refresh_div');
			...
			setTimeout(function(){
				...
				if( scripts.length ) {
					scripts.forEach(function(code){
						var injectedCode = code.replaceAll('let jch_', 'jch_').replaceAll('const jch', 'window.hikashop.jch');
						injectedCode = injectedCode.replace(/(^|[;{}\n)])([ \t]*)(const|let|class)([ \t]+)([A-Za-z_$][\w$]*)/g, function(m, pre, ws, kw, sp, name) {
							var declared = false;
							try { (0, eval)(name); declared = true; }
							catch (err) { declared = !(err instanceof ReferenceError); }
							if(!declared)
								return m;
							return pre + ws + 'window.hikashop.' + name + (kw === 'class' ? ' = class' : '');
						});
						// Run each inline script on its own so one that throws (a third-party
						// or template script) cannot abort the ones after it, which used to
						// leave the price cursor and other widgets un-initialised.
						var script = d.createElement('script');
						script.setAttribute('type', 'text/javascript');
						script.text = injectedCode;
						// A runtime error in an injected inline script is reported to window.onerror
						// rather than thrown back to appendChild, so catch it there and log it
						// cleanly instead of as an uncaught error. The listener is active only for
						// this single script's synchronous execution.
	...
	addToCart: function(el, type, container, data) {
		...
		if(typeof data !== 'undefined') {
			...
			if(window.FormData)
				...
				data = o.getFormData(container);
			// Collect orphaned inputs associated with the form via the form attribute
			// (browsers may not include dynamically inserted inputs with form= in FormData)
			if(window.FormData && data && container.id) {
				var scope = container.closest('.hikashop_product_page') || container.parentNode;
				var orphans = scope.querySelectorAll('input[form="'+container.id+'"], select[form="'+container.id+'"], textarea[form="'+container.id+'"]');
				for(var oi = 0; oi < orphans.length; oi++) {
					var orphan = orphans[oi];
					if(!orphan.name || orphan.disabled || data.has(orphan.name))
						continue;
					var etype = orphan.tagName.toLowerCase() == 'input' ? orphan.type.toLowerCase() : '';
					if(etype == 'file' || etype == 'submit')
						continue;
					if((etype == 'radio' || etype == 'checkbox') && !orphan.checked)
						continue;
					data.append(orphan.name, orphan.value);
				}
			}
			if(extraContainer) {
	...
	toggleOverlayBlock: function(el, type, state) {
		...
		if(type != 'hover' && type != 'toggle' && type != 'drawer')
			type = 'click';
		if(type == 'drawer') {
			var target = d.getElementById(el.getAttribute('aria-controls'));
			if(!target) return false;
			if(target.classList.contains(hkDrawer.settings['activeClass']))
				hkDrawer.closeDrawer(target);
			else
		...
		if(type == 'hover') {
			...
			if(jQuery) {
				jQuery(el).on('mouseleave', el.toggleFunctionHover);
			} else {
	...
	updateFieldError: function(el) {
		if(!el || !el.id) return;
		var errorSpan = document.getElementById(el.id + '_error');
		if(!errorSpan) return;
		var isInvalid = el.classList.contains('invalid') || (el.validity && !el.validity.valid);
		if(isInvalid) {
			var msg = el.getAttribute('data-validation-text') || errorSpan.getAttribute('data-validation-text') || '';
