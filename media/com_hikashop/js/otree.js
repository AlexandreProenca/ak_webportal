/*
	...
	Copyright (C) 2010-2026  Jerome GLATIGNY
...
(function () {
	...
	var oTree = function (id, conf, callbackFct, data, render) {
		...
		t.config = {
			...
			checkbox_leaves: conf.checkbox_leaves || false,
	...
	oTree.prototype = {
		...
		rnode: function (node) {
			...
			// Checkbox mode. With the "checkbox_leaves" option activated, only allow checkboxes for products and variants (not for categories)
			if (t.config.checkbox && !node.noselection && (!t.config.checkbox_leaves || ((node.state == 0 || node.state == 5) && node.id != 0 && node.pid > 0))) {
		...
		sel: function (id) {
			...
			if (t.config.checkbox && (!t.config.checkbox_leaves || d.getElementById(t.id + '_c' + id))) {
				t.chk(cn, -1);
			}
		 ...
		 *
		 */
		autoCheck: function (nId) {
			var t = this, d = document;
			var node = t.get(nId);
			if (!node) return;
			if (node.value) {
				if (d.getElementById(t.id + "-" + node.value)) {
					node.checked = true;
				}
			}
			if (node.children && node.children.length > 0) {
				for (var k = 0; k < node.children.length; k++) {
					t.autoCheck(node.children[k]);
				}
			}
		},
		/**
	...
	oNamebox.treeCbFct = function (t, url, keyword, tree, node, ev) {
		...
		o.xRequest(url.replace(keyword, node.value), null,
			function (xhr, params) {
				...
				if (tree.config.checkbox && tree.config.checkbox_leaves) {
					// preselect checkboxes based on values already selected
					tree.autoCheck(node.id);
				}
				tree.update(node);
	...
	oNamebox.prototype = {
		...
		init: function () {
			...
			if (t.mode == 'list') {
				...
				if (t._conf.checkbox !== undefined) options.checkbox = t._conf.checkbox;
				if (t._conf.tricheckbox !== undefined) options.tricheckbox = t._conf.tricheckbox;
				if (t._conf.checkbox_leaves !== undefined) options.checkbox_leaves = t._conf.checkbox_leaves;
				...
				t.content.callbackSelection = function (tree, id) {
					...
					// in checkbox leaves mode, do not select nodes that are not leaves
					if (tree.config.checkbox && tree.config.checkbox_leaves && ((node.state == 0 || node.state == 5) && node.id != 0)) {
						return;
					}
		...
		validateCheckboxes: function () {
			var t = this;
			if (t.content && t.content.lNodes) {
				for (var i = 0; i < t.content.lNodes.length; i++) {
					var node = t.content.lNodes[i];
					if (node && node.checked === true && !node.noselection) {
						t.set(node.name, node.value);
					}
				}
			}
