...
hikaVote.vote = function(val, from, skip=true){
	...
	if(m != null) {
		...
		if(!infos)
			infos = d.getElementById("hikashop_vote_rating_id");
		if(!infos)
			return;
	...
	// When the page was loaded with an order_token (guest customer reviewing
	// a product from their order), the form template emits two hidden inputs
	// that we forward to the submit endpoint so the server can re-validate
	// the token and accept the vote / comment as verified-purchaser even
	// without a Joomla account.
	var order_token = "", order_id = 0;
	if(d.getElementById("order_token")) {
		order_token = d.getElementById("order_token").value;
	}
	if(d.getElementById("order_id")) {
		order_id = d.getElementById("order_id").value;
	}

	var data = "vote_type=" + encodeURIComponent(type) + "&hikashop_vote_type=vote" +
	...
	if(order_token) {
		data += "&order_token=" + encodeURIComponent(order_token) +
			"&order_id=" + parseInt(order_id);
	}
	if(hikaVote.options.token)
		data += "&" + encodeURIComponent(hikaVote.options.token) + "=1";
	window.Oby.xRequest(hikaVote.options.urls.save, {mode: "POST", data: data}, function(xhr) {
		var response = window.Oby.evalJSON(xhr.response);
		if(response.error) {
		...
		if(comment_task) {
			...
			// Let layouts that conditionally suppress the comment-list area
			// (e.g. show_tabular.php hides the Comments tab on a product
			// with no comments yet) inject it on the fly before we try to
			// find it. Handlers receive {ref_id, type}.
			if(window.Oby && window.Oby.fireAjax)
				window.Oby.fireAjax('vote.beforeListRefresh', {ref_id: ref_id, type: type});
			...
			if(!section) {
				// No handler created the container; the comment was already
				// saved server-side, so reload the page to render the now-
				// populated listing instead of silently doing nothing.
				window.location.reload();
				return;
			}

			var data = "data_id="+parseInt(ref_id);
...
hikaVote.useful = function(vote_id, val) {
	...
	var data = "hikashop_vote_type=useful&value=" + parseInt(val) + "&hikashop_vote_id=" + parseInt(vote_id) + "&vote_type=" + encodeURIComponent(type);
	if(hikaVote.options.token)
		data += "&" + encodeURIComponent(hikaVote.options.token) + "=1";
	window.Oby.xRequest(hikaVote.options.urls.save, {mode: "POST", data: data}, function(xhr) {
		var response = window.Oby.evalJSON(xhr.response);
...
hikaVote.prototype = {
	...
	init: function(el, opt, cb) {
		...
		if(!t.options.showSelectBox && t.selectBox && t.selectBox.nodeName.toLowerCase() == 'select' && typeof(jQuery) != 'undefined' && jQuery().chosen) {
			setTimeout(function(){
				var id = t.selectBox.getAttribute('id') + '_chzn';
	...
	click: function(e) {
		...
		if(hikashop_send_vote){
			...
			var from_id = 'hikashop_vote_rating_id';
			var rating_value = 0;
			if(!el) {
