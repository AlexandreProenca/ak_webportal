...
!function ($) {
	...
	HKDropdown.prototype = {
		...
		keydown: function (e) {
			...
			if (!/(13|35|36|38|40|27)/.test(e.keyCode)) return;
			...
			if (e.keyCode == 35) index = $items.length - 1; // end
			if (e.keyCode == 13) { // enter — activate focused item
				$items.filter(':focus').trigger('click');
				return;
			}
			if (!~index) index = 0;
