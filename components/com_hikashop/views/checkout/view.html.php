...
class CheckoutViewCheckout extends CheckoutViewCheckoutLegacy {
	...
	public function termsandconditions() {
		...
		if(defined('HIKASHOP_WORDPRESS')) {
			$db->setQuery('SELECT post_content, post_excerpt FROM #__posts WHERE ID = ' . (int)$terms_article);
			$data = $db->loadObject();
			$article = '';
			if(is_object($data)) {
				$article = $data->post_excerpt . $data->post_content;
				if(function_exists('do_blocks'))
					$article = do_blocks($article);
				if(function_exists('do_shortcode'))
					$article = do_shortcode($article);
				if(function_exists('wpautop'))
					$article = wpautop($article);
			}
		...
		if(is_object($data) && !in_array($data->language, array('all', JFactory::getLanguage()->getTag())) ) {
	...
	public function show() {
		...
		foreach($this->workflow['steps'][$this->workflow_step]['content'] as $k => &$content) {
			...
			if(!empty($ctrl) && method_exists($ctrl, 'display')) {
