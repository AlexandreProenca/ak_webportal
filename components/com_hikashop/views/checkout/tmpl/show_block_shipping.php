...
<?php
if(!empty($this->options['dynamic_address_refresh'])) {
	$previewUrl = hikashop_completeLink('checkout&task=showblock&blocktask=shipping'.$this->cartIdParam.'&Itemid='.$this->itemid, 'ajax', false, true);
?>
<script type="text/javascript">
(function(){
	var shStep = <?php echo (int)$this->step; ?>,
		shPos = <?php echo (int)$this->module_position; ?>,
		debounceTimer = null,
		waitingForState = false,
		previewUrl = "<?php echo $previewUrl; ?>";

	var watchedFields = ['address_country', 'address_state', 'address_city', 'address_post_code', 'address_street'];

	function isShippingForm(addressBlock) {
		var editForm = addressBlock.querySelector('.hikashop_checkout_checkout_address_block');
		if(!editForm) return false;

		var addrType = editForm.getAttribute('data-address-type');

		if(!addrType) {
			var typeInput = editForm.querySelector('input[name^="data[address_type_"]');
			if(typeInput) addrType = typeInput.value;
		}

		if(!addrType) return false;

		if(addrType === 'shipping' || addrType === 'both') return true;

		if(addrType === 'billing') {
			var sameCheckbox = editForm.querySelector('input[name^="data[address_bothtypes_"]');
			if(sameCheckbox && sameCheckbox.checked) return true;
		}

		return false;
	}

	function findShippingAddressForm() {
		var form = document.getElementById('hikashop_checkout_form');
		if(!form) return null;

		var addressBlocks = form.querySelectorAll('.hikashop_checkout_address[data-checkout-step="' + shStep + '"]');
		for(var i = 0; i < addressBlocks.length; i++) {
			if(isShippingForm(addressBlocks[i]))
				return addressBlocks[i];
		}
		return null;
	}

	function collectAddressData(addressBlock) {
		var editForm = addressBlock.querySelector('.hikashop_checkout_checkout_address_block');
		if(!editForm) return null;

		var data = {}, hasData = false;
		for(var i = 0; i < watchedFields.length; i++) {
			var field = editForm.querySelector('[name*="[' + watchedFields[i] + ']"]');
			if(field && field.value) {
				data[watchedFields[i]] = field.value;
				hasData = true;
			}
		}
	...
	function triggerShippingRefresh() {
		var addressBlock = findShippingAddressForm();
		if(!addressBlock) return;

		var data = collectAddressData(addressBlock);
		if(!data) return;

		var el = document.getElementById('hikashop_checkout_shipping_' + shStep + '_' + shPos);
		if(!el) return;

		window.checkout.setLoading(el, true);

		var url = previewUrl,
			sep = (url.indexOf('?') >= 0) ? '&' : '?';
		url += sep + 'cid=' + shStep + '&blockpos=' + shPos;
		for(var key in data) {
			if(data.hasOwnProperty(key))
				url += '&preview_address[' + encodeURIComponent(key) + ']=' + encodeURIComponent(data[key]);
		}

		window.Oby.xRequest(url, {}, function(x, p) {
			el = document.getElementById('hikashop_checkout_shipping_' + shStep + '_' + shPos);
			if(!el) return;
			window.checkout.setLoading(el, false);
			window.hikashop.updateElem(el, x.responseText);
			window.checkout.handleEnter('shipping', shStep, shPos);
		});
	}

	function debouncedRefresh(delay) {
		if(debounceTimer) clearTimeout(debounceTimer);
		debounceTimer = setTimeout(triggerShippingRefresh, delay);
	}

	function isWatchedField(target) {
		if(!target.name) return false;

		var addressBlock = target.closest('.hikashop_checkout_address[data-checkout-step="' + shStep + '"]');
		if(!addressBlock) return false;

		if(!target.closest('.hikashop_checkout_checkout_address_block')) return false;

		for(var i = 0; i < watchedFields.length; i++) {
			if(target.name.indexOf('[' + watchedFields[i] + ']') !== -1)
				return isShippingForm(addressBlock);
		}
		return false;
	}
	...
	if(form) {
		form.addEventListener('change', function(e) {
			...
			if(target.name && target.name.indexOf('address_bothtypes_') !== -1) {
				var addressBlock = target.closest('.hikashop_checkout_address[data-checkout-step="' + shStep + '"]');
				if(!addressBlock) return;
				if(target.checked) {
					...
					if(debounceTimer) clearTimeout(debounceTimer);
					window.checkout.refreshShipping(shStep, shPos);
				}
				return;
			}

			if(!isWatchedField(target)) return;

			if(target.name.indexOf('[address_country]') !== -1) {
				waitingForState = true;
				if(debounceTimer) clearTimeout(debounceTimer);
				debounceTimer = setTimeout(function() {
					waitingForState = false;
					triggerShippingRefresh();
				}, 2000);
				return;
			}

			if(target.name.indexOf('[address_state]') !== -1 && waitingForState)
				return;
		...
		form.addEventListener('input', function(e) {
			...
			if(!isWatchedField(e.target)) return;
