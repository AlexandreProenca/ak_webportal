...
?>
			<fieldset class="hika_address_field hikashop_checkout_checkout_address_block"<?php
		$edit_address_type = '';
		if($this->options['edit_address'] === true && !empty($this->options['new_address_type'])) {
			$edit_address_type = $this->options['new_address_type'];
		} elseif($this->options['edit_address'] !== true && !empty($this->edit_address->address_type)) {
			$edit_address_type = $this->edit_address->address_type;
		}
		if(!empty($edit_address_type)) echo ' data-address-type="' . htmlspecialchars($edit_address_type) . '"';
?>>
<?php
		$fields_type = '';
...
<?php
		...
		if(!empty($fields_type))
			$fields_type .= '_';
		$after = array();
		$this->fields = $this->cart_addresses[$fields_type.'fields'];
		$this->inputPrefix = 'data[address_'.$this->step . '_' . $this->module_position.']';
		$this->idPrefix = 'hikashop_checkout_address_' . $this->step . '_' . $this->module_position;
		$this->onChangeJs = "window.hikashop.toggleField(this,'{FIELD}','address_".$this->step.'_'.$this->module_position."',0,'hikashop_checkout_');";
		$this->address = $this->edit_address;

		echo $this->loadHkLayout('address_form_layout');

		if(!empty($this->extraData[$this->module_position]) && !empty($this->extraData[$this->module_position]->address_middle)) { echo implode("\r\n", $this->extraData[$this->module_position]->address_middle); }
?>
