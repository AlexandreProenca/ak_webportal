...
?><?php if(empty($this->print_cart)) {
	$hasCpanelSidebar = !empty($this->cpanel_active_page);
	if($hasCpanelSidebar) {
		$js = '';
		$sidebarParams = new HikaParameter('');
		$sidebarParams->set('active_page', $this->cpanel_active_page);
	?>
<div class="hikashop_cpanel_main_interface">
<div class="hk-row-fluid hikashop_dashboard" id="hikashop_dashboard">
<?php echo hikashop_getLayout('user', 'cpanel_sidebar', $sidebarParams, $js); ?>
...
<?php if($this->checkbox_column) { ?>
			<th scope="col" style="width:1%" data-title="<?php echo JText::_('SELECT_ALL'); ?>" ><input type="checkbox" id="hikashop_cart_selectall" onchange="window.hikashop.checkAll(this);" /><label for="hikashop_cart_selectall" class="element-invisible"><?php echo JText::_('SELECT_ALL'); ?></label></th>
<?php } ?>
<!-- EO SELECT ALL HEADER -->
<!-- PRODUCT NAME HEADER -->
			<th scope="col" class="hikashop_cart_name_title title"><?php
...
<?php
	if(hikashop_level(1) && !empty($this->productFields)) {
		foreach($this->productFields as $fieldname => $field) {
			echo '<th scope="col" class="hikashop_cart_product_'.$fieldname.' title">'.$this->fieldsClass->trans($field->field_realname).'</th>';
		}
	}
?>
<!-- EO CUSTOM PRODUCT FIELDS HEADER -->
<!-- STATUS HEADER -->
			<th scope="col" class="hikashop_cart_status_title title"><?php
				echo JText::_('HIKASHOP_CHECKOUT_STATUS');
			?></th>
<!-- EO STATUS HEADER -->
<!-- UNIT PRICE HEADER -->
			<th scope="col" class="hikashop_cart_price_title title"><?php
				echo JText::_('CART_PRODUCT_UNIT_PRICE');
			?></th>	
<!-- EO UNIT PRICE HEADER -->
<!-- QUANTITY HEADER -->
			<th scope="col" class="hikashop_cart_quantity_title title"><?php
		...
		if($group) {
			foreach($this->cart->products as $opt_k => $opt_product) {
				if($opt_product->cart_product_option_parent_id != $product->cart_product_id)
					continue;
				$option_name = $opt_product->product_name;
				if($opt_product->cart_product_quantity != $product->cart_product_quantity) {
					$option_name .= ' x'.round($opt_product->cart_product_quantity / $product->cart_product_quantity, 2);
				}
				$html .= '<p class="hikashop_cart_option_name">' . $option_name . '</p>';
