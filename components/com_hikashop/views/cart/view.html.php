...
class CartViewCart extends HikaShopView {
	...
	public function show() {
		...
		if(!(int)$config->get('cpanel_legacy', false)) {
			$directCartMenu = !empty($menu) && method_exists($menu, 'getParams') && in_array($menu->link, array(
				'index.php?option=com_hikashop&view=cart&layout=show',
				'index.php?option=com_hikashop&view=cart&layout=listing',
			));
			if(!$directCartMenu)
				$this->cpanel_active_page = (!empty($cart) && $cart->cart_type == 'wishlist') ? 'wishlist' : 'cart';
		}
	}
	...
	public function listing() {
		...
		if($new_button) {
			$this->toolbar['new'] = array(
				...
				'url' => hikashop_completeLink('cart&task=add&cart_type='.$cart_type.'&Itemid='.$Itemid.'&'.hikashop_getFormToken().'=1'),
		...
		if(!empty($menu) && method_exists($menu, 'getParams') && $menu->link == 'index.php?option=com_hikashop&view=cart&layout=listing') {
			...
			if(!(int)$config->get('cpanel_legacy', false))
				$this->cpanel_active_page = ($cart_type == 'wishlist') ? 'wishlist' : 'cart';
		}
