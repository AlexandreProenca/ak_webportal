...
class VoteViewvote extends HikaShopView {
	...
	function form(){
		...
		$page_order_id = 0;
		$page_order_token = '';
		if($config->get('access_vote',0)=='buyed' && $type_item == 'product'){
			$voteClass = hikashop_get('class.vote');
			$hikashop_vote_user_id = hikashop_loadUser();

			if(!empty($hikashop_vote_user_id)){
				if($voteClass->hasBought($hikashop_vote_ref_id, $hikashop_vote_user_id))
					$purchased = 1;
			}

			if($purchased == 0){
				$page_order_token = (string)hikaInput::get()->getString('order_token', '');
				$page_order_id = (int)hikaInput::get()->getInt('order_id', 0);
				if(!empty($page_order_token)){
					$token_user_id = $voteClass->hasBoughtViaToken($hikashop_vote_ref_id, $page_order_token, $page_order_id);
					if(!empty($token_user_id)){
						$purchased = 1;
						$orderClass = hikashop_get('class.order');
						$orderData = $orderClass->loadFullOrder($page_order_id, true, false);
						if(!empty($orderData)){
							if(!empty($orderData->billing_address)){
								$prefill_pseudo = trim(@$orderData->billing_address->address_firstname . ' ' . @$orderData->billing_address->address_lastname);
							}
							if(!empty($orderData->customer->user_email)){
								$prefill_email = $orderData->customer->user_email;
							}
						}
					}
				}
		...
		$row->order_id = $page_order_id;
		$row->order_token = $page_order_token;
		$row->prefill_pseudo = $prefill_pseudo;
		$row->prefill_email = $prefill_email;
