...
?>
	<span style="float: right;" class="hikashop_sort_listing_comment">
		<label for="hikashop_sort_comment" class="element-invisible"><?php echo JText::_('HIKASHOP_LISTING_COMMENT');?></label>
		<select name="sort_comment" id="hikashop_sort_comment" onchange="refreshCommentSort(this.value); return false;">
...
<?php } ?>
		...
		if($nb_star_vote != 0) {
			?><span class="element-invisible"><?php echo JText::sprintf('HIKA_VOTE_STAR', $nb_star_vote, $nb_star_config); ?></span><?php
			for($k = 0; $k < $nb_star_vote; $k++) {
				?><span class="hika_comment_listing_full_stars hk-rate-star state-full" aria-hidden="true"></span><?php
			}
			$nb_star_empty = $nb_star_config - $nb_star_vote;
			for($k = 0; $k < $nb_star_empty; $k++) {
				?><span class="hika_comment_listing_empty_stars hk-rate-star state-empty" aria-hidden="true"></span><?php
			}
		}
?>
		</td>
...
?>
		<td class="hika_comment_listing_useful"><button type="button" class="hika_useful_btn" title="<?php echo JText::_('HIKA_USEFUL'); ?>" onclick="if(!window.Oby.hasClass(this.parentNode, 'next_button_disabled')) hikashop_vote_useful(<?php echo $elt->vote_id;?>,1); window.Oby.addClass(this.parentNode, 'next_button_disabled');"><span class="element-invisible"><?php echo JText::_('HIKA_USEFUL'); ?></span></button></td>
<?php if($row->useful_style == 'thumbs'){?>
		<td class="hika_comment_listing_useful_p"><?php
			echo $hika_useless;
		?></td>
<?php } ?>
		<td class="hika_comment_listing_useless"><button type="button" class="hika_useless_btn" title="<?php echo JText::_('HIKA_USELESS'); ?>" onclick="if(!window.Oby.hasClass(this.parentNode, 'next_button_disabled')) hikashop_vote_useful(<?php echo $elt->vote_id;?>,2); window.Oby.addClass(this.parentNode, 'next_button_disabled');"><span class="element-invisible"><?php echo JText::_('HIKA_USELESS'); ?></span></button></td>
<?php
...
<?php } ?>
	<tr>
		<td colspan="8">
			<div id="<?php echo $i++; ?>"<?php echo $div_elements; ?> class="hika_comment_listing_content" data-nosnippet><?php echo nl2br($this->escape($elt->vote_comment)); ?></div>
<?php if(!empty($elt->vote_answer)) { ?>
			<div class="hikashop_vote_answer" data-nosnippet>
				<label class="hikashop_vote_answer_label"><i class="fa fa-comment"></i> <?php echo JText::_('OUR_ANSWER'); ?></label>
				<div class="hikashop_vote_answer_content"><?php echo $elt->vote_answer; ?></div>
			</div>
<?php } ?>
		</td>
	</tr>
