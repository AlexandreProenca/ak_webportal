...
if(hikashop_loadUser() == null) {
	$prefill_pseudo_attr = !empty($row->prefill_pseudo) ? ' value="'.htmlspecialchars($row->prefill_pseudo, ENT_QUOTES, 'UTF-8').'"' : '';
	$prefill_email_attr  = !empty($row->prefill_email)  ? ' value="'.htmlspecialchars($row->prefill_email,  ENT_QUOTES, 'UTF-8').'"' : ' value=""';
?>
	<table class="hikashop_comment_form">
		<tr class="hikashop_comment_form_name">
			...
			<td>
				<input  type='text' name="pseudo_comment" id='pseudo_comment'<?php echo $prefill_pseudo_attr; ?> class="<?php echo HK_FORM_CONTROL_CLASS; ?>" />
			</td>
		</tr>
<?php
...
?>
		<tr class="hikashop_comment_form_mail">
			...
			<td>
				<input  type='text' name="email_comment" id='email_comment'<?php echo $prefill_email_attr; ?> class="<?php echo HK_FORM_CONTROL_CLASS; ?>"/>
			</td>
		</tr>
<?php
	} else {
...
?>
<?php
	if(!empty($row->order_token)) {
?>
	<input type="hidden" name="order_token" id="order_token" value="<?php echo htmlspecialchars($row->order_token, ENT_QUOTES, 'UTF-8'); ?>" />
	<input type="hidden" name="order_id" id="order_id" value="<?php echo (int)$row->order_id; ?>" />
<?php
	}
?>
