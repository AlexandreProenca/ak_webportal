...
?>
	<input type="hidden" name="hikashop_vote_rating" data-votetype="<?php echo $row->type_item; ?>" data-max="<?php echo $row->hikashop_vote_nb_star; ?>" data-ref="<?php echo $row->vote_ref_id;?>" data-rate="<?php echo $row->hikashop_vote_average_score_rounded; ?>" data-original-title="<?php echo htmlspecialchars($tooltip, ENT_QUOTES, 'UTF-8'); ?>" id="<?php echo $select_id;?>" />
	<span id="hikashop_vote_status_<?php echo $row->vote_ref_id;?>" class="hikashop_vote_notification_mini"></span>
<?php
} else {
?>
	<div class="hk-rating" data-original-title="<?php echo htmlspecialchars($tooltip, ENT_QUOTES, 'UTF-8'); ?>" data-toggle="hk-tooltip">
<?php
