...
if(!$this->module){
	...
	if(isset($data->hk_product) && (is_object($data->hk_product) || is_array($data->hk_product))) {
		...
		$hkHadSubcategories = !empty($this->rows) && count($this->rows);
		if($hkHadSubcategories) {
			hikaInput::get()->set('hk_parent_has_subcategories', 1);
		}
		foreach($this->modules as $module){
			$html .= JModuleHelper::renderModule($module);
		}
		if($hkHadSubcategories) {
			hikaInput::get()->set('hk_parent_has_subcategories', 0);
		}
		if(!empty($html)){
