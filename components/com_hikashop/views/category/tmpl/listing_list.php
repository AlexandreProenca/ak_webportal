...
if($in_hikashop_context && empty($cid) && !empty($app->getMenu())) {
	$hkActiveMenu = $app->getMenu()->getActive();
	if(!empty($hkActiveMenu) && !empty($hkActiveMenu->type) && $hkActiveMenu->type == 'alias') {
		$hkAliasParams = $hkActiveMenu->getParams();
		$hkAliasTarget = (int)$hkAliasParams->get('aliasoptions', 0);
		if(!empty($hkAliasTarget))
			$hkActiveMenu = $app->getMenu()->getItem($hkAliasTarget);
	}
	if(!empty($hkActiveMenu)) {
		$hkMenuParams = $hkActiveMenu->getParams();
		$hkMenuCat = $hkMenuParams->get('hk_product', $hkMenuParams->get('hk_category', null));
		if(!empty($hkMenuCat->category))
			$cid = (int)$hkMenuCat->category;
		elseif(!empty($hkMenuCat->selectparentlisting))
			$cid = (int)$hkMenuCat->selectparentlisting;
	}
}
