...
class CategoryViewCategory extends HikaShopView {
	...
	function listing() {
		...
		if(empty($this->module)) {
			...
			if(!(method_exists($pathway, 'isPrefilled') && $pathway->isPrefilled())) {
				$breadcrumbItems = $categoryClass->buildBreadcrumbItems($cid, $this->params->get('selectparentlisting'), $menu_id);
	...
	function getLink($cid, $alias = '') {
		...
		if(!empty($Itemid) && $config->get('forward_to_submenus', 1)) {
			static $submenuCategoryMap = array();
			if(!isset($submenuCategoryMap[$Itemid])) {
				$submenuCategoryMap[$Itemid] = array();
				$app = JFactory::getApplication();
				$menus	= $app->getMenu();
				$query = 'SELECT a.id as itemid FROM `#__menu` as a WHERE a.client_id=0 AND a.parent_id IN ('.(int)$Itemid.', (SELECT b.parent_id FROM `#__menu` as b WHERE b.id='.(int)$Itemid.'))';
				$db = JFactory::getDBO();
				$db->setQuery($query);
				...
				foreach($submenus as $submenu){
					...
					if(!empty($menu) && !empty($menu->link) && strpos($menu->link,'option='.HIKASHOP_COMPONENT)!==false && (strpos($menu->link,'view=category')!==false || strpos($menu->link,'view=')===false || strpos($menu->link,'view=product')!==false)) {
						...
						if(!empty($params) && !empty($parent) && !isset($submenuCategoryMap[$Itemid][(int)$parent]))
							$submenuCategoryMap[$Itemid][(int)$parent] = (int)$submenu->itemid;
					}
				}
			}

			if(isset($submenuCategoryMap[$Itemid][(int)$cid->category_id])) {
				$submenu_itemid = $submenuCategoryMap[$Itemid][(int)$cid->category_id];
				$url = JRoute::_('index.php?option=com_hikashop&Itemid='.$submenu_itemid);
