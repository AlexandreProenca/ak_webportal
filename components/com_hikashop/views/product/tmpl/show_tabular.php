...
if(!empty($this->fields)){
	foreach($this->fields as $fieldName => $oneField) {
		if(!empty($oneField->field_display) && !empty($oneField->field_display['show_tabular_own_tab'])) {
			$tabFields[$fieldName] = $oneField;
	...
	<div id="hikashop_product_description_main_mini" class="hikashop_product_description_main_mini"><?php
		if(!empty($this->element->product_description)) {
			...
			if (!empty($this->element->product_description) && strlen($this->element->product_description)>300)
				$resume .= " ...<a class='hiakshop_product_tabular_mini_description_read_more' href='#hikashop_show_tabular_description'>".JText::_('READ_MORE')."</a>";
...
<?php if(empty ($this->element->characteristics) || $this->params->get('characteristic_display') != 'list') { ?>
		<div id="hikashop_product_quantity_main" class="hikashop_product_quantity_main"><?php
			...
			$this->ajax = 'if(hikashopCheckChangeForm(\'item\',\'' . $rawFormName . '\')){ return hikashopModifyQuantity(\'' . $this->row->product_id . '\',field,1' . $form . ',\'cart\'); } else { return false; }';
...
<?php } ?>
<!-- EO VOTE TAB TITLE -->
<!-- CUSTOM FIELD TAB TITLES -->
<?php
	if(!empty($tabFields)) {
		foreach($tabFields as $fieldName => $oneTabField) {
			$tabFieldValue = '';
			...
			if(isset($this->element->$fieldName))
				$tabFieldValue = trim($this->element->$fieldName);
			if(!empty($tabFieldValue) || $tabFieldValue === '0' || $oneTabField->field_type == 'customtext') {
				if(empty($selected)) $selected = 'hikashop_show_tabular_field_' . $oneTabField->field_namekey;
?>
			<li id="hikashop_show_tabular_field_<?php echo $oneTabField->field_namekey; ?>_li" class="hikashop_tabs_li ui-corner-top"><?php echo $this->fieldsClass->getFieldName($oneTabField); ?></li>
<?php
			}
		}
	}
?>
<!-- EO CUSTOM FIELD TAB TITLES -->
		</ul>
<?php if(!empty($description) || !empty ($this->element->product_url)) { ?>
	...
	if(!empty($tabFields)) {
		...
		foreach($tabFields as $fieldName => $oneTabField) {
			...
			if(isset($this->element->$fieldName))
				$tabFieldValue = trim($this->element->$fieldName);
			if(!empty($tabFieldValue) || $tabFieldValue === '0' || $oneTabField->field_type == 'customtext') {
				$oneTabField->currentElement = $this->element;
?>
		<div class="hikashop_tabs_content" id="hikashop_show_tabular_field_<?php echo $oneTabField->field_namekey; ?>">
			<div class="hikashop_product_custom_tab_field hikashop_product_custom_tab_<?php echo $oneTabField->field_namekey; ?>">
				<?php echo $this->fieldsClass->show($oneTabField, $tabFieldValue); ?>
			</div>
		</div>
<?php
			}
