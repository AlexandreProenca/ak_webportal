...
?>
<div class="hikashop_products"<?php if($listingMicrodata) { ?> itemscope itemtype="https://schema.org/ItemList"<?php } ?>>
<?php
$columns = 1;
...
<?php } ?>
		<table class="hikashop_products_table adminlist table table-striped table-hover" cellpadding="1"
			 <?php if($listingMicrodata) { ?>itemscope itemtype="https://schema.org/ItemList"<?php } ?>>
			<thead>
				<tr>
<!-- IMAGE HEADER -->
<?php if($this->config->get('thumbnail')){ $columns++; ?>
					<th scope="col" class="hikashop_product_image title hk_center"><?php
						echo JText::_('HIKA_IMAGE');
					?></th>
<?php } ?>
<!-- EO IMAGE HEADER -->
<!-- NAME HEADER -->
					<th scope="col" class="hikashop_product_name title hk_center"><?php
						echo JText::_('PRODUCT');
					?></th>
<!-- EO NAME HEADER -->
<!-- CODE HEADER -->
<?php if ($this->config->get('show_code')) { $columns++; ?>
					<th scope="col" class="hikashop_product_code title hk_center"><?php
	...
	if(hikashop_level(1) && !empty($this->productFields)) {
		...
		if(!empty($productFields)) {
			foreach($productFields as $field) {
				$columns++;
?>
					<th scope="col" class="hikashop_product_field title hk_center"><?php
...
?>
	<th scope="col" class="hikashop_product_characteristic_on_listing hikashop_product_characteristic_on_listing_<?php echo $characteristic->characteristic_id; ?>">
...
<?php if($this->params->get('show_vote')){ $columns++; ?>
					<th scope="col" class="hikashop_product_vote title hk_center">
...
<?php
	...
	if($this->params->get('show_price')) {
		$columns++;
?>
					<th scope="col" class="hikashop_product_price title hk_center"><?php
						echo JText::_('PRICE');
					?></th>
<?php } ?>
<!-- EO PRICE HEADER -->
...
<?php if($this->params->get('add_to_cart') || $this->params->get('add_to_wishlist') || (hikashop_level(1) && $this->params->get('product_contact_button', 0)) || (int)$this->params->get('details_button', 0)) { $columns++; ?>
					<th scope="col" class="hikashop_product_add_to_cart title hk_center">
					</th>
<?php } ?>
<!-- EO BUTTONS HEADER -->
<!-- COMPARISON HEADER -->
<?php if(hikaInput::get()->getVar('hikashop_front_end_main', 0) && hikaInput::get()->getVar('task') == 'listing' && $this->params->get('show_compare')) { $columns++; ?>
					<th scope="col" class="hikashop_product_compare title hk_center">
					</th>
<?php } ?>
...
?>
				<tr class="<?php echo implode(' ', $hk_main_classes); ?>" <?php if($listingMicrodata) { ?>itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"<?php } ?>>
