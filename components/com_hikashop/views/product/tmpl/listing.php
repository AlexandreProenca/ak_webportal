...
if($filter_type !== 3) {
	...
	if(!$this->module && (!$infinite_scroll || empty($this->pageInfo->limit->start)))
		echo $mainInfo;
	if(!empty($html)){
		if($this->module && (!$infinite_scroll || empty($this->pageInfo->limit->start))) echo $mainInfo;
		if(!empty($htmlFilter) && $ctrl == 'category')
			echo $htmlFilter;
?>
...
<?php
		...
		} elseif(!$this->params->get('category_has_subcategories', 0) && !hikaInput::get()->getVar('hk_parent_has_subcategories', 0)) {
	...
	if((!empty($allrows) || !$this->module || hikaInput::get()->getVar('hikashop_front_end_main',0)) && in_array($pagination, array('top','both')) && $this->params->get('show_limit') && $this->pageInfo->elements->total > $this->pageInfo->limit->value) {
...
<?php
	...
	foreach($this->categories as $category) {
		...
		if(!(int)$this->config->get('simplified_breadcrumbs', 2))
...
<?php
	...
	if((!empty($allrows) || !$this->module || hikaInput::get()->getVar('hikashop_front_end_main',0)) && in_array($pagination,array('bottom','both')) && $this->params->get('show_limit') && $this->pageInfo->elements->total > $this->pageInfo->limit->value) {
