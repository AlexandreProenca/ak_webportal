...
if($this->params->get('characteristic_display') != 'list') {
	...
	$variant_preselected = isset($this->selected_variant_id) && $this->selected_variant_id;
	...
	if(!file_exists($chromePath)) {
		...
		if(!empty($parentTemplate)) {
			$chromePath = JPATH_THEMES . DS . $parentTemplate . DS . 'html' . DS . 'hikashop_characteristics.php';
		}
	}
	if(file_exists($chromePath)) {
					...
					foreach($firstCharacteristic->values as $value){
						...
						if(!empty($secondCharacteristic->values)) {
							foreach($secondCharacteristic->values as $value2) {
								...
								$radio .= " /></span>";
								if(!empty($classspan)) {
									$radio .= '<span class="element-invisible">'.JText::_('HIKA_OUT_OF_STOCK').'</span>';
								}
								$html .= '<td>'.$radio.'</td>';
							}
						}
...
<?php	if($variant_preselected) { ?>
	...
	if(firstEl) {
		if(firstEl.tagName.toLowerCase() == 'select') {
			...
			var checkedInput = firstRow.querySelector('input:checked');
			if(checkedInput)
				hikashopVariantSelected(checkedInput);
		}
	}
<?php	} ?>
<?php } ?>
...
function hikashopUpdateVariant(obj) {
	...
	for (var i = 0; i < len; i++) {
		...
		var checkFields = form ? form.elements['hikashop_product_characteristic['+options[i]+']'] : null;
		if(!checkFields) {
			var charBlock = document.getElementById('hikashop_product_characteristics');
			var list = charBlock ? charBlock.querySelectorAll('[name="hikashop_product_characteristic['+options[i]+']"]') : [];
			if(list.length == 1 && list[0].type != 'radio' && list[0].type != 'checkbox' && list[0].value) {
				selection += '_' + list[0].value;
				continue;
			}
			checkFields = list;
		}
...
function hikashopUpdateVariantData(selection) {
	...
	var mainFormId = 'hikashop_product_form';

	var savedQty = '';
	var qtyEl = document.querySelector('#hikashop_product_quantity_main .hikashop_product_quantity_field');
	if(qtyEl) savedQty = qtyEl.value;

	for(var i = 0; i < len; i++){
		var el = document.getElementById('hikashop_product_'+names[i]+'_main');
		var el2 = document.getElementById('hikashop_product_'+names[i]+selection);
		if(el && el2) {
			el.innerHTML = el2.innerHTML.replace(/_VARIANT_NAME/g, selection).replace(/data-content/g, 'content');
			var swapped = el.querySelectorAll('input[form^="hikashop_product_form_"], select[form^="hikashop_product_form_"], textarea[form^="hikashop_product_form_"], button[form^="hikashop_product_form_"]');
			for(var j = 0; j < swapped.length; j++) {
				swapped[j].setAttribute('form', mainFormId);
			}
		}
	}

	if(savedQty) {
		qtyEl = document.querySelector('#hikashop_product_quantity_main .hikashop_product_quantity_field');
		if(qtyEl) qtyEl.value = savedQty;
	}
	var cartBtn = document.querySelector('#hikashop_product_quantity_main [data-addToCart]');
	if(cartBtn) cartBtn.setAttribute('data-addTo-div', mainFormId);
	var wishBtn = document.querySelector('#hikashop_product_quantity_main [data-addToWishlist]');
	if(wishBtn) wishBtn.setAttribute('data-addTo-div', mainFormId);

	if(window.hikaProductOptions) hikaProductOptions.refreshPrice();
	else if(typeof window['hikashopRefreshOptionPrice'] == 'function') hikashopRefreshOptionPrice();
