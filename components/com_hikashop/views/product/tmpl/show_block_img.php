...
if(!$this->config->get('thumbnail')) {
	if(!empty ($this->element->images)) {
		echo '<img src="' . $this->image->uploadFolder_url . $image->file_path . '" alt="' . $image->file_name . '" id="hikashop_main_image'.$variant_name.'" style="margin-top:10px;margin-bottom:10px;display:inline-block;vertical-align:middle" />';
	}
} else {
	$style = '';
	...
	if( !empty($this->element->images) && count($this->element->images) > 1) {
		...
		foreach($this->element->images as $key => $img) {
			...
			$prev_btn .= '<a href="#" id="hikashop_main_image'.$variant_name.'_prev_'.$key.'" class="hikashop_slide_prev '.$selected_slide_prev.'" onclick="onMouseOverTrigger('.$prev_id.'); return false;">'
				. '<i class="fas fa-chevron-left" aria-hidden="true"></i><span class="element-invisible">'.JText::_('HIKA_PREVIOUS').'</span>'
			. '</a>';
			$next_btn .= '<a href="#" id="hikashop_main_image'.$variant_name.'_next_'.$key.'" class="hikashop_slide_next '.$selected_slide_next.'" onclick="onMouseOverTrigger('. $next_id .'); return false;">'
				. '<i class="fas fa-chevron-right" aria-hidden="true"></i><span class="element-invisible">'.JText::_('HIKA_NEXT').'</span>'
			. '</a>';
		}
	}
?>
	...
	if($this->image->override) {
		...
		if(@$img->success) {
			...
			if(!empty($img->video_url)) {
				if($img->video_type == 'youtube') {
					$video_html = '<iframe width=\''.$width.'\' height=\''.$height.'\' src=\'https://www.youtube.com/embed/'.$img->video_id.'\' frameborder=\'0\' allowfullscreen></iframe>';
				} elseif($img->video_type == 'vimeo') {
					$video_html = '<iframe src=\'https://player.vimeo.com/video/'.$img->video_id.'\' width=\''.$width.'\' height=\''.$height.'\' frameborder=\'0\' allow=\'autoplay; fullscreen; picture-in-picture\' allowfullscreen></iframe>';
				} elseif($img->video_type == 'dailymotion') {
					$video_html = '<iframe frameborder=\'0\' width=\''.$width.'\' height=\''.$height.'\' src=\'//www.dailymotion.com/embed/video/'.$img->video_id.'\' allowfullscreen allow=\'autoplay\'></iframe>';
				}
			}

			if(!empty($video_html)) {
				echo '<div id="hikashop_main_image'.$variant_name.'_video_0" class="hikashop_video_container">'.$video_html.'</div>';
				$attributes = 'style="display:none;margin-top:10px;margin-bottom:10px;vertical-align:middle"';
			} else {
			...
			if(!empty($video_html)) {
				$attributes = str_replace('display:inline-block;', 'display:none;', $attributes);
				echo '<a id="hikashop_main_image'.$variant_name.'_link" style="display:none;" '.$attr.' href="'.$img->origin_url.'">'.$html.'</a>';
			} else {
...
<?php
	if( !empty($this->element->images) && count($this->element->images) > 1) {
		$firstThunb = true;
		$i = 0;
		$video_preloads = array(); // Collect video iframes to preload in main image area
		foreach($this->element->images as $key => $image) {
			...
			if(!empty($img->video_url)) {
				...
				if(!empty($video_html)) {
					$video_preloads[$key] = '<div id="hikashop_main_image'.$variant_name.'_video_'.$key.'" class="hikashop_video_container" style="display:none;">'.$video_html.'</div>';
					$video_ref = "'video_".$key."'";
				}
			}

			$attr = $thumbnail_class.' title="'.$this->escape((string)@$image->file_description).'" onmouseover="return window.localPage.changeImage('
				. 'this, \'hikashop_main_image'.$variant_name.'\', \''.$img->url.'\', '.$main_image_size.', \''.str_replace(array("'", '"'),
				array("\'", '&quot;'),@$image->file_description).'\', \''.str_replace(array("'", '"'),array("\'", '&quot;'),@$image->file_name).'\', \''.$ref.'\', '.$video_ref.');"';
...
<!-- EO THUMBNAILS -->
<?php
if(!empty($video_preloads)) {
	echo '<div id="hikashop_preloaded_videos'.$variant_name.'" class="hikashop_preloaded_videos" style="position:absolute;">';
	foreach($video_preloads as $video_preload) {
		echo $video_preload;
	}
	echo '</div>';
}
