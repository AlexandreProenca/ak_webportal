...
if(!$this->module || hikaInput::get()->getVar('hikashop_front_end_main',0)) {
	...
	$priceDisplayParams = $active ? $active->getParams() : $this->params;
} else {
...
?>
	...
	if(empty($this->row->prices)) {
		echo JText::_(!empty($this->row->product_free_price_message) ? $this->row->product_free_price_message : 'FREE_PRICE');
	} else {
		$first = true;
