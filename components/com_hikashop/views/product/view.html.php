...
class ProductViewProduct extends HikaShopView {
	...
	public function display($tpl = null, $params = array()) {
		...
		$this->lang_param = '&lang=' . urlencode(JFactory::getLanguage()->getTag());
	...
	public function listing() {
		...
		if(!empty($this->module)) {
			...
			if($config->get('show_feed_link', 1) == 1) {
				if($config->get('hikarss_format') != 'none') {
					...
					$doc_title = htmlentities(strip_tags($doc_title), ENT_QUOTES, 'UTF-8');
		...
		if(!empty($this->filters) && !empty($rows)) {
			$fieldClass = hikashop_get('class.field');
			$cid_state = hikaInput::get()->getInt('cid', 'itemid_' . hikaInput::get()->getInt('Itemid', 0));
			$params_parts = array();
			foreach($this->filters as $filter) {
				$customField = @$filter->filter_options['custom_field'];
				if(empty($customField))
					continue;
				$fieldDef = $fieldClass->getField($customField, 'item');
				if(empty($fieldDef))
					continue;
				$filterValue = $app->getUserState('com_hikashop.' . $cid_state . '_filter_' . $filter->filter_namekey, '');
				if(is_array($filterValue))
					$filterValue = reset($filterValue);
				$filterValue = trim((string)$filterValue);
				$filterValue = str_replace('_m', '', $filterValue);
				if(!empty($filterValue) && $filterValue != $filter->filter_namekey && $filterValue != 'none')
					$params_parts[] = 'item_data_' . urlencode($customField) . '=' . urlencode($filterValue);
			}
			if(!empty($params_parts)) {
				$itemDataStr = implode('&', $params_parts);
				foreach($rows as &$row) {
					$row->item_data_params = $itemDataStr;
				}
				unset($row);
			}
		}
		...
		if(empty($this->module)) {
			...
			if(empty($menu)) {
				...
				if(!(method_exists($pathway, 'isPrefilled') && $pathway->isPrefilled())) {
					$breadcrumbItems = $categoryClass->buildBreadcrumbItems(reset($pageInfo->filter->cid));
					foreach($breadcrumbItems as $crumb) {
						$pathway->addItem($crumb->name, $crumb->link);
					}
				}
			} else {
				$category_pathway = '&'.$pathway_sef_name.'='.reset($pageInfo->filter->cid);
				$activeCid = (int)reset($pageInfo->filter->cid);
				$baseCat = (int)$this->params->get('selectparentlisting');
				if(!empty($activeCid) && $activeCid !== $baseCat) {
					$categoryClass = hikashop_get('class.category');
					$pathway = $app->getPathway();
					if(!(method_exists($pathway, 'isPrefilled') && $pathway->isPrefilled())) {
						$breadcrumbItems = $categoryClass->buildBreadcrumbItems($activeCid, $baseCat, $url_itemid);
						foreach($breadcrumbItems as $crumb) {
							$pathway->addItem($crumb->name, $crumb->link);
						}
					}
		...
		if((int)$config->get('simplified_breadcrumbs', 2))
			$category_pathway = '';
	...
	public function show() {
		...
		$element = $productClass->loadFullProduct($product_id);
		if(empty($element))
			return;

		$product_id = (int)$element->product_id;
		$main_product_id = !empty($element->main->product_id) ? (int)$element->main->product_id : $product_id;
		$this->selected_variant_id = ($requested_product_id != $main_product_id) ? $requested_product_id : 0;
		hikaInput::get()->set('product_id', $product_id);
		...
		$arr[$main_product_id] = $main_product_id;
		$_SESSION['hikashop_viewed_products'] = array_reverse($arr, true);

		if($productClass->hit($main_product_id))
			$element->product_hit++;

		$productClass->checkSortPriceOnView($element);
		...
		if(empty($menu) || !(strpos($menu->link,'option='.HIKASHOP_COMPONENT) !== false && strpos($menu->link,'view=product') !== false && strpos($menu->link,'layout=show') !== false)) {
			...
			if(method_exists($pathway, 'isPrefilled') && $pathway->isPrefilled()) {
				...
				$simplified_breadcrumbs = (int)$config->get('simplified_breadcrumbs', 2);

				if(empty($category_pathway) && $simplified_breadcrumbs == 2 && !empty($element->_categories)) {
					$mainCat = reset($element->_categories);
					if(!empty($mainCat->category_id)) {
						$category_pathway = (int)$mainCat->category_id;
					}
				}
				...
				if($category_pathway) {
					...
					$breadcrumbItems = $categoryClass->buildBreadcrumbItems($category_pathway, $parent, $url_itemid);
					foreach($breadcrumbItems as $crumb) {
						$pathway->addItem($crumb->name, $crumb->link);
					}
				}
				...
				if($simplified_breadcrumbs || !$category_pathway) {
					$category_pathway = '';
				} else {
					$pathway_sef_name = $config->get('pathway_sef_name','category_pathway');
				...
				$breadcrumb_name = (!empty($element->main->product_name)) ? strip_tags(hikashop_translate($element->main->product_name)) : strip_tags($product_name);
				$pathway->addItem($breadcrumb_name, hikashop_completeLink('product&task=show&cid='.(int)$element->product_id.'&name='.$element->alias.$category_pathway.$url_itemid));
			}
		}
		...
		$fields = isset($element->_custom_fields) ? $element->_custom_fields : $fieldsClass->getFields('frontcomp', $element, 'product', 'checkout&task=state');
		$this->assignRef('fieldsClass', $fieldsClass);
		$this->assignRef('fields', $fields);

		$this->itemFields = isset($element->_item_fields) ? $element->_item_fields : null;
		...
		if($config->get('show_other_product_shortcut')) {
			...
			if(empty($category_id)) {
				$nav_product_id = !empty($main_product_id) ? $main_product_id : $product_id;
				$query = 'SELECT a.category_id FROM '.hikashop_table('product_category').' AS a WHERE a.product_id='.(int)$nav_product_id.' ORDER BY a.product_category_id ASC';
			...
			if(!empty($articles)) {
				...
				foreach($articles as $k => $article){
					$nav_main_id = !empty($main_product_id) ? $main_product_id : $product_id;
					if($article != $element->product_id && $article != $element->product_parent_id && $article != $nav_main_id)
						continue;
					...
					$n = $k;
					while(isset($articles[$n]) && ($articles[$n] == $element->product_id || $articles[$n] == $element->product_parent_id || $articles[$n] == $nav_main_id)) {
						$n++;
					}
		...
		$categories = isset($element->_categories) ? $element->_categories : array();
		...
		if(!empty($product_id)) {
			...
			if(!empty($element)){
				...
				if($element->product_type=='variant') {
					...
					$filters=array('a.product_id='.(int)$element->product_parent_id);
					hikashop_addACLFilters($filters,'product_access','a');
					$query = 'SELECT a.*,b.* FROM '.hikashop_table('product').' AS a LEFT JOIN '.hikashop_table('product_category').' AS b ON a.product_id = b.product_id WHERE '.implode(' AND ',$filters). ' LIMIT 1';
					$database->setQuery($query);
...
";
	...
	function _resolveProductParam($product, $configValue) {
		if(empty($configValue))
			return '';
		if(isset($product->$configValue)) {
			return (string)$product->$configValue;
		}
		return $configValue;
	}

	function _buildReturnPolicy($product, $gp) {
		if(empty($gp['return_policy_country']))
			return null;

		$category = '';
		if(!empty($gp['return_policy_category_field'])) {
			$fieldName = $gp['return_policy_category_field'];
			if(isset($product->$fieldName)) {
				if(empty($product->$fieldName))
					return null;
				$category = $product->$fieldName;
			}
		}
		if(empty($category)) {
			if(empty($gp['return_policy_category']))
				return null;
			$category = $gp['return_policy_category'];
		}

		$type = '@type';
		$obj = new stdClass();
		$obj->$type = 'MerchantReturnPolicy';
		$obj->returnPolicyCategory = 'https://schema.org/' . $category;

		if($category === 'MerchantReturnFiniteReturnWindow' && !empty($gp['return_policy_days'])) {
			$days = $this->_resolveProductParam($product, $gp['return_policy_days']);
			if($days !== '')
				$obj->merchantReturnDays = (int)$days;
		}

		$countries = array_map('trim', explode(',', $gp['return_policy_country']));
		$obj->applicableCountry = (count($countries) === 1) ? $countries[0] : $countries;
		if(!empty($gp['return_policy_fees']) && $category !== 'MerchantReturnNotPermitted')
			$obj->returnFees = 'https://schema.org/' . $gp['return_policy_fees'];
		if(!empty($gp['return_policy_method']) && $category !== 'MerchantReturnNotPermitted')
			$obj->returnMethod = 'https://schema.org/' . $gp['return_policy_method'];
		if(!empty($gp['return_policy_link']))
			$obj->merchantReturnLink = $gp['return_policy_link'];

		return $obj;
	}

	function _buildShippingDetails($product, $gp, $currencyCode) {
		if(empty($gp['shipping']))
			return array();

		$shippingText = $this->_resolveProductParam($product, $gp['shipping']);
		if(empty($shippingText))
			return array();

		$type = '@type';
		$details = array();
		$shipping_methods = explode(',', $shippingText);

		foreach($shipping_methods as $shipping_method) {
			$sd = explode(':', $shipping_method);
			if(count($sd) < 4 || empty($sd[0]))
				continue;

			$detail = new stdClass();
			$detail->$type = 'OfferShippingDetails';

			$dest = new stdClass();
			$dest->$type = 'DefinedRegion';
			$dest->addressCountry = trim($sd[0]);
			if(!empty($sd[1]))
				$dest->addressRegion = trim($sd[1]);
			$detail->shippingDestination = $dest;

			if(!empty($sd[3])) {
				$priceParts = explode(' ', trim($sd[3]));
				$rate = new stdClass();
				$rate->$type = 'MonetaryAmount';
				$rate->value = (float)$priceParts[0];
				$rate->currency = isset($priceParts[1]) ? $priceParts[1] : $currencyCode;
				$detail->shippingRate = $rate;
			}

			$hasTime = !empty($sd[4]) || !empty($sd[5]) || !empty($sd[6]) || !empty($sd[7]);
			if($hasTime) {
				$dt = new stdClass();
				$dt->$type = 'ShippingDeliveryTime';
				if(isset($sd[4]) && $sd[4] !== '' && isset($sd[5]) && $sd[5] !== '') {
					$ht = new stdClass();
					$ht->$type = 'QuantitativeValue';
					$ht->minValue = (int)$sd[4];
					$ht->maxValue = (int)$sd[5];
					$ht->unitCode = 'DAY';
					$dt->handlingTime = $ht;
				}
				if(isset($sd[6]) && $sd[6] !== '' && isset($sd[7]) && $sd[7] !== '') {
					$tt = new stdClass();
					$tt->$type = 'QuantitativeValue';
					$tt->minValue = (int)$sd[6];
					$tt->maxValue = (int)$sd[7];
					$tt->unitCode = 'DAY';
					$dt->transitTime = $tt;
				}
				$detail->deliveryTime = $dt;
			}
