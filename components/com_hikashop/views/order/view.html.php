...
class OrderViewOrder extends hikashopView {
	...
	public function listing() {
		...
		if(!empty($menu) && method_exists($menu, 'getParams') && $menu->link == 'index.php?option=com_hikashop&view=order&layout=listing') {
			...
			if(!(int)$config->get('cpanel_legacy', false))
				$this->cpanel_active_page = 'order';
		}
	...
	public function show() {
		...
		if($this->invoice_type == 'order') {
			...
			if(hikashop_level(1) && in_array($this->element->order_status, $print_statuses) && !($excludeFreeOrders && empty($this->element->order_invoice_number))) {
			...
			if(hikashop_get('class.withdrawal')->isWithdrawable($this->element)) {
				$url = 'withdrawal&task=form&withdrawal_order_id='.$this->element->order_id.$url_itemid;
				$token = hikaInput::get()->getVar('order_token');
				if(!empty($token))
					$url .= '&order_token='.urlencode($token);
				$toolbar_array['withdrawal'] = array(
					'icon' => 'out',
					'name' => JText::_('HIKA_WITHDRAW_FROM_CONTRACT'),
					'url' => hikashop_completeLink($url),
					'fa' => array('html' => '<i class="fas fa-undo"></i>')
				);
			}
