...
<?php if($this->invoice_type == 'order' && !empty($product->product_id)) {
	$product_url = hikashop_contentLink('product&task=show&cid='.$product->product_id.$this->url_itemid, $productData);
	if(!empty($this->order->order_token)) {
		$product_url .= (strpos($product_url, '?') !== false ? '&' : '?')
			. 'order_id='.(int)$this->order->order_id
			. '&order_token='.urlencode($this->order->order_token);
	}
?>
									<a class="hikashop_order_product_link" href="<?php echo $product_url; ?>">
<?php } ?>
...
<!-- EO ENTRIES -->
	...
	<input type="hidden" name="cancel_redirect" value="<?php echo htmlspecialchars(hikaInput::get()->getString('cancel_redirect', ''), ENT_QUOTES, 'UTF-8'); ?>" />
	<input type="hidden" name="cancel_url" value="<?php echo htmlspecialchars(hikaInput::get()->getString('cancel_url', ''), ENT_QUOTES, 'UTF-8'); ?>" />
