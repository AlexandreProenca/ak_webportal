	...
	public function show() {
			...
			if(!empty($cart_id)) {
				...
				if(!empty($cart) && !empty($cart->cart_products)) {
					if(empty($user_id)) {
						...
						$app->redirect(JRoute::_($url . $suffix . '&return='.urlencode(base64_encode(hikashop_currentUrl('', false))), false));
						return true;
					} elseif((int)$cart->user_id !== (int)$user_id) {
						$cart = null;
					}
				}
			}
		...
		if(!empty($cart)) {
			if($cart->cart_type == 'wishlist' && $cart->user_id != $user_id && $cart->cart_share == 'email') {
				...
				if(!empty($cart->cart_params->token) && !hash_equals($cart->cart_params->token, $token)) {
	...
	public function product_save() {
		...
		if(!empty($new_product_id)) {
			...
			if($old_product->product_parent_id != $new_product->product_parent_id) {
				...
				$options['product_being_removed'] = $product_id;
			}
		...
		$delete_old = false;
		if(!isset($cart_products[0]['pid']) || $cart_products[0]['pid'] != $cart_product_id) {
			$delete_old = true;
		}
