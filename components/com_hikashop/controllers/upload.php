...
class uploadController extends hikashopController {
	...
	private function initController() {
		...
		if(!empty($controllerName)) {
			...
			if(empty($this->base_controller) || !method_exists($this->base_controller, 'getUploadSetting'))
	...
	public function addImage() {
		...
		if($ret !== false && empty($ret->error)) {
			...
			foreach($ret as &$r) {
				...
				$form_id = hikaInput::get()->getString('form_id', '');
				if(!empty($form_id) && !empty($r->html)) {
					$r->html = hikashop_add_form_attribute($r->html, $form_id);
				}
	...
	public function upload() {
		...
		if($ret !== false && empty($ret->error) && empty($ret->partial)) {
			...
			if($type == 'image') {
				...
				if(!empty($options['upload_dir'])) {
					$imageHelper->uploadFolder = $options['upload_dir'];
				}
				if(!empty($options['upload_url'])) {
					$imageHelper->uploadFolder_url = $options['upload_url'];
				}
			}
			$file = new stdClass();
			$file->file_description = '';
			...
			if($type == 'image') {
				...
				if(empty($options['custom_field'])) {
					...
					$ret->thumbnail_url = hikashop_completeLink('order&task=download&field_table='.$options['field_table'].'&field_namekey='.urlencode(base64_encode($options['field_namekey'])).'&name='.urlencode(base64_encode($file->file_path)).$params);
				}
		...
		$form_id = hikaInput::get()->getString('form_id', '');
		if(!empty($form_id) && !empty($ret->html)) {
			$ret->html = hikashop_add_form_attribute($ret->html, $form_id);
		}
