...
class orderController extends hikashopController {
	...
	protected function _check($message = true) {
		...
		if(@$user->user_type === 'guest' || empty($user->user_cms_id) || (int)$user->user_cms_id == 0) {
			...
			if(empty($current_user_id) || (int)$user->user_id !== (int)$current_user_id) {
				return false;
			}
		}
	...
	public function send_email() {
		...
		if($send) {
			...
			if(!empty($element->email) && filter_var($element->email, FILTER_VALIDATE_EMAIL)) {
				$user_name = !empty($element->name) ? (string)$element->name : '';
		...
		if($status) {
			...
			$app->enqueueMessage(JText::_('AN_ERROR_OCCURED_DURING_THE_SENDING_OF_THE_EMAIL'), 'error');
		}
		...
		if(empty($redirect_url)) {
			...
			$redirect_url = hikashop_completeLink('order&task=show&cid='.$order->order_id.$url_itemid.$url_token);
		}
	...
	public function invoice() {
		...
		$config =& hikashop_config();
		if($config->get('invoice_exclude_free_orders', 0)) {
			$order_id = hikashop_getCID('order_id');
			$orderClass = hikashop_get('class.order');
			$order = $orderClass->get($order_id);
			if(!empty($order) && empty($order->order_invoice_number)) {
				$app = JFactory::getApplication();
				$app->redirect(hikashop_completeLink('order&task=show&order_id='.(int)$order_id));
				return false;
			}
		}
	...
	public function pay() {
		...
		if(@$order->customer->user_type === 'guest' || (isset($order->customer->user_cms_id) && (int)$order->customer->user_cms_id == 0)){
			$token = hikaInput::get()->getVar('order_token');
			if(empty($order->order_token) || !hash_equals($order->order_token, $token)){
				$app->enqueueMessage(JText::sprintf('ORDER_X_NOT_FOUND', $order_id), 'error');
				$app->redirect( hikashop_completeLink('order&task=listing'.$url_itemid, false, true) );
	...
	public function download() {
		...
		if(empty($file_id)){
			$field_table = hikaInput::get()->getCmd('field_table', '');
			...
			if(strpos($name, '..') !== false || strpos($field_namekey, '..') !== false) {
				$app=JFactory::getApplication();
