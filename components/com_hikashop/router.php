...
function _HikashopBuildRoute( &$query, $separator = '-' )
{
	...
	if(function_exists('hikashop_config') || include_once(rtrim(JPATH_ADMINISTRATOR,DS).DS.'components'.DS.'com_hikashop'.DS.'helpers'.DS.'helper.php')) {
		...
		if($config->get('activate_sef',1)){
			...
			if(!empty($query['Itemid']) && isset($query['cid'])
				&& (
					(isset($query['ctrl']) && $query['ctrl'] === 'category'
						&& (!isset($query['task']) || $query['task'] === 'listing'))
					|| (isset($query['view']) && $query['view'] === 'category'
						&& (!isset($query['layout']) || $query['layout'] === 'listing'))
				) && HIKASHOP_J30) {
				$menuItem = JFactory::getApplication()->getMenu()->getItem((int)$query['Itemid']);
				if($menuItem && !empty($menuItem->link)) {
					$menuCat = 0;
					if($menuItem->link === 'index.php?option=com_hikashop&view=category&layout=listing') {
						$catParams = $menuItem->getParams()->get('hk_category');
						if(isset($catParams->category)) $menuCat = (int)$catParams->category;
					} elseif($menuItem->link === 'index.php?option=com_hikashop&view=product&layout=listing') {
						$prodParams = $menuItem->getParams()->get('hk_product');
						if(isset($prodParams->category)) $menuCat = (int)$prodParams->category;
					}
					if($menuCat > 0 && $menuCat === (int)$query['cid']) {
						unset($query['ctrl']);
						unset($query['task']);
						unset($query['view']);
						unset($query['layout']);
		...
		if(isset($query[$pathway_sef_name])&& (empty($query[$pathway_sef_name])) || $simplified_breadcrumbs){
		...
		if(isset($query[$related_sef_name])&& $simplified_breadcrumbs){
